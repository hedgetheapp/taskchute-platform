...
Cloudflare Access is optional and is not required by this initial posture. The persistent non-production deployment and verification history is recorded in `docs/CURRENT.md` and `docs/TEST_MATRIX.md`. Routine R2B persistent non-production migration / deploy / authenticated representative verification passed on 2026-09-01. D-049 initial production creation, migration, secure bootstrap, deployment, baseline recovery evidence, and signed-in smoke also passed on 2026-09-01; production deep feature mutation remains outside that smoke scope. The public production Worker remains bootstrap-disabled and public signup remains disabled.

## Approved production target

D-049 production resources are established: the named `production` Wrangler environment, Worker `taskchute-web-production`, and separate D1 databases `taskchute-auth-production` / `taskchute-app-production` using the `apac` location hint. The production environment enables its canonical `workers.dev` route and disables version preview URLs. The public Worker keeps `BOOTSTRAP_ENABLED=false`; initial provisioning used a loopback-only local Worker with explicitly remote production D1 bindings and ignored secret material. Do not copy non-production domain history into production, enable public signup, or enable bootstrap on the public Worker.
