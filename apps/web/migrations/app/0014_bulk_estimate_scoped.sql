...
-- Compatibility-only extension: preserve every existing routine guard and operation row/schema.
CREATE TABLE routine_command_guards_bulk_estimate_scoped (
...
INSERT INTO routine_command_guards_bulk_estimate_scoped SELECT * FROM routine_command_guards;
DROP TABLE routine_command_guards;
ALTER TABLE routine_command_guards_bulk_estimate_scoped RENAME TO routine_command_guards;

CREATE TABLE operations_bulk_estimate_scoped (
  app_user_id TEXT NOT NULL,
  operation_id TEXT NOT NULL,
  command_type TEXT NOT NULL CHECK (command_type IN (
...
INSERT INTO operations_bulk_estimate_scoped SELECT * FROM operations;
DROP TABLE operations;
ALTER TABLE operations_bulk_estimate_scoped RENAME TO operations;
