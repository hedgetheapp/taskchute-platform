...
-- D-103 extends the shared Document foundation.  Rebuild only the typed
-- CHECK constraint; every existing Document and Task Primary relation is
-- copied verbatim and no historical data is rewritten.
CREATE TABLE documents_d103 (
  document_id TEXT PRIMARY KEY NOT NULL,
  app_user_id TEXT NOT NULL,
  kind TEXT NOT NULL CHECK (kind IN ('standalone', 'task_primary', 'project_primary')),
  title TEXT,
...
CREATE TABLE task_primary_documents_d103 (
  app_user_id TEXT NOT NULL,
  ...
  FOREIGN KEY (app_user_id, document_id, document_kind)
    REFERENCES documents_d103(app_user_id, document_id, kind) ON DELETE RESTRICT
);

INSERT INTO task_primary_documents_d103
...
ALTER TABLE documents_d103 RENAME TO documents;
ALTER TABLE task_primary_documents_d103 RENAME TO task_primary_documents;
...
CREATE TABLE project_primary_documents (
  app_user_id TEXT NOT NULL,
...
-- DeleteProject needs to carry the linked Document identity across the
-- guarded relation delete so it can remove exactly one Project Primary row.
-- This is a short-lived command assertion, not a persisted Project relation.
ALTER TABLE project_command_guards ADD COLUMN project_primary_document_id TEXT;

-- Preserve all existing operation rows while allowing only the two new
-- Project Primary commands.
CREATE TABLE operations_d103 (
  app_user_id TEXT NOT NULL,
...
INSERT INTO operations_d103 SELECT * FROM operations;
DROP TABLE operations;
ALTER TABLE operations_d103 RENAME TO operations;
