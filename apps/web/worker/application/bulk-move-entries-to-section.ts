import type { BulkMoveEntriesToSectionRequest, BulkMoveEntriesToSectionResult } from "../../src/shared/contracts";
...
export function isBulkMoveEntriesToSectionRequest(value: unknown): value is BulkMoveEntriesToSectionRequest {
...
interface DayRow { logical_date: string; placement_revision: number }
interface TargetRow {
  id: string;
...
async function reject(
  db: D1Database,
  appUserId: string,
  request: BulkMoveEntriesToSectionRequest,
  requestFingerprint: string,
  code: "resource_not_found" | "resource_conflict",
  message: string,
): Promise<BulkMoveEntriesToSectionResult> {
  return persistRejection<BulkMoveEntriesToSectionResult>(db, {
    appUserId,
    operationId: request.operation_id,
    commandType: "BulkMoveEntriesToSection",
    requestFingerprint,
...
export async function bulkMoveEntriesToSection(
  db: D1Database,
  appUserId: string,
  request: BulkMoveEntriesToSectionRequest,
  ...
  if (prior) return replayOperation<BulkMoveEntriesToSectionResult>(prior, "BulkMoveEntriesToSection", requestFingerprint);
  ...
  if (request.section_id !== null && sectionResult.results.length === 0) {
    return reject(db, appUserId, request, requestFingerprint, "resource_not_found", "Section is unavailable in this TaskChuteDay");
  }
  ...
  if (day.logical_date < currentLogicalDate) {
    return reject(db, appUserId, request, requestFingerprint, "resource_conflict", "Bulk Section change is unavailable for a past TaskChuteDay");
  }
  if (day.logical_date === currentLogicalDate && request.section_id !== null) {
    const targetSection = sectionResult.results[0] as { actual_end_instant?: string | null } | undefined;
    if (targetSection?.actual_end_instant && Date.parse(targetSection.actual_end_instant) <= Date.parse(now)) {
      return reject(db, appUserId, request, requestFingerprint, "resource_conflict", "The destination Section has already ended for the current TaskChuteDay");
    }
  }
  ...
  if (targets.some((target) => target.lifecycle_state !== "planned")) {
    return reject(db, appUserId, request, requestFingerprint, "resource_conflict", "Only planned Entries can have their Section changed in bulk");
  }
  if (targets.some((target) => target.routine_occurrence_id !== null)) {
    return reject(db, appUserId, request, requestFingerprint, "resource_conflict", "Routine Entries cannot be included in a bulk Section change");
  }
  ...
  const changedIds = request.entry_ids.filter((id) => {
    const entry = targetById.get(id)!;
    return entry.section_id !== request.section_id || entry.planned_start_minute !== targetPlannedStart;
  });
  const changedSet = new Set(changedIds);

  const displayRows = (await db.prepare(`SELECT e.id, e.section_id, e.position, e.lifecycle_state,
      e.planned_start_minute, e.routine_occurrence_id, COALESCE(c.context_order, -1) AS context_order
    FROM entries e
   ...
   ORDER BY CASE WHEN e.section_id IS NULL THEN -1 ELSE c.context_order END, e.position, e.id`)
    .bind(appUserId, request.taskchute_day_id).all<TargetRow & { context_order: number }>()).results;
  ...
  const positionUpdates = movingIds.map((entryId, index) => ({ entry_id: entryId, position: nextPosition + index }));
  ...
  const result: BulkMoveEntriesToSectionResult = {
    taskchute_day_id: request.taskchute_day_id,
    entry_ids: request.entry_ids,
    changed_entry_ids: changedIds,
    section_id: request.section_id,
    planned_start_minute: targetPlannedStart,
    placement_revision: request.expected_placement_revision + (changed ? 1 : 0),
  };
  const assertionId = `bulk-section:${request.operation_id}`;
  ...
  try {
    const [guard, , , , assertion, operationPersist] = await db.batch([
      ...
      db.prepare(`WITH requested(entry_id, position) AS (
        ...
        WHERE app_user_id = ? AND taskchute_day_id = ? AND lifecycle_state = 'planned'
          AND routine_occurrence_id IS NULL AND id IN (SELECT entry_id FROM requested)
      ...
      db.prepare(`UPDATE taskchute_days SET placement_revision = placement_revision + 1
        WHERE app_user_id = ? AND id = ? AND placement_revision = ? AND ? = 1
          AND EXISTS (SELECT 1 FROM placement_command_guards WHERE app_user_id = ? AND operation_id = ?)`)
        .bind(appUserId, request.taskchute_day_id, request.expected_placement_revision, changed ? 1 : 0,
          appUserId, request.operation_id),
      db.prepare(`INSERT INTO transaction_assertions (app_user_id, id, ok)
        SELECT ?, ?, CASE WHEN
          ...
          AND NOT EXISTS (
            ...
            JOIN entries e ON e.app_user_id = ? AND e.taskchute_day_id = ?
              ...
              AND e.position = CAST(json_extract(requested.value, '$.position') AS INTEGER)
            WHERE e.section_id IS NOT ? OR e.lifecycle_state != 'planned' OR e.routine_occurrence_id IS NOT NULL
          )
          THEN 1 ELSE 0 END
        ...
        .bind(appUserId, assertionId, appUserId, request.taskchute_day_id, result.placement_revision,
          snapshotJson, appUserId, request.taskchute_day_id, request.section_id, targetPlannedStart,
          positionUpdatesJson, appUserId, request.taskchute_day_id, request.section_id,
          appUserId, request.operation_id),
      db.prepare(`INSERT INTO operations (app_user_id, operation_id, command_type, request_fingerprint_version,
        ...
        SELECT ?, ?, 'BulkMoveEntriesToSection', ?, ?, 'success', ?, ?
    ...
    if (guard.meta.changes === 0) {
      ...
      if (committed) return replayOperation<BulkMoveEntriesToSectionResult>(committed, "BulkMoveEntriesToSection", requestFingerprint);
      ...
      if (latest?.placement_revision !== request.expected_placement_revision) {
        return persistRejection<BulkMoveEntriesToSectionResult>(db, {
          appUserId, operationId: request.operation_id, commandType: "BulkMoveEntriesToSection", requestFingerprint,
          outcomeKind: "revision_conflict", result: { code: "revision_conflict", message: "The placement revision is stale" },
        });
      }
      return reject(db, appUserId, request, requestFingerprint, "resource_conflict", "Selected Entries changed before the bulk Section change could commit");
    }
    if (assertion?.meta.changes === 0 || operationPersist?.meta.changes === 0) {
      throw new HttpError(503, "infrastructure_ambiguous", "The bulk Section change did not converge", true);
    }
    return result;
  } catch {
    const committed = await readOperation(db, appUserId, request.operation_id);
    if (committed) return replayOperation<BulkMoveEntriesToSectionResult>(committed, "BulkMoveEntriesToSection", requestFingerprint);
    throw new HttpError(503, "infrastructure_ambiguous", "The bulk Section change outcome is unknown; reload canonical state and retry", true);
  }
}
