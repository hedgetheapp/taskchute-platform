...
interface EntryRow extends DayRow {
  entry_id: string;
...
function isCommonCorrectionRequest(value: unknown): value is Record<string, unknown> {
...
export function isSetExecutionTimesRequest(value: unknown): value is SetExecutionTimesRequest {
  return isCommonCorrectionRequest(value);
}
...
async function reject<T>(
  ...
  commandType: "RevertEntryStart" | "SetExecutionTimes",
...
async function readEntry(db: D1Database, appUserId: string, entryId: string): Promise<EntryRow | null> {
  return db.prepare(`SELECT e.id AS entry_id, e.taskchute_day_id, e.lifecycle_state, e.section_id, e.planned_start_minute, e.position,
      d.id, d.placement_revision, d.start_instant, d.end_instant
...
export async function revertEntryStart(
  ...
  if (prior) return replayOperation(prior, "RevertEntryStart", requestFingerprint);
  const [entry, execution] = await Promise.all([
  ...
  if (converged) return replayOperation(converged, "RevertEntryStart", requestFingerprint);
  if (!entry || !execution) return reject(db, appUserId, request, "RevertEntryStart", requestFingerprint,
    "resource_not_found", "Entry or active Execution is unavailable");
  if (execution.entry_id !== request.entry_id || execution.ended_at !== null
    || entry.lifecycle_state !== "running" || !sameInstant(execution.started_at, request.expected_started_at)) {
    return reject(db, appUserId, request, "RevertEntryStart", requestFingerprint, "resource_conflict",
      "Only the current active Start can be reverted");
  }
  ...
  try {
    const [guard] = await db.batch([
      db.prepare(`INSERT INTO lifecycle_command_guards (app_user_id, operation_id, entry_id, execution_id, command_type)
        SELECT ?, ?, e.id, x.id, 'RevertEntryStart'
         ...
         WHERE e.app_user_id = ? AND e.id = ? AND e.lifecycle_state = 'running'
           AND x.id = ? AND x.ended_at IS NULL AND julianday(x.started_at) = julianday(?)`)
        .bind(appUserId, request.operation_id, appUserId, request.entry_id, request.execution_id, request.expected_started_at),
      ...
      db.prepare(`UPDATE entries SET lifecycle_state = 'planned' WHERE app_user_id = ? AND id = ? AND lifecycle_state = 'running'
        AND EXISTS (SELECT 1 FROM lifecycle_command_guards WHERE app_user_id = ? AND operation_id = ?)`)
      ...
      db.prepare(`INSERT INTO transaction_assertions (app_user_id, id, ok) SELECT ?, ?, CASE WHEN
        ...
        .bind(appUserId, assertionId, appUserId, request.entry_id, appUserId, request.execution_id, appUserId, request.operation_id),
      db.prepare(`INSERT INTO operations (app_user_id, operation_id, command_type, request_fingerprint_version,
        ...
        SELECT ?, ?, 'RevertEntryStart', ?, ?, 'success', ?, ?
          WHERE EXISTS (SELECT 1 FROM lifecycle_command_guards WHERE app_user_id = ? AND operation_id = ?)`)
        .bind(appUserId, request.operation_id, REQUEST_FINGERPRINT_VERSION, requestFingerprint, JSON.stringify(result), new Date().toISOString(), appUserId, request.operation_id),
    ...
    if (guard.meta.changes === 0) {
      ...
      if (committed) return replayOperation(committed, "RevertEntryStart", requestFingerprint);
      return reject(db, appUserId, request, "RevertEntryStart", requestFingerprint, "resource_conflict",
        "Only the current active Start can be reverted");
    }
    return result;
...
export async function setExecutionTimes(
  ...
  const requestFingerprint = await fingerprint(request);
  const prior = await readOperation(db, appUserId, request.operation_id);
  if (prior) return replayOperation(prior, "SetExecutionTimes", requestFingerprint);
  ...
  const converged = await readOperation(db, appUserId, request.operation_id);
  if (converged) return replayOperation(converged, "SetExecutionTimes", requestFingerprint);
  if (!entry) return reject(db, appUserId, request, "SetExecutionTimes", requestFingerprint, "resource_not_found", "Entry is unavailable");

  if (request.expected_lifecycle_state !== entry.lifecycle_state) {
    return reject(db, appUserId, request, "SetExecutionTimes", requestFingerprint, "resource_conflict", "Entry lifecycle state changed");
  }
  if (request.expected_lifecycle_state === "completed" && request.ended_at === null) {
    return reject(db, appUserId, request, "SetExecutionTimes", requestFingerprint, "resource_conflict",
      "A completed Entry cannot be reopened");
  }
  if (request.expected_lifecycle_state === "planned") {
    if (execution || request.expected_started_at !== null || request.expected_ended_at !== null) {
      return reject(db, appUserId, request, "SetExecutionTimes", requestFingerprint, "resource_conflict", "A planned Entry must create a new Execution");
    }
  }
  else {
    if (!execution || execution.entry_id !== request.entry_id || execution.ended_at === null && request.expected_lifecycle_state === "completed"
      || execution.ended_at !== null && request.expected_lifecycle_state === "running") {
      return reject(db, appUserId, request, "SetExecutionTimes", requestFingerprint, "resource_conflict", "Execution is no longer editable in this lifecycle state");
    }
    if (request.expected_started_at === null || !sameInstant(execution.started_at, request.expected_started_at)
      ...
      return reject(db, appUserId, request, "SetExecutionTimes", requestFingerprint, "resource_conflict", "Execution changed before correction");
    }
  }
  const windowError = validateActualWindow(request, entry, now);
  if (windowError) return reject(db, appUserId, request, "SetExecutionTimes", requestFingerprint, "resource_conflict", windowError);
  ...
  if (request.expected_placement_revision !== undefined && entry.placement_revision !== request.expected_placement_revision) {
    return persistRejection(db, { appUserId, operationId: request.operation_id, commandType: "SetExecutionTimes",
      requestFingerprint, outcomeKind: "revision_conflict", result: { code: "revision_conflict", message: "The placement revision is stale" } });
  }
  ...
  try {
    ...
    if (movesFromUnsectioned && placementGuard.meta.changes === 0) {
      const committed = await readOperation(db, appUserId, request.operation_id);
      if (committed) return replayOperation(committed, "SetExecutionTimes", requestFingerprint);
      return persistRejection(db, { appUserId, operationId: request.operation_id, commandType: "SetExecutionTimes", requestFingerprint,
        outcomeKind: "revision_conflict", result: { code: "revision_conflict", message: "The placement revision is stale" } });
    }
    if (guard.meta.changes === 0) {
      const committed = await readOperation(db, appUserId, request.operation_id);
      if (committed) return replayOperation(committed, "SetExecutionTimes", requestFingerprint);
      return reject(db, appUserId, request, "SetExecutionTimes", requestFingerprint, "resource_conflict", "Execution correction did not converge");
    }
    return result;
  } catch {
    const committed = await readOperation(db, appUserId, request.operation_id);
