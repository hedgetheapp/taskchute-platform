...
export async function reorderEntries(db: D1Database, appUserId: string, request: ReorderEntriesRequest): Promise<ReorderEntriesResult> {
  ...
  const [dayResult, sectionResult, entriesResult] = await db.batch([
    ...
    db.prepare(`SELECT id, position FROM entries WHERE app_user_id = ? AND taskchute_day_id = ? AND section_id = ? ORDER BY position, id`)
  ...
  const currentIds = entriesResult.results.map((row) => (row as { id: string }).id);
  ...
  const orderedEntryIdsJson = JSON.stringify(request.entry_ids);
  try {
    const statements: D1PreparedStatement[] = [
      ...
      db.prepare(`UPDATE entries SET position = position + ? WHERE app_user_id = ? AND taskchute_day_id = ? AND section_id = ?
        AND EXISTS (SELECT 1 FROM placement_command_guards WHERE app_user_id = ? AND operation_id = ?)`)
        .bind(request.entry_ids.length, appUserId, request.taskchute_day_id, request.section_id, appUserId, request.operation_id),
      db.prepare(`WITH requested(entry_id, target_position) AS (
        ...
        WHERE app_user_id = ? AND taskchute_day_id = ? AND section_id = ?
          ...
          AND EXISTS (SELECT 1 FROM placement_command_guards WHERE app_user_id = ? AND operation_id = ?)`)
        .bind(orderedEntryIdsJson, appUserId, request.taskchute_day_id, request.section_id, appUserId, request.operation_id),
      ...
      db.prepare(`INSERT INTO transaction_assertions (app_user_id, id, ok) SELECT ?, ?, CASE WHEN
        ...
        AND (SELECT COUNT(*) FROM entries WHERE app_user_id = ? AND taskchute_day_id = ? AND section_id = ? AND position BETWEEN 1 AND ?) = ?
        AND NOT EXISTS (
          SELECT 1 FROM json_each(?) requested
          LEFT JOIN entries e ON e.app_user_id = ? AND e.taskchute_day_id = ? AND e.section_id = ?
            AND e.id = CAST(requested.value AS TEXT) AND e.position = CAST(requested.key AS INTEGER) + 1
          WHERE e.id IS NULL
        )
        ...
        .bind(appUserId, assertionId, appUserId, request.taskchute_day_id, result.placement_revision,
          appUserId, request.taskchute_day_id, request.section_id, request.entry_ids.length, request.entry_ids.length,
          orderedEntryIdsJson, appUserId, request.taskchute_day_id, request.section_id,
      ...
      db.prepare(`INSERT INTO operations (app_user_id, operation_id, command_type, request_fingerprint_version, request_fingerprint, outcome_kind, result_json, created_at)
        SELECT ?, ?, 'ReorderEntries', ?, ?, 'success', ?, ? WHERE EXISTS (SELECT 1 FROM placement_command_guards WHERE app_user_id = ? AND operation_id = ?)`)
