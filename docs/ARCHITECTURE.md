...
The Day projection left-joins only `primary_document_id`; Markdown body and Note editor state are not loaded into the Day table. Authenticated Ensure/Update routes reuse the existing operation fingerprint, assertion, replay, and revision-CAS conventions. The shared URL `/?view=note&document=<document-id>` identifies the owner-scoped Document, and a read-only resolver distinguishes standalone and Task Primary projections without falling back on missing or cross-owner IDs. A same-origin new-tab bootstrap may ensure a missing relation and then replace its location with the canonical GET permalink; ordinary GET remains non-mutating. No realtime coordination, attachment storage, or new runtime service is introduced.

## D-102 Multi-Task Note window ownership

D-102ではmulti-windowのpresentation coordinationを`App`のmemory-only `taskNoteWindows` registryに集約する。registryはTask/Document stable identity、preferred initial geometry、stack order、focus/restore/outside request counter、dirty/unresolved state、対象windowのflush callbackを保持し、single canonical routeはactive Task Noteだけを指す。`TaskNoteEditor`は各instance内でdraft、CAS/ambiguity、beforeunload、drag/resize/minimize、keyboard focusを保持するため、window追加はDocument command/APIやserver identityを増やさない。
