...
- D-043でeditable planned EntryのSection / planned-start full synchronizationをApprovedした。D-031 / D-039の旧clear / explicit move semanticsはsupersedeされたが、runtimeはまだ旧挙動でD-043 implementation / verificationは`NOT_IMPLEMENTED`。D-044 / D-045 / D-046でR2A first-slice scope/UX/propagation、APP migration、legacy normalization、typed persistence directionをApprovedしたが、exact implementationは未着手でruntime / migration / verificationは`NOT_IMPLEMENTED`。
...
UI-1のstructural prerequisite完了に加え、D-044 / D-045 / D-046でR2A first-slice scope-choice / override / reset / propagation、legacy normalization、typed occurrence persistence、default concurrency controlのDecisionは解決した。exact migration SQL、HTTP / DTO / operation naming、propagation statement、evidenceに基づくindex判断はimplementation reviewが残り、runtime / migration / verificationは未着手である。

D-043〜D-046を一貫して実装するRoutine R2A first-slice implementation Task Contractが次のcandidateである。typed physical directionとAPP migrationはApproved済みだが、legacy authorityを解決できないdata、current D1 safe boundsを越えるpropagation、D-045 / D-046を越えるcompatibility changeが判明した場合はMaterial reviewへ戻す。real-browser controlled inclusive end-dateとdeployed non-null inclusive-date subcheckはtooling boundaryにより`NOT_VERIFIED`を維持し、productionは別gateとする。
