...
- Normal build、exact nonprod build、Wrangler nonprod dry-run、full Web、migration helper、real-local browser、persistent nonprod browser、DB read-only evidenceはこのsection更新時点では未完了であり、結果確定後に追記する。D-081はschema / migration / dependency / API semantic changeを必要としない想定で、migrationは`NOT_REQUIRED`として扱う。production / restore / destructive cleanup / Releaseは行わない。
