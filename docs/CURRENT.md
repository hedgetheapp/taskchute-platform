...
既存の認証済みbrowser sessionで、非機微なsynthetic Task `D116B QA 20260916`を作成・開始・完了し、Entry historical Projectを`D103 Browser Fixture`、historical Modeを`D105 Browser Mode`へ補正した。DBでshared Task `project_id=NULL`とEntry snapshot Projectの差、Mode snapshotを確認後、Todayの「将来のルーティンを作成」を一度実行した。RoutineはEntry snapshot Project/Modeを採用し、daily・開始日`2026-09-17`・enabled・新しいTask/Routine identityで作成された。source Entryはcompleted、`routine_occurrence_id=NULL`、Execution 1件のまま。correlationと成功operationは各1件で、reload後も作成済み表示へ収束し、再作成actionは出なかった。Day `2026-09-17`のpreviewを合成trigger Taskで確立したが、canonical Routine materializerはcurrent Day loadで実行されるため、次logical day開始前のfuture occurrenceはまだ`0`。future materialization exactly-onceはWorker/D1 testでPASSだが、このbrowser subcaseは`NOT_RUN`（2026-09-17がcurrent logical dayになる前）とする。合成fixtureはnonprodに残置（source correlationのRESTRICT意味論があるためsourceは削除していない）。演習中browser consoleのwarning/errorは取得結果`0 / 0`。Production `NOT_RUN`、restore `NOT_RUN`、Released `NO`。
