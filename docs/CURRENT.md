...
- D-082はApproved Decisionとして、current established DayのSection boundary到達時に、設定が有効な場合だけ過去timed Sectionの未実行planned ordinary / Routine-derived Entryをcurrent SectionへServer-authoritatively移動する。planned startはcurrent Section logical startへ更新し、ordinaryのsource relative orderとtarget cohortへの後置、RoutineのSection override、D-081 historical physical position、placement revision exactly onceを保持する。past / future / preview / Sectionなし / running / completed / suppressed Routineは対象外。APP migration `0024_auto_carry_overdue_planned.sql`を追加し、AUTH schemaは変更していない。
