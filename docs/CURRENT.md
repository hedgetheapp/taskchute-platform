...
D-043 `Section placement and planned start are fully synchronized`はApproved。通常のeditable planned stateでは、開始予定の設定・変更から`[start, end)`でreal Sectionをderiveし、explicit real Section選択からSection開始minuteを設定する。開始予定clearと`Sectionなし`選択はいずれもSection absence + `NULL`へ同期する。D-031 / D-039の旧clear / explicit move clausesはこの範囲でsupersedeされたが、current runtimeは旧挙動のままでD-043 implementation / verificationは`NOT_IMPLEMENTED`である。

D-044でRoutine R2A first sliceのProduct / Domain / UX semanticsをApprovedした。current-Day planned Routine-derived Entryの`Section + 開始予定`同期unitと独立見積unitを対象に、no-write candidate、unit別のexplicit `今回だけ / ルーティンに反映`、persistent explicit-NULL occurrence override、current Routine defaultへのreset、eligible non-overridden planned occurrenceへのpropagation、future no-materialization、historical protectionを確定した。D-045でlegacy editable real Section + planned start NULLをauthoritative Section startへnormalizeし、解決不能時は推測・partial rewriteなしで停止する方針をApprovedした。APP migration requirementはApprovedだがphysical schema / exact commandは未確定で、runtime / migration / verificationは`NOT_IMPLEMENTED`である。current R1 runtimeのRoutine由来Section / 開始予定 / 見積は引き続きread-onlyである。
...
- D-043でeditable planned EntryのSection / planned-start full synchronizationをApprovedした。D-031 / D-039の旧clear / explicit move semanticsはsupersedeされたが、runtimeはまだ旧挙動でD-043 implementation / verificationは`NOT_IMPLEMENTED`。D-044 / D-045でR2A first-slice scope/UX/propagation、APP migration requirement、legacy normalization policyをApprovedしたが、physical schema / exact commandは未確定でruntime / migration / verificationは`NOT_IMPLEMENTED`。
...
UI-1のstructural prerequisite完了に加え、D-044 / D-045でR2A first-slice scope-choice / override / reset / propagationとlegacy normalizationのProduct semanticsは解決した。physical override schema、default concurrency control、exact command / operation shape、index、SQL propagationはimplementation design reviewが残り、runtime / migration / verificationは未着手である。

D-043 synchronizationとD-044 / D-045 Routine R2A first sliceを一貫して実装するphysical design review / implementation Task Contractが次のcandidateである。APP migrationはApproved in principleだが、exact schema / commandをdocs-only approvalから自動確定せず、legacy authorityを解決できないdataやD-045を越えるcompatibility changeが判明した場合はMaterial reviewへ戻す。real-browser controlled inclusive end-dateとdeployed non-null inclusive-date subcheckはtooling boundaryにより`NOT_VERIFIED`を維持し、productionは別gateとする。
