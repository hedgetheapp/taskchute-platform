...
**Status: A/C/D/E/F/G/H/I IMPLEMENTED / INTEGRATED / focused PASS / AVD PARTIAL / B DEFERRED / Galaxy S23 NOT_RUN / Production NOT_RUN / Released NO**。Routine repeat icon accent、Routine occurrence-only delete、planned Routine occurrence D&D、Quick Add / Startのoptimistic canonical ordering、Routine planned-start Section同期、established past planned ordinary Taskのcurrent/future forward move、Today Add FABのtemporary bounded dragを実装した。Bのmanual minute-granularity actual-start補正はProduct Owner承認により完全保留で、input_precisionやWorker execution-correctionには触れていない。

Implementation b8a84a99a69a0a90f9791217abe1d5fbf145e260。Focused JVM、instrumentation compile、debug assemble、exact-SHA CI 36248977313、signed APK artifact taskchute-android-debug-b8a84a99a69a0a90f9791217abe1d5fbf145e260 / ID 10908412479はPASS。D-147A代表AVD 5 / 5はPASS、Today全体は35 / 44で既存fixture由来の9件を含むPARTIAL。MainActivity起動、APK install、crash bufferはPASS。Worker/API、shared contract、schema、migration、dependency、nonprod、productionは不変・未実施。Galaxy S23はNOT_RUN / PRODUCT_OWNER_MANUAL。
### D-138 Android Today Planning / Lifecycle Refinements v0.1 — 2026-09-24

**Decision: APPROVED / implementation: 83c910bd + test corrective ca455525 / Integrated: YES / focused verification: PASS / persistent nonprod: PASS / Galaxy S23 representative smoke: PASS / USER_CONFIRMED / Production: NOT_RUN / Released: NO**。completed-end minuteと同じplanned start、Routine occurrence-onlyのcurrent / established future Section-plan、ordinary / Routine Running estimate、Completed actual start/end projection、planned actual-start-only→RunningをD-138でcanonical化した。Workerの既存typed command / endpointとAndroidの既存editor authorityを拡張し、Routine Definition・recurrence・historical facts・placement revision semanticsは変更しない。local Worker 4 files / 38 tests、Android focused JVM 28 tests、Web typecheck、Android compile / instrumentation compile / assemble、exact-SHA CI、persistent nonprod safetyをPASSした。Focused AVD、authenticated Web / disposable QAはNOT_RUN。Galaxy S23 representative smokeはPASS / USER_CONFIRMED（Product Ownerがcurrent D-138 APKを「問題なし」と確認。網羅的device suiteではない）。migration / schema / dependency / production operationはない。
...
APP migration `0034_daily_primary_documents.sql`とpersistent nonprod runtime/DB safety evidence（Worker version `d6493c1a-602a-461c-a3e5-6f390d4eb4ce`、root `200`、protected API `401`、pending `0`、quick_check `ok`、FK empty、read-only `rows_written=0`）は従来証拠を維持。date-header correctiveのfocused unit `1 / 1`、`:app:compileDebugKotlin`、`:app:compileDebugAndroidTestKotlin`、`:app:assembleDebug`、`git diff --check`、exact-SHA CI `35958642230` Android JVM / signed APK PASS、Web/Worker SKIP。APK artifact `taskchute-android-debug-083b16454ae714e4ed96deb441e36c08ba34ab3c` / ID `10791685990` / expires `2026-10-01T05:11:20Z`。Daily-specific authenticated Web/runtime、Galaxy S23は`NOT_RUN / PRODUCT_OWNER_MANUAL`、既存data mutationはなし。
...
Focused Markdown JVM `18 / 18`、最終Android JVM `186 / 186`、Notes AVD `scripts/android-qa.ps1 -Surface Notes` `15 / 15`、`:app:compileDebugAndroidTestKotlin`、`:app:assembleDebug`、`git diff --check`はPASS。Standalone NotesとToday Task Primary Noteは従来どおり同じ`MarkdownLiveEditor`を利用する。D-135のraw Markdown、IME / composition、autosave / CAS / conflict / ambiguous retry / safe flush、Worker/API、Web、schema、migration、dependencyは不変。Galaxy S23 `NOT_RUN / PRODUCT_OWNER_MANUAL`、Production `NOT_RUN`、Released `NO`。

### D-135 Android Markdown Live Preview + IME Toolbar - 2026-09-23

**Decision: D-135 APPROVED / Implementation: IMPLEMENTED / Android JVM: PASS / Notes AVD: PASS / Galaxy S23: NOT_RUN / Production: NOT_RUN / Released: NO**。既存の共通live-preview editorへ、白いcaret、白いtask-list marker、inactive markerの安全な実タップとaccessibility actionによるchecked / unchecked切替を追加した。raw Markdownは変更対象markerだけを通常のonValueChange経由で更新し、D-111 autosave / CAS / conflict / ambiguous retry / safe flushは不変。focused Markdown JVM 14 / 14、最終Android JVM 182 / 182、Notes AVD 13 / 13、marker実タップ、:app:compileDebugAndroidTestKotlin、:app:assembleDebug、git diff --checkはPASS。Implementation 3d9781bd175ea35b95438597e80294c9a8a69ed8、exact-SHA CI 35835388710はclassifier / Android JVM / signed APK / instrumentation APK compile / signing / upload PASS、Web/Worker SKIP。artifact taskchute-android-debug-3d9781bd175ea35b95438597e80294c9a8a69ed8、ID 10739460429、expires 2026-09-30T08:09:39Z。Worker/API、Web、schema、migration、offline persistence、dependencyは変更なし。Galaxy S23はNOT_RUN / PRODUCT_OWNER_MANUAL、ProductionはNOT_RUN、ReleasedはNO。

### Android Today move-success feedback / Routine enabled toggle corrective — 2026-09-23

**Implementation: IMPLEMENTED / Integrated: YES / Focused JVM: PASS / Focused AVD: PASS / Galaxy S23: NOT_RUN / Production: NOT_RUN / Released: NO**。Todayの前日・次日・指定日MoveToDay成功時に永続成功文を表示しないようcallerだけを修正し、既存のpending / failure / ambiguous / retry semanticsとMoveToDay payloadは維持した。Routine一覧は既存SetRoutineEnabled / toggleRoutine経路のSwitchを状態ラベル付きで明示し、D-118どおり有効 / 無効を表示する。Implementation 3327d056a11ce99bb3d9d252330fbb40fb93c7a3、Today focused instrumentation 1 / 1 PASS、Settings focused instrumentation 1 / 1 PASS、Android JVM suite PASS、:app:compileDebugAndroidTestKotlin / :app:assembleDebug / git diff --check PASS。Exact-SHA CI 35821183604はclassifier / Android JVM / signed APK / instrumentation APK compile PASS、Web/Worker SKIP。APK artifact taskchute-android-debug-3327d056a11ce99bb3d9d252330fbb40fb93c7a3 / ID 10733521142。Galaxy S23はNOT_RUN / PRODUCT_OWNER_MANUAL、ProductionはNOT_RUN、ReleasedはNO。Worker/API、schema、migration、dependency、Webは変更していない。

### D-134 Android Running Progress Player — 2026-09-23

**Decision: D-134 APPROVED / Implementation: IMPLEMENTED / Focused JVM: PASS / Today AVD: PARTIAL (focused Running panel PASS; unrelated existing Today fixture failures remain) / Galaxy S23: NOT_RUN / Production: NOT_RUN / Released: NO**。current Dayのみ、既存RunningTaskPanelを104dp progress playerへ置換した。activeStartedAt、なければfirstStartedAtを表示専用tickerの開始時刻に使い、elapsed / remaining / estimate progress / overrunをtotal `HH:MM:SS`で表示する。正でないestimateはremaining / progress / overrunを表示せず、invalid startは`--:--:--`とする。Completeは既存commandを維持し、Future / Past境界、Worker/API、schema、migration、dependencyは変更していない。Implementation `419eb5a0a766c7837838f49fe45f5f4a731010d4`、最終Android JVM `168 / 168`、RunningProgress `8 / 8`、exact-SHA CI `35817804199` Android JVM / signed APK PASS、Web/Worker SKIP、artifact `taskchute-android-debug-419eb5a0a766c7837838f49fe45f5f4a731010d4` / ID `10732232016`。Today AVD full surfaceは`35 / 44`でpartial、Running panel focusedはPASS、final full Today rerunはNOT_VERIFIED。


### Android Completed Task end-time clipping corrective — 2026-09-22

**Implementation: IMPLEMENTED / Integrated: YES / Focused verification: PASS / AVD: NOT_RUN / Galaxy S23: NOT_RUN / Production: NOT_RUN / Released: NO**。Completed metadataの開始→終了時刻を85dpで優先確保し、実績durationを残余幅側でellipsis可能にした。Implementation `59bd426d5363295fc55d13db0a0f53e920ba3702`。Focused regressionは`09:12 → 16:21`と`100分`を確認するinstrumentation testを追加し、`:app:compileDebugAndroidTestKotlin`、`:app:assembleDebug`、`git diff --check`はPASS。Exact-SHA Actions `35731290718`はclassifier PASS、Android JVM / signed Debug APK / instrumentation APK compile / signing verification / artifact upload PASS、Web / WorkerはSKIP。APK artifact `taskchute-android-debug-59bd426d5363295fc55d13db0a0f53e920ba3702`、ID `10695960488`、expiry `2026-09-29T13:08:34Z`。`adb devices`に接続端末がなく、`emulator`コマンドも利用不可のためAVD/MainActivity/crash-buffer runtimeは未実施。QA Task作成、既存Task/Entry/Execution mutationはなし。Worker/API、schema、migration、dependency、productionは不変。
...
**Decision: D-133 APPROVED / Implementation: IMPLEMENTED / Integrated: YES / Tests: PASS / Persistent nonprod: PASS / Authenticated Web: NOT_RUN / Galaxy S23: NOT_RUN / Production: NOT_RUN / Released: NO**。Implementation `62d4d804db0683a6bc33cad0f4c4722212201fb0` adds the full Android Routine Create / Edit form for Task名・Project・Mode・Section・開始予定・見積・繰り返し・開始日・終了日. D-133 corrective `c7cced74e1fda8a718a76801f85ceb6bb3ef1886` centralizes the 14-family schedule validation and disables Schedule Sheet Apply for invalid or incomplete drafts, including malformed weekday tokens. The corrective focused helper test is `2 / 2 PASS` and exact-SHA CI run `35735293187` is PASS; APK artifact `taskchute-android-debug-c7cced74e1fda8a718a76801f85ceb6bb3ef1886` / ID `10697646632`. New Routine creation sends one atomic `CreateRoutine` request; the local Schedule Secondary Sheet remains draft-only until Add / Save. `900` / `0900` / `09:00` normalize to minute `540`. Worker focused `19 / 19 PASS`, Android Settings focused `18 / 18 PASS`（Controller `13 / 13`、HTTP `5 / 5`）、Web typecheck、Android debug assemble / instrumentation compile、`git diff --check`、exact-SHA Actions `35725615763`、fresh signed APK artifact、persistent nonprod deploy/runtime/read-only DB safety are PASS. APK `taskchute-android-debug-62d4d804db0683a6bc33cad0f4c4722212201fb0`、ID `10693353178`、expiry `2026-09-29T12:13:17Z`; Worker version `90d25052-a1f8-45e3-9f9f-6d12601265c3`; root `200`、protected API `401`、APP/AUTH quick_check `ok`、FK empty、read-only probes `rows_written=0`. CUA kernel resetのためauthenticated Web / isolated QAは`NOT_RUN / SAFE_SKIP`、既存data mutationはなし。Galaxy S23は`NOT_RUN / PRODUCT_OWNER_MANUAL`、schema / migration / dependency / Production / Releasedは変更・実施なし。
...
**Implementation: `ea137da294dc7f7e239228dca49bacc2ff8150f8` / Worker focused `7 / 7 PASS` / Web App focused `298 / 298 PASS` / Android `TaskPlanningHttpRepositoryTest` focused PASS / `npm run typecheck` PASS / `git diff --check` PASS / exact-SHA Actions run `35683758984` rerun PASS（初回はD-132変更外の既存`TodayDirectManipulationTest.deterministicFailureDismissalIsGenerationSafe` 1件のみ失敗、単体local再実行PASS）/ APK `taskchute-android-debug-ea137da294dc7f7e239228dca49bacc2ff8150f8`、ID `10675468961`、expiry `2026-09-29T03:45:03Z` / persistent nonprod deploy PASS、Worker version `73378404-e353-4aeb-b36d-3eafe7de3204` / root `200`、protected API `401`、disabled bootstrap `404`、APP/AUTH pending `0 / 0`、quick_check `ok`、FK empty、guards/assertions empty、read-only probes `rows_written=0` / authenticated Web `AUTHENTICATED_WEB_NOT_RUN`（CUA kernel reset、credentials/re-loginなし）/ QA object作成・既存data mutation・cleanupはなし / Galaxy S23 `NOT_RUN` / Production `NOT_RUN` / Released `NO`**。

既存active Executionが1件あったため、既存Executionを止めない安全境界によりRunning remote QAはSKIPした。認証済みWeb sessionを利用できなかったためCompleted remote QAも`NOT_RUN`。read-only snapshotではcurrent Day `2026-09-22`が1件、placement revision `14`、QA prefix `D132-QA-20260922-C7F4`の既存Task/Entryは0件だった。
...
**Implementation: IMPLEMENTED / Local: Android JVM 135/135 PASS, instrumentation compile PASS, debug assemble PASS / Authenticated runtime: NOT_RUN / HUNG (`scripts/android-qa.ps1 -Surface Today` reached `connectedDebugAndroidTest` without a result; MainActivity resolved and crash buffer was empty) / Exact-SHA CI: PASS (`35540773296`, head `b46d10ff5231a3d08157bea16ffb26b523420624`; Android JVM/signed APK/instrumentation compile PASS, Web/Worker SKIPPED; artifact `taskchute-android-debug-b46d10ff5231a3d08157bea16ffb26b523420624`, ID `10614224508`, expiry `2026-09-27T22:12:55Z`) / Old artifact Galaxy S23: FAIL / USER_REPORTED — immediate drag cancel / New corrective Galaxy S23: PASS / USER_REPORTED / Screenshot comparison: NOT_RUN / Production: NOT_RUN / Released: NO**。新correctiveはProduct OwnerのGalaxy S23実機確認で問題なしと報告され、physical-device evidenceを`PASS / USER_REPORTED`としてcloseする。AVD instrumentationの`NOT_RUN / HUNG`、screenshot comparison、Production、Releasedの状態は変更しない。
...
**Implementation: IMPLEMENTED / Local: Android JVM 135/135 PASS, instrumentation compile PASS, debug assemble PASS / Authenticated runtime: NOT_RUN / HUNG (`scripts/android-qa.ps1 -Surface Today` reached `connectedDebugAndroidTest` without a result; MainActivity resolved and crash buffer was empty) / Exact-SHA CI: PASS (`35539849451`; Android JVM/APK PASS, Web/Worker SKIPPED; artifact `taskchute-android-debug-aaa372086dd5e533ca5d0e32664bfa0c254490ce`, ID `10614048129`, expiry `2026-09-27T21:51:23Z`) / New corrective Galaxy S23: NOT_RUN / Screenshot comparison: NOT_RUN / Production: NOT_RUN / Released: NO**。旧artifactのfailure evidenceは履歴として保持し、新correctiveを実機Verifiedへ昇格させない。
...
The corrective gives each Quick Add reference picker explicit focus ownership and hides the text IME when switching from a text field. Drag presentation removes the source from the provisional list, inserts a separate teal placeholder, and renders the lifted Task as a pointer-following overlay; entry anchors exclude Completed and Routine-derived rows, while empty / Completed-only Sections use Section-only placement. Deterministic failure feedback has a generation token and an accessibility-aware transient dismissal; ambiguous, auth, and full-screen load failures remain persistent under D-125. No new Decision is created, and Server authority, operation identity, CAS, retry, API, Worker, schema, migration, dependency, and persistent local authority are unchanged.

**Implementation: IMPLEMENTED / Local: focused Android JVM PASS (26 tests), Android-test compile PASS / Focused authenticated runtime: NOT_RUN pending AVD run / Corrective Galaxy S23: NOT_RUN / Screenshot comparison: NOT_RUN / Production: NOT_RUN / Released: NO**。off-screen drag auto-scrollは今回追加していない。visibleな連続empty Sectionのstable logical bandsを対象とするfocused regressionを追加し、必要性が確認された場合に別correctiveで扱う。
...
Local evidence: D116B focused Worker/D1 `18 / 18`、delete/runtime focused regressionsを含む`3 files / 60 tests`、focused App `1 passed`、bounded migration `0001 → 0033` fresh / `0032 → 0033` upgrade preservationはPASS。full Worker/D1 `37 files / 330 tests`、full Web `14 files / 462 tests`、typecheck、normal build、exact nonprod build、deploy guard、Wrangler nonprod dry-run、`git diff --check`はPASS。Implementation commit `7e8fd058b5abee839ed5300a21fae2260d3e42ba`は`main`へfast-forward push済みで、exact-SHA GitHub Actionsのclassifier、Web/Worker、Android jobsはすべてPASS。Wrangler log-directory `EPERM`と既存Vite chunk-size warningは出たが、該当build/test/dry-runはexit `0`。

Persistent migration gateでは、APP export `1,079,292 bytes` / SHA-256 `610864C5A72A75A9AAA38EA3924A5379D1BF8568DD83C3DEFD04EAC3D1B80C5C`、AUTH export `11,235 bytes` / SHA-256 `E04055DD05AB549E43EF4502F413DAE1DD400A24F0E4C7207DDDFA920F7875B8`を取得し、別DBへ隔離importして両方`quick_check=ok`・FK emptyを確認した。restoreは行っていない。APP `0033_completed_entry_future_routine.sql`のみremote適用、AUTH migrationなし、pending `0 / 0`。適用前バックアップとの照合で既存13テーブルの件数と43種類のoperation command件数分布がすべて一致した。

Exact pushed mainをguarded canonical Worker `taskchute-web-nonprod`へdeployし、Worker version `fb2ddc25-2f0c-44da-bedc-27e5252f289b`を確認した。root `200`、未認証`POST /api/v1/entries/:entryId/future-routine` `401`。APP/AUTHは`quick_check=ok`、FK empty、pending `0 / 0`。APP 0033 relationの列・owner scoped FK・source/Routine uniqueness、operation allow-list、新テーブル/operationの重複・orphan・invalid JSON、Document kind/revision、temporary-table、assertion/guard residueはいずれも正常（anomaly `0`）。成功したread-only D1 probesは`rows_written=0`。

既存の認証済みbrowser sessionで、非機微なsynthetic Task `D116B QA 20260916`を作成・開始・完了し、Entry historical Projectを`D103 Browser Fixture`、historical Modeを`D105 Browser Mode`へ補正した。DBでshared Task `project_id=NULL`とEntry snapshot Projectの差、Mode snapshotを確認後、Todayの「将来のルーティンを作成」を一度実行した。RoutineはEntry snapshot Project/Modeを採用し、daily・開始日`2026-09-17`・enabled・新しいTask/Routine identityで作成された。source Entryはcompleted、`routine_occurrence_id=NULL`、Execution 1件のまま。correlationと成功operationは各1件で、reload後も作成済み表示へ収束し、再作成actionは出なかった。Day `2026-09-17`でcurrent Day loadを実行し、future Routine occurrenceは初回表示`1`、reload後も`1`だった。D1 occurrence countも`1`で、source Entryはcompleted ordinary、`routine_occurrence_id=NULL`、Execution count `1`のまま不変。read-only probeは`rows_written=0`。合成fixtureはnonprodに残置（source correlationのRESTRICT意味論があるためsourceは削除していない）。Production `NOT_RUN`、restore `NOT_RUN`、Released `NO`。
...
既存の認証済みbrowser tabで、Standalone Noteのcanonical Create後のtitle focus、autosave、reload後の再読込とMarkdown本文保持をsyntheticなfixtureで確認した。通常の可視Save actionはなく、状態表示は保存済みだった。追加のD-115 correctiveとして、Task Noteの初回Create→open後の同一Today行トグルclose、既存Task Noteのopen→同一行トグルclose、別Task Noteを残した対応Task Noteだけのclose、header `×` closeを確認した。Project Primary NoteはNotes inlineからfloatingへ移行し、inline側が競合writerとして残らず、syntheticなautosave後のreload/reopenで本文保持を確認した。console warning/errorは`0 / 0`。修正SHA `b8e2878632dc4a7a37dee0cd03cfcf044fb4e07c`をWorker version `3c574852-4a89-48b0-a2ee-ffc60aa6382e`へdeploy済み。fixtureはnonprodに残している。Productionは`NOT_RUN`、Releasedは`NO`。
...
全体configuration updateを使い、削除時は隣接Sectionへ時間帯を吸収する。Projectはactive / archived、
作成・編集・archive / restore・server-owned order・hard-delete確認、Routineは作成・編集・有効/停止・
既存canonical soft-deleteを利用する。owner scope、operation / revision、認証、データauthorityは既存
...
Workerはsame-origin / authenticated WebSocket upgradeをserver-derived app userへroutingし、wrong-origin・unauthenticated・ordinary HTTPを拒否する。Web clientはsigned-in tabごとにbounded reconnectを行い、401だけD-104 reauth barrierへ接続する。dirty/pending/unresolved local state、D-066 retained operation、Document draftは通知で上書きしない。focused protocol/client/DO tests、full Web `14 files / 442 tests`、full Worker/D1 `36 files / 307 tests`、typecheck、normal/exact nonprod build、guard、Wrangler dry-run、diff checkはPASSした。追加是正では`wss://` probeをHTTP probeへ変換し、Project mutationのDocument依存scopeを補い、同一microtask内の重複・wildcard Day invalidationをcoalesceし、DO bindingを単一にguardした。

最終exact pushed mainをcanonical persistent nonprod Worker `taskchute-web-nonprod`へ再deployし、Worker version `e21c68c9-6778-47a3-8e18-772c2425ecc6`、`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`、`REALTIME_HUB (RealtimeHub)` SQLite bindingを確認した。rootは`200`、protected `/api/v1/documents`と`/api/v1/realtime`は未認証`401`。APP/AUTH pendingは`0 / 0`、quick checkは`ok`、FKはempty、read-only probesは`rows_written=0`、document/position/active-execution/guard anomalyは0だった。APP/AUTH schema migrationは追加・適用していない。

今回のauthenticated browser closeoutでは、同一既存認証セッション上の独立タブA/Bを使用して伝播を確認し、Bを閉じた後に同じセッションのfresh tab Cでcanonical persistenceを確認した。手動login、credential取得、再loginは行っていない。同じcurrent Dayを開いたA/B間で、Task rename、Project rename、Mode作成、Routine作成が手動reloadなしに伝播し、いずれも概ね1秒以内に可視化した。Aで作成した`D105 Browser Note`について、cleanなBの表示へ本文更新が手動reloadなしに伝播し、重複editor/windowは発生しなかった。

Document safetyでは、Bで未保存の非機密dirty draftを保持したままAからcanonical本文を更新し、dirty draftがrealtime通知で上書きされないことを確認した。70,000文字級のdraftは既存D-104 UTF-8 payload hard-stop（`70,203 / 65,536 bytes`）が表示され、サーバー送信されなかったため、同じbrowser runでのCAS conflict save完了までは主張しない。Aのsame-tab reload後、および同じ既存認証セッションで開いたfresh tab Cで、Note本文`D105 canonical v4 from A`とtitleがcanonicalに復元された。

CUAにはsocketだけを切断して同一ページをreconnectさせる操作手段がないため、disconnect/reconnect browser E2Eは`NOT_RUN`（tab close/reopenやpage reloadをreconnect PASSとは扱わない）。公開HTTP read-only probeはroot `200`、未認証Documents / Realtime API `401`。browser consoleで取得できたA/Cのwarning/errorは`0 / 0`だった。browser操作後のWrangler D1 read-only再照会はCloudflare API `7403 (account not valid or not authorized)`で実行できず、直前deploy時のDB integrity evidenceを保持しつつ今回runのpost-browser D1再照会は`NOT_RUN`とする。

D-105 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / REALTIME_HUB_LOCAL_VERIFIED / REALTIME_INVALIDATION_PROTOCOL_VERIFIED / REALTIME_CLIENT_RECONNECT_VERIFIED_LOCAL / REALTIME_AUTH_ORIGIN_ISOLATION_VERIFIED / GITHUB_CI_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_TWO_BROWSER_PROPAGATION_VERIFIED / DOCUMENT_CLEAN_REFRESH_VERIFIED / DOCUMENT_DIRTY_DRAFT_PROTECTION_VERIFIED / SAME_TAB_RELOAD_VERIFIED / FRESH_AUTHENTICATED_TAB_VERIFIED / BROWSER_CONSOLE_VERIFIED / BROWSER_SOCKET_RECONNECT_NOT_RUN / POST_BROWSER_DB_RECHECK_NOT_RUN / APP_AUTH_MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。
...
Focused Webは`5 files / 328 tests PASS`、full Webは`13 files / 438 tests PASS`、full Worker/D1は`34 files / 301 tests PASS`、typecheck、normal build、exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`はPASSした。`npm run preflight`のworkflow helperだけはWindows `.git/FETCH_HEAD` permission errorで完走しなかったが、direct fetch、`git ls-remote`、branch/HEAD/origin一致を別途確認した。GitHub Actions run `34800601555`は追加テストSHA `cd8a4882cf0534e278ee948ce57a98571de831c4`に対してtypecheck、Web tests、Worker/D1 tests、production buildを含む全stepがsuccessである。

GitHub ruleset `Protect main history` (id `23241921`)は`main`のdeletionとnon-fast-forwardを禁止し、PR requirementとrequired status checksは有効化していないためdirect normal FF pushを維持する。非本番HTTPはroot `200`、未認証Documents API `401`、APP/AUTH migration listはともに`No migrations to apply!`。最終Worker `8d413923-1f2f-444c-b760-b4b4a06ac134`でAPP/AUTH `quick_check`は`ok`、FKはempty、read-only probeは`rows_written=0`、invalid document kind/revision、owner/document duplicate key、orphan document/operation/relation、assertion/guard residue、active executionはすべて`0`だった。
...
- Integrated to `main`: `YES`
- Released: `NO`
D-131 implementation commit `fb0d0542d4509912aaaa4e5e306ad65a395cc241` reconciles current and already-established future Day Section contexts to the latest active configuration while preserving established past contexts and leaving unestablished future Days unmaterialized. Planned entries keep `Sectionなし` + null planned start or are re-derived from the canonical Section interval; Routine-derived entries retain the D-043 pair; running / completed entries preserve lifecycle and execution facts and are rehomed only when their prior Section is removed. Affected Day `placement_revision` increments exactly once per successful update, canonical ordering remains stable, and exact same-operation replay does not increment again. The update uses the existing `UpdateSectionConfiguration` operation / replay / assertion path and does not change API route / DTO / command contracts, schema, migration, or dependency.

D-131 local evidence: focused Worker `63 / 63 PASS` (including current / established-future / past / unestablished-future, planned / Routine-derived / running / completed reconciliation, revision and replay coverage), additional Section interval / B3 / Day Navigation `38 / 38 PASS`, `npm run typecheck PASS`, and `git diff --check PASS`. The exact current `main` passed the canonical nonprod build and deploy guard, then deployed to `taskchute-web-nonprod` as Worker version `cb6f27b7-d9b8-4a4b-a2fb-c18c782c9dcf` at `2026-09-22T02:45:18.647Z`; root `200`, protected API `401`, APP/AUTH pending `0 / 0`, quick/FK, and read-only integrity probes passed. Authenticated Web D-131 A/B/C/D was `AUTHENTICATED_BROWSER_NOT_RUN` because the available CUA session reset its kernel; no credentials or user-visible Section mutation were attempted. Production and Release were not run. Classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / CURRENT_FUTURE_RECONCILIATION_VERIFIED_LOCAL / PAST_FROZEN / UNESTABLISHED_FUTURE_NOT_MATERIALIZED / REVISION_REPLAY_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / PERSISTENT_RUNTIME_DB_VERIFIED / AUTHENTICATED_BROWSER_NOT_RUN / API_CONTRACT_UNCHANGED / SCHEMA_UNCHANGED / MIGRATION_NOT_REQUIRED / DEPENDENCY_UNCHANGED / PRODUCTION_NOT_RUN / RELEASED_NO`.
...
- D-095はD-094 browser feedbackに対するWeb-only refinementとして実装した。TodayのMonday-first calendarを`CalendarPopover`へ抽出し、Routine開始日・終了日およびSettings > 営業日 / 休日の日付text fieldのclick / focusで同じcalendarを開くようにした。月heading、前年 / 前月 / 次月 / 翌年、selected / today / adjacent-month state、keyboard navigation、outside click、Escapeを共通化し、8桁`YYYYMMDD`とdashed`YYYY-MM-DD`のstrict logical-date入力、partial/invalid入力のno-write境界を維持した。
- Routine recurrence triggerはselect-like border/background・chevron・`aria-haspopup`・`aria-expanded`を備え、成功noticeは通常flowを占有しないfixed top-center `.transient-status`へ移行した。Notes line-number railは常設42pxの固定列・同一white surfaceとし、OFFはglyphだけを隠す。今回のbrowser geometryではON/OFFともeditor `306×341`、textarea `262×339`、rail `42px`で、left/top/heightが不変だった。Worker/API/Domain/schema/migration/dependency/binding/security postureは変更していない。
- Implementation commit `9a69893773520422c71e99e67ae308f623d6254f`を、canonical `main` `426bed0d7dd7726f1a632c8b856e363103ddca57`からfast-forward pushした。focused App/Routine/Notes `274 / 274`、full Worker/D1 `32 files / 285 tests`、full Web `5 files / 294 tests`、typecheck、normal build、exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`はPASS。build中の既知Wrangler log `EPERM`とclient chunk-size warningは非致命で、各command exit `0`だった。
- Exact pushed mainをcanonical persistent nonprod Worker `taskchute-web-nonprod`へdeployし、Worker versionは`f4a5d095-909a-46f5-848d-61ecceb19eb7`。guard / generated configは`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`を確認した。root `200`、未認証`GET /api/v1/documents` `401`、APP/AUTH migration pending `0 / 0`、両DB quick_check `ok`、FK empty、read-only probes `rows_written=0`、Documents invalid/duplicate checks `0`を確認した。
- 既存authenticated persistent tabでviewport約`592×884`（sidebar open時、zoomはCUAから正確取得不可）と、同じtabでsidebar closedのRoutine/Effective Day calendarを確認した。Today、Effective Day、Routine開始日・終了日は同じcalendar structure（month/year controls、月曜始まりheader/grid、selected/outside-month state）を表示し、text field click/focus起点、outside/Escape closeをPASS。Notesは非機密fixtureを同じtabでON→OFF→ONと切替し、同一geometry・white rail・OFF glyph非表示を計測、same-tab reload後も保存済みNote/行番号ONを確認した。fresh authenticated tab（約`1234×711`、sidebar open）でもNotes fixture/行番号ONとEffective Day calendarを確認した。CUA `dev.logs()`は`[]`でconsole warning/errorなし。
- 検証で既存の非機密Notes fixture以外の意味あるdataは変更していない。D-095はD-094を後からrefineしたものであり、D-094の履歴記録は変更しない。production、restore/recovery execution、credential取得・再設定、bootstrap変更、無関係なdestructive cleanup、branch / PR / merge / tag / releaseは行っていない。APP/AUTH migrationの追加・再適用はなく、Releasedは`NO`。

D-095 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / WEB_UI_CONSISTENCY_VERIFIED / ROUTINE_FLOATING_NOTICE_VERIFIED / ROUTINE_RECURRENCE_DROPDOWN_VERIFIED / TODAY_STYLE_SHARED_DATE_PICKER_VERIFIED / NOTES_STABLE_GUTTER_GEOMETRY_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_REPRESENTATIVE_VERIFIED / FRESH_TAB_PERSISTENCE_VERIFIED / DB_INTEGRITY_VERIFIED / CONSOLE_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`.

## D-095 corrective — Today-state parity and text-field-only calendar trigger

- canonical Decisionの追補に合わせ、`LogicalDateInput`へcanonical current logical dateを`todayDate`として明示的に渡した。Routine開始日・終了日、Settings > 営業日 / 休日ではselected dateとtoday dateを別stateとして描画し、selected dateが異なる場合もTodayと同じ`aria-current="date"`を保持する。Today pageのcurrent markerも回帰確認した。ブラウザdateをauthorityにはしていない。
- 日付text field横の独立calendar trigger button/iconと専用CSSを削除し、text fieldを唯一の通常triggerへ変更した。input自身に`aria-haspopup="dialog"` / `aria-expanded`を付与し、click / focus、直接8桁入力、Escape / outside close、calendar navigationを維持した。Worker/API/Domain/schema/migration/dependencyは変更していない。
- Corrective commit `704570bc7c618329f3a7bd41c780c59a38e5b43d`を`fe8554d8807a81556b5e6b1399bac26366ad2318`からfast-forward pushした。focused App/Routine/Notes `276 / 276`、full Web `5 files / 296 tests`、full Worker/D1 `32 files / 285 tests`、typecheck、normal/exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`はPASS。既知Wrangler log `EPERM`とclient chunk-size warningは非致命で、各command exit `0`だった。
- canonical persistent nonprod Worker `taskchute-web-nonprod`へdeployし、Worker versionは`a62d2f99-bec2-463c-b352-3d91d8594482`。`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`を確認した。root `200`、未認証Documents API `401`、APP/AUTH migration pending `0 / 0`、quick_check `ok`、FK empty、read-only probes `rows_written=0`、APP Documents `3` / invalid `0`を確認した。
- 同一authenticated tab（Today / Routine / Effective Day、約`592×884`）で、canonical today `2026-09-12`とRoutine selected `2026-09-10`の分離、Routine開始日・終了日、Effective Dayのtoday markerを確認した。独立calendar triggerは0件、input expanded state、8桁`20260910`の正規化、Escape / outside close、month/year navigationをPASS。same-session fresh tab（`1280×720`）でもRoutine trigger 0件、Effective Day selected/today、input expanded stateを確認し、console `[]`だった。既存fixtureのoverride mutationは行っていない。
- D-095のprior refinement evidenceは維持し、このcorrectiveでToday-state parityとtext-field-only triggerのgapを閉じた。production、restore/recovery execution、credential取得・再設定、bootstrap変更、destructive cleanup、branch / PR / merge / tag / release、migration追加・再適用は行っていない。Releasedは`NO`。

D-095 corrective classification: `APPROVED / CORRECTED / INTEGRATED / TESTED / MAIN_PUSHED / WEB_UI_CONSISTENCY_VERIFIED / ROUTINE_FLOATING_NOTICE_VERIFIED / ROUTINE_RECURRENCE_DROPDOWN_VERIFIED / TODAY_STYLE_SHARED_DATE_PICKER_VERIFIED / SHARED_CALENDAR_TODAY_STATE_VERIFIED / DATE_TEXT_FIELD_ONLY_TRIGGER_VERIFIED / NOTES_STABLE_GUTTER_GEOMETRY_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_REPRESENTATIVE_VERIFIED / FRESH_TAB_PERSISTENCE_VERIFIED / DB_INTEGRITY_VERIFIED / CONSOLE_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`.

## D-083 corrective — Empty real-Section Shift traversal

- Independent source review found that `Shift + ArrowUp / ArrowDown` built its cross-Section traversal list only from groups containing entries. An empty configured real Section could therefore be skipped, while a draft-only `Sectionなし` group could become a synthetic traversal stop. Corrective implementation commit `bdaa561c311661154aa11ea20f04adaa02a9b80f` traverses every configured real Section, includes `Sectionなし` only when an effective unsectioned Entry actually exists, uses the existing Section-only `MoveEntry` path for an empty target, and preserves the existing Routine scope chooser. No Worker/API/schema/migration/dependency change was introduced.
- Focused corrective Web coverage is `4 / 4 PASS`: immediate next empty real Section, one-empty-Section-at-a-time convergence, upward traversal, and Routine empty-target chooser. The final full Web suite is `5 files / 300 tests PASS`; full Worker/D1 is `32 files / 285 tests PASS`; typecheck, normal build, exact nonprod build, guarded deploy preflight, Wrangler nonprod dry-run, and `git diff --check` are PASS. Migration and dependency status are respectively `NOT_REQUIRED` and unchanged.
- Exact pushed main was deployed to canonical `taskchute-web-nonprod` as Worker version `4a4df4fa-21fd-42f5-9b43-ec5872d7a236`; generated config confirms `RUNTIME_ENV=nonprod`, `BOOTSTRAP_ENABLED=false`, APP `taskchute-app-nonprod`, and AUTH `taskchute-auth-nonprod`. Root `GET /` returned `200`, protected `GET /api/v1/projects` returned `401`, and APP/AUTH migration lists reported no pending migrations.
- Existing authenticated persistent browser session was reused with Sidebar open. Disposable fixtures `D083 Empty Browser A 20260912` (`01a09437-cfdb-7e96-b0a4-420668f2f237`) and `D083 Empty Browser C 20260912` (`01a09438-026a-7269-a909-4a54fdf844a8`) remained nonprod-only. A real `Shift + ArrowDown` moved A from Morning into the adjacent configured Day Section; same-tab reload and a fresh authenticated tab restored A in Day and C in Evening. The browser surface showed no `Sectionなし` summary, and no synthetic DOM event or internal reorder helper was used. The exact empty-target case was not separately browser-observable because the adjacent Day Section already contained pre-existing dogfood rows; its authority is the focused automated evidence above. CUA did not expose numeric viewport/zoom or console APIs, so those are `NOT_EXPOSED` / `NOT_VERIFIED`.
- Post-browser APP read-only evidence reports `PRAGMA quick_check=ok`, empty FK violations, `active_executions=0`, placement/lifecycle/routine/transaction guard residue `0`, duplicate positions `0`, duplicate Routine Entry/day groups `0`, and `rows_written=0` for successful probes. Final fixture state is planned A in Day at `720`, planned C in Evening at `1200`, with placement revision `32`; fixture rows remain as disposable verification data.

D-083 corrective classification: `APPROVED / CORRECTED / INTEGRATED / TESTED / MAIN_PUSHED / SHIFT_CROSS_SECTION_EMPTY_TARGET_VERIFIED_LOCAL / SECTIONLESS_NON_SYNTHESIS_VERIFIED / ROUTINE_SHIFT_EMPTY_TARGET_VERIFIED_LOCAL / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_REPRESENTATIVE_VERIFIED / SAME_TAB_RELOAD_VERIFIED / FRESH_AUTHENTICATED_TAB_VERIFIED / DB_INTEGRITY_VERIFIED / CONSOLE_NOT_VERIFIED / VIEWPORT_NOT_EXPOSED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`.

## D-096 — Continuous Shift placement input + D-083 suppression parity

- Implementation commit `80fb52e11b51bd78997824b98035810854c82807` extends the existing current-Day serial placement queue so ordinary planned `Shift + ArrowUp / ArrowDown` accepts successive same-Section reorder and cross-Section Move intents from the effective pending projection. Move steps remain separate logical queue items; Move → Reorder is dependency-ordered and is not blindly coalesced. Lifecycle, retained-ambiguity, Routine, historical, future/past, and other non-commutative placement barriers remain unchanged. Entry-identity focus restoration covers optimistic cross-Section movement.
- Worker `ReorderEntries` now applies the Day projection's suppression-visible predicate consistently to membership, validation, CAS guards, mutation targets, assertions, and rereads. Suppressed physical Routine rows are excluded from visible payloads and retain their physical positions; temporary shifting uses the full physical Section range. A deterministic Move → Reorder fixture test covers a suppressed Routine Entry in the target Section.
- Local evidence is PASS: Worker `32 files / 287 tests`, Web `5 files / 303 tests`, typecheck, normal build, exact nonprod build, deploy guard, Wrangler dry-run, and diff-check. No API/Worker contract, schema, migration, dependency, or security change was introduced; migration is `NOT_REQUIRED`.
- Persistent nonprod Worker `3b36f2db-14ad-4f49-83ab-df6ba78095cd` is canonical `taskchute-web-nonprod` with `RUNTIME_ENV=nonprod`, `BOOTSTRAP_ENABLED=false`, APP/AUTH nonprod bindings. Root `200`, unauthenticated Documents API `401`, pending `0 / 0`, quick_check `ok`, FK empty, active executions `0`, duplicate positions/origin-Day keys `0`, transient guard/assertion residue `0`, and successful read-only probes `rows_written=0`.
- Existing authenticated persistent in-app browser accepted two real `Shift + ArrowDown` gestures without waiting for the save indicator to clear; `保存中 1件` was observed during the fast response path, so exact multi-request race timing was not browser-observable. Same-tab reload and a fresh tab using the existing session restored the resulting canonical Day projection. The connector viewport was approximately `332 × 914` with Sidebar open; zoom and console APIs were unavailable, so console evidence is `NOT_VERIFIED`. Deferred-response automated tests remain authoritative for queue timing and Move/Reorder ordering.

D-096 classification: `APPROVED / CORRECTED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / REORDER_PROJECTION_SUPPRESSION_PARITY_VERIFIED / SUPPRESSED_ENTRY_PRESERVATION_VERIFIED / CONTINUOUS_SHIFT_INPUT_VERIFIED / MOVE_TO_MOVE_QUEUE_VERIFIED / MOVE_TO_REORDER_QUEUE_VERIFIED / REORDER_TO_MOVE_QUEUE_VERIFIED / EMPTY_SECTION_TRAVERSAL_VERIFIED / ROUTINE_SCOPE_BARRIER_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_RAPID_KEY_VERIFIED / DB_INTEGRITY_VERIFIED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO` (exact deferred race timing and console remain `NOT_VERIFIED`).

## D-097 — Today initial Arrow focus and distinct Task focus indicator

- Canonical D-097 Decision was already added and approved on compatible main commit `d7a702e165dc42733f94fdea2a6279c819f7f85f`; this implementation uses `docs/decisions/D-097_TODAY_INITIAL_ARROW_FOCUS_AND_FOCUS_INDICATOR.md` and does not create a duplicate Decision file. Implementation commit `64943a64ef4a691ce8d092e4922892aaf77b898b` adds a Today-scoped capture-phase Arrow bootstrap only for plain `ArrowUp` / `ArrowDown` when no Task owns focus, targeting the first visible canonical Task row. Interactive/editing/modal/menu/popover/calendar/IME and modifier-key guards remain in place; normal row navigation, J/K, Shift placement, and Worker/API behavior are unchanged.
- Task focus styling now uses a 2px `#2383e2` outline via `:focus-visible`; lifecycle backgrounds remain independent. Focused rows retain their completed/running/selected state classes, with no focus-owned background or left stripe and no layout reservation. Local evidence is PASS: App `251` tests, full Web `5 files / 309 tests`, full Worker/D1 `32 files / 287 tests`, typecheck, normal build, exact nonprod build, deploy guard, Wrangler dry-run, and `git diff --check`.
- Exact pushed main was deployed to canonical persistent nonprod Worker `taskchute-web-nonprod`, version `73097671-06de-4f7c-9a2c-5c0889f49d80`, with `RUNTIME_ENV=nonprod`, `BOOTSTRAP_ENABLED=false`, APP `taskchute-app-nonprod`, and AUTH `taskchute-auth-nonprod`. Root returned `200`; unauthenticated Documents API returned `401`; APP/AUTH migration lists reported no pending migrations (`0 / 0`). APP/AUTH quick checks are `ok`, FK checks are empty, active executions `0`, Section/Day-scoped duplicate positions `0`, duplicate Routine origin-Day groups `0`, orphan Entries `0`, transient guard residue `0`, and successful read-only probes report `rows_written=0`.
- Existing authenticated persistent in-app browser session was reused. At fresh tab `10` the usable rendered viewport was `1280×720`, Sidebar open, zoom metadata unavailable. Real `ArrowDown` in fresh tab `9` and real `ArrowUp` in fresh tab `10` both focused the same first visible canonical Task (`歯医者に行く`) in execution-first order; the completed background remained visible together with the blue row outline. No synthetic DOM event was used. A running+focused browser case and browser console count remain `NOT_VERIFIED` because no safe running fixture was available and the connector exposes no console API; automated running/completed focus coverage is PASS.
- During browser verification, a stale Accessibility index caused one existing non-sensitive `test1` Task to be accidentally started and immediately completed; no active execution remains. This was not an intended D-097 mutation and is recorded here for audit transparency. The dedicated D083 fixture was subsequently used for a temporary Runner display and completed normally; no fixture cleanup or direct database mutation was performed. Production, restore/recovery, credential/re-login, bootstrap change, and destructive cleanup remain not run; no Worker/API/schema/migration/dependency change was introduced and migration status is `NOT_REQUIRED`.

D-097 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / TODAY_ARROW_BOOTSTRAP_VERIFIED_LOCAL / TODAY_FOCUS_INDICATOR_VERIFIED_LOCAL / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_ARROW_REPRESENTATIVE_VERIFIED / DB_INTEGRITY_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / RUNNING_FOCUS_BROWSER_NOT_VERIFIED / CONSOLE_NOT_VERIFIED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`.

### D-097 corrective — Focus frame above sticky Day cells

- Independent visual review found that the original parent `:focus-visible` outline could be occluded by sticky Bulk / Execution / Task descendants. Corrective commit `c2e88cf81d6c92e20a9355e6d0e8c2f77cf5748e` makes `.task-row` positioned and replaces the outline with a row-local absolute `::after` overlay (`z-index: 7`, `inset: 0`, `2px solid #2383e2`, `pointer-events: none`). Lifecycle backgrounds, selected/completed styling, and D&D drop shadows remain unchanged. No Worker/API/schema/migration/dependency change was introduced.
- Local evidence is PASS: focused CSS/DOM source regression, full Web `5 files / 310 tests`, full Worker/D1 `32 files / 287 tests`, typecheck, normal build, exact nonprod build, deploy guard, Wrangler dry-run, and `git diff --check`.
- Exact pushed main was deployed to canonical `taskchute-web-nonprod` as Worker version `10624c3e-7534-49f1-ba82-d5813f299176`; generated config confirms `RUNTIME_ENV=nonprod`, `BOOTSTRAP_ENABLED=false`, APP `taskchute-app-nonprod`, and AUTH `taskchute-auth-nonprod`. Root returned `200`, unauthenticated Projects API returned `401`, and both migration lists reported no pending migrations (`0 / 0`). Post-deploy read-only APP/AUTH checks reported `quick_check=ok`, empty FK violations, APP active executions `0`, duplicate positions `0`, duplicate Routine origin-Day groups `0`, orphan Entries `0`, routine guards `0`, and successful probes `rows_written=0`.
- The existing authenticated persistent tab `1` was reused after reload at approximately `1569 × 914`, Sidebar open, normal zoom with exact zoom metadata unavailable. Real keyboard focus on a planned row and a completed row showed a continuous blue frame across the visible Bulk / Execution / Task and nonsticky cells; after horizontal scrolling the frame remained continuous across the visible row. Existing D-097 Shift-placement evidence remains unchanged. Running-focus browser observation and console counts remain `NOT_VERIFIED`; no synthetic DOM events were used in this corrective.
- Production, restore/recovery execution, credential/re-login, bootstrap mutation, unrelated destructive cleanup, branch / PR / merge / tag / Release remain not run. Migration status is `NOT_REQUIRED`.

## D-098 — Today Add focus continuity and hierarchical Escape

- D-098はToday Web-only correctiveとして、pending Add Task行のfocus intentをgeneration付きsemantic locator（row / cell / control index）で保持し、canonical Add reconcile後に同じcellへfocusを復元する。ユーザーがAdd中に別rowまたはrow外へ移動した場合はその新しいfocusを奪わず、移動していない場合だけ既存のdefault row focusへ戻す。Add成功・ambiguous reconcileを同じ境界で扱い、Worker/API/schema/migration/dependencyは変更していない。
- Today rowのEscapeは、editor → cell trigger → rowの一段だけを閉じる階層とし、overlay / menu / calendar / IMEのowner precedenceと既存のinline editor commit/cancel semanticsを維持した。document capture-phaseのpointer intent記録により、toolbar等のDOM focusを伴わないpointer操作後にAdd reconcileが古いrowへfocusを戻すことも防止した。
- Implementation commit `b5460c22e8362b34c30b7b950e460aa99d44005a`、pointer-focus authority corrective `6a9a0f36c190b61e6f7e0f77726a1cf8fb3debf4`をmainへ順次fast-forward pushした。focused D-098 Web `7 / 7 PASS`、full Web `5 files / 317 tests PASS`、full Worker/D1 `32 files / 287 tests PASS`、typecheck、normal/exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`はPASS。No migration / dependency / Worker/API change。
- Exact pushed mainからcanonical persistent nonprod Worker `taskchute-web-nonprod`へdeployし、Worker versionは`1eae8a40-5bf7-4e45-8cb7-3262b61d6d93`。generated configは`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`。root `200`、未認証Documents API `401`、APP/AUTH migration pending `0 / 0`、両DB quick_check `ok`、FK empty、read-only probes `rows_written=0`を確認した。APPのactive Executionは既存状態として`1`件だったため、開始・完了・cleanupは行っていない。
- 既存authenticated persistent tab（約`1569 × 914`、Sidebar open、zoom metadata unavailable）を再利用し、実ブラウザ操作で既存Taskのtitle editorを開き、Escapeでeditorを閉じることは確認した。CUA `dev.logs()`は`[]`。ただしconnectorのAX index/input stateが途中で不安定になり、専用disposable Add fixtureを安全に確立できなかったため、Add→Tab pending/canonical focus continuityとcell→rowのsecond Escapeはbrowser `NOT_VERIFIED`。途中で表示されたstale placement warning / active Executionを含む既存tab stateは追加操作・直接cleanupせず残置し、該当raceのauthorityはdeferred automated testsとする。
- production、restore/recovery、credential取得・再設定、bootstrap変更、APP/AUTH write、destructive cleanup、branch / PR / merge / tag / releaseは行っていない。D-098は automated/local/nonprod safety evidenceまでを確認済みで、browser representative classificationは未完了のまま保持する。

D-098 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / TODAY_ADD_FOCUS_CONTINUITY_VERIFIED_LOCAL / TODAY_HIERARCHICAL_ESCAPE_VERIFIED_LOCAL / PERSISTENT_NONPROD_DEPLOYED / PERSISTENT_NONPROD_SAFETY_VERIFIED / AUTHENTICATED_BROWSER_PARTIAL / ADD_FOCUS_BROWSER_NOT_VERIFIED / HIERARCHICAL_ESCAPE_BROWSER_NOT_VERIFIED / CONSOLE_EMPTY_ARRAY_OBSERVED / DB_INTEGRITY_VERIFIED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`.

## D-099 — Hit-a-Hint v0.1

- D-099のApproved Decisionとcanonical pointerを追加し、既存のunreconciled historical Hit-a-Hint記述をD-099のscopeへ更新した。現行mainのshortcut conflict inventoryではplain `F / f`の既存割当はなく、`? / N / E / D / I / X / S / J / K / Arrow / Shift+Arrow / Tab / Escape`の既存分岐は変更していない。
- Web-onlyの`HitAHint` central engineを追加した。neutral signed-in surfaceでのみplain `F`を受理し、capture-phaseの単一listenerがactive Hint mode中の通常unmodified keyboard inputをexclusiveに処理する。modifier chordはcancelしてbrowser/systemへ返し、Escape / Backspace / invalid character、pointer / scroll / resize / view change cancel、local modal/menu/popover/calendar/editor/IME owner precedenceを実装した。
- 対象はdocument orderのvisible enabled actionable elementsから取得し、hidden / disabled / aria-disabled / inert / collapsed / zero-size / offviewport / overlay / Logoutを除外する。Task rowとeditable controlはfocus-only、button / link / action roleは既存`click()` pathを使う。labelは`ASDFGHJKLQWERTYUIOPZXCVBNM`で26件まで1文字、超過時は全件2文字のprefix-free固定長とした。portal badgeはfixed viewport座標、`aria-hidden`、`pointer-events: none`でlayoutへ参加しない。
- D-098の`focusFromUserIntent` / `markUserFocusIntent`へ接続し、Hintによるfocus/actionを既存focus authorityとして扱う。shortcut helpへ`F`とmode操作を追加した。Worker / API / schema / migration / dependency / lifecycle / placement semanticsは変更していない。
- Implementation commit `061c25b5cc998bd9293dad38f806c7e320f43b46`を、upstream docs/agent-only FF後の`609c77e329ff7a5a7cdee3a13b19d8a1940c698b`からmainへfast-forward pushした。Hit-a-Hint focused `32 / 32 PASS`、App + Hit-a-Hint `291 / 291 PASS`、full Web `6 files / 350 tests PASS`、full Worker/D1 `32 files / 287 tests PASS`、typecheck、normal build、exact `CLOUDFLARE_ENV=nonprod` build、`verify:nonprod-deploy`、Wrangler nonprod dry-run、`git diff --check`をPASSした。build時の既知Wrangler log `EPERM`とclient chunk-size warningは非致命で、required commandはexit `0`。Migration / API / Worker / dependencyは`NOT_REQUIRED`。
- exact pushed mainからcanonical persistent nonprod Worker `taskchute-web-nonprod`へdeployし、Worker versionは`94c944e2-d6cd-4110-96e4-e5c3939de7ef`。generated configは`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`。root `200`、未認証`/api/v1/documents` `401`、APP/AUTH migration pending `0 / 0`を確認した。
- 既存authenticated persistent tabを再利用し、viewport `1569 × 912`、DPR `1`、Sidebar open、zoom metadata unavailableで実ブラウザ検証を行った。reload後Todayでplain `F`を押すと124 badgesが表示され、Hint label `AF`で安全な`ノート`画面へ移動した。Notesでも`F`で17 badges、`?`はshortcut helpを開かず、`Escape`で解除した。same-tab reload後もTodayで124 badgesを確認し、fresh authenticated tab（同一session、browser tab `12`）でも66 badges表示とEscape解除を確認した。既存Notesのauthenticated loadと`保存済み`も確認し、両tabのCUA console logsは空配列`[]`だった。App dataの新規作成・更新・削除は行っていない。
- APP read-only evidenceは`PRAGMA quick_check=ok`、FK empty、Document 3 rows、invalid kind `0`、negative revision `0`、duplicate document identity `0`、duplicate active owner/title `0`、`transaction_assertions` residue `0`、全probe `rows_written=0`。AUTHもquick_check `ok`、FK empty、全probe `rows_written=0`。No migration was applied by D-099.
- Production、restore/recovery、credential取得・再設定、bootstrap変更、destructive cleanup、branch / PR / merge / tag / Releaseは行っていない。既存の非機密fixture / active-stateは直接cleanupせず残置した。

D-099 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED_LOCAL / HIT_A_HINT_ACTIVATION_VERIFIED / PREFIX_FREE_HINTS_VERIFIED / HINT_KEYBOARD_EXCLUSIVITY_VERIFIED / SHORTCUT_COLLISION_REGRESSION_VERIFIED / TARGET_ACTION_SEMANTICS_VERIFIED / D098_FOCUS_AUTHORITY_REGRESSION_VERIFIED / EXISTING_SHORTCUTS_PRESERVED / FULL_WEB_VERIFIED / FULL_WORKER_VERIFIED / TYPECHECK_VERIFIED / BUILD_VERIFIED / NONPROD_BUILD_VERIFIED / DEPLOY_GUARD_VERIFIED / WRANGLER_DRY_RUN_VERIFIED / DIFF_CHECK_VERIFIED / MAIN_PUSHED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / API_SAFETY_VERIFIED / DB_INTEGRITY_VERIFIED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`.

## D-099 corrective — Hit-a-Hint Escape neutral focus recovery

- Source review confirmed that the capture-phase Hint Escape handler previously cleared only the transient Hint session. When a neutral `.day-shell[tabindex="-1"]` owned focus, the browser could therefore leave the full Day container outline visible after cancellation.
- Corrective commits `9e8e4cd5052c8ac213da10a65063f93771819e8d` and `5cf352b11b3ac1e9cf5594b89bfd23f0154315fd` add Escape-only neutral-focus cleanup in `HitAHint.tsx`. The helper blurs only a connected element with an explicit negative tabindex after excluding body/documentElement, editing/actionable/local-owner elements, and meaningful `tabIndex=0` surfaces. A next-frame check runs only if that same element remains active, so a user's subsequent focus is not touched.
- Focused Hit-a-Hint coverage is `34 / 34 PASS`; final standard Web verification is `6 files / 352 tests PASS`, with typecheck, normal build, exact nonprod build, deploy guard, Wrangler dry-run, and `git diff --check` PASS. No Worker/API/schema/migration/dependency change was introduced; Worker/D1 verification is `NOT_REQUIRED` by source-scope review.
- Final corrective deployment to canonical persistent nonprod `taskchute-web-nonprod` used Worker version `f4a1e106-5d43-4a0c-a3fc-2192ef543946`; generated config confirms `RUNTIME_ENV=nonprod`, `BOOTSTRAP_ENABLED=false`, APP `taskchute-app-nonprod`, and AUTH `taskchute-auth-nonprod`. Root `200`, protected Documents/Projects API `401`, remote APP/AUTH migrations `0 / 0`, both `PRAGMA quick_check=ok`, FK empty, and read-only probes `rows_written=0` were confirmed.
- Existing authenticated persistent browser was reused at `1379 × 912`, DPR `1`, Sidebar open, normal zoom with zoom metadata unavailable. A real neutral Day surface focus followed by `F` → `Escape` closed the badges, left `BODY` active, removed the black full-container outline, and allowed `ArrowDown` to focus the first Task row. A real completed Task row retained its same logical focus through `F` → `Escape`. CUA console logs were `[]`; no Task data mutation was performed.
- Production, restore/recovery, credentials/re-login, bootstrap mutation, destructive cleanup, branch / PR / merge / tag / Release remain not run. Existing untracked review artifacts remain untouched.

D-099 corrective classification: `APPROVED / CORRECTED / IMPLEMENTED / INTEGRATED / TESTED_LOCAL / HIT_A_HINT_ESCAPE_FOCUS_RECOVERY_VERIFIED / MEANINGFUL_FOCUS_PRESERVED / FULL_WEB_VERIFIED / TYPECHECK_VERIFIED / BUILD_VERIFIED / NONPROD_BUILD_VERIFIED / DEPLOY_GUARD_VERIFIED / WRANGLER_DRY_RUN_VERIFIED / DIFF_CHECK_VERIFIED / MAIN_PUSHED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / API_SAFETY_VERIFIED / DB_INTEGRITY_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`.

## D-100 — Executable Development Workflow / Codex Cycle Acceleration

D-100は既存verification policyを変更せず、`apps/web/scripts/dev-workflow.mjs`と`dev-workflow.test.mjs`を追加し、authoritative preflight、明示surface必須のprofile別automated core、per-step timing artifact、保守的なevidence summaryを実行可能にした。Initial implementation/docs commit `5748ba980c1eb5199f3f760e616b42a6de1d7e94`と、明示evidence state mappingのcorrective commit `669055751c71bb58d543f43fdb30168e4dca54b0`をcanonical `main`へfast-forward pushした。`verify:*`はfull affected Web / Worker-D1 suite、typecheck、normal build、`git diff --check`をsurfaceに応じて実行し、`--nonprod-static`ではexact nonprod build、既存deploy guard、Wrangler dry-runだけを追加する。実deploy、DB write、credential / auth変更、restore、production operationは行わず、manual / persistent evidenceを自動的にPASSへ昇格しない。

Focused tooling testsは8 / 8 PASS。D-100のSTANDARD / cross orchestrator実行ではfull Web、full Worker/D1、typecheck、normal build、exact nonprod build、deploy guard、Wrangler dry-run、diff-checkのtimingを記録する。explicit evidence state mappingのfocused regressionも8 / 8 PASS。runtime / API / schema / migration / dependencyの変更はなく、source reviewに基づきpersistent nonprod deploy、authenticated browser、API、DB feature probesは`NOT_REQUIRED`とする（static nonprod gatesは別途PASS）。同一8-step planの比較として、初回runは`171,110 ms`、修正後の最終runは`163,572 ms`（約`4.4%`短縮）であり、これは初回helperから最終helperへの測定比較で、過去の手作業cycle時間を推定したものではない。最終artifactを初期optimization baselineとして保持し、3–5 work item後に比較する。D-100 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / WORKFLOW_AUTOMATION_VERIFIED / TIMING_BASELINE_RECORDED / RUNTIME_CHANGE_NONE / MIGRATION_NOT_REQUIRED / PERSISTENT_NONPROD_NOT_REQUIRED / BROWSER_NOT_REQUIRED / API_RUNTIME_VERIFICATION_NOT_REQUIRED / DB_RUNTIME_VERIFICATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。
## D-101 — Task Primary Note and Document Permalinks

D-101 adds the first Task-owned use of the shared Markdown-native Document foundation. A Task may have one owner-scoped `task_primary` Document shared by its Entries; the Task title remains authoritative and the Document stores only Markdown source body. The Today table adds a visible `ノート` column, and opening it lazily ensures the relation before presenting the shared `/?view=note&document=<document-id>` permalink. The authenticated read resolver discriminates standalone and Task Primary Documents by exact owner-scoped ID; archived standalone Notes open exactly, while missing/cross-owner IDs remain unavailable without fallback. Side-peek and user-selected new-tab modes reuse the same canonical Task Note editor; GET bootstrap remains non-mutating. The resolver's Task Primary read projection now joins the owner-scoped Task authority and returns its current title, so generic and legacy permalink routes do not depend on the currently loaded Today projection.

Implementation commit `c077480fff07569a9fbb51b311ad853e459e09db` added APP `0031_task_primary_documents.sql`, typed Ensure/Update commands, owner-scoped relation enforcement, Day projection identity, the v4 Day-column default, Task Note editor, open-mode storage, and guarded navigation/bootstrap behavior. Test-stability commit `ebaf7a36f66f6bdd07f2087b74257cc57e8460aa` and corrective implementation commit `ef29268098324012aac393ed1b73ca108916fda6` were fast-forward pushed to `main`; no pre-existing untracked review artifacts were staged or changed.

Local evidence is PASS: bounded fresh/upgrade migration validation for `0031`, focused corrective Worker/D1 `23 / 23`, focused corrective Web `297 / 297`, full Web `9 files / 372 tests`, full Worker/D1 `33 files / 294 tests`, typecheck, normal build, exact nonprod build, deploy guard, Wrangler nonprod dry-run, and `git diff --check`. Known Wrangler log-file `EPERM` and Vite client chunk-size warnings were non-fatal; required commands exited `0`.

Exact pushed `main` at corrective commit `ef29268098324012aac393ed1b73ca108916fda6` was deployed to canonical persistent nonprod Worker `taskchute-web-nonprod` as version `a00c5342-a1a4-4734-b72e-040adab43ded`. Generated config and deploy guard confirm `RUNTIME_ENV=nonprod`, `BOOTSTRAP_ENABLED=false`, APP `taskchute-app-nonprod`, and AUTH `taskchute-auth-nonprod`. Root returned `200`, unauthenticated Documents API returned `401`, and APP/AUTH migration lists reported `0 / 0` pending.

The existing authenticated persistent tab was reused at the normal connector viewport; numeric viewport metadata was not exposed. The Task Primary fixture opened through `/?view=note&document=<document-id>` without `task=`, copied the shared URL, and restored its canonical body after same-tab reload and in a fresh tab in the existing authenticated session. A disposable standalone Note was created through the UI, saved, opened by exact shared permalink while active, archived and opened again by the same exact permalink in the archived/read-only view, verified as unavailable for a missing ID without list fallback, restored through the UI, and restored after same-tab reload and a fresh authenticated tab. Copy-link success UI was observed for both editors; main-tab console warning/error logs were empty (`[]`). The connector exposes read-only page evaluation, so direct v3 localStorage fixture injection was not performed; automated v3 migration coverage remains PASS and the persistent v3 fixture is `NOT_VERIFIED`. No credentials, bootstrap, production, restore, or unrelated destructive cleanup was used.

Post-deploy read-only APP/AUTH evidence is PASS: both `PRAGMA quick_check` results are `ok`, FK result sets are empty, all successful probes report `rows_written=0`, invalid Document constraints `0`, duplicate Document identity `0`, invalid Task Primary kind/shape `0`, duplicate/orphan Task Primary relations `0`, active executions `0`, duplicate Routine origin-Day groups `0`, duplicate Entry positions `0`, and transient assertion/placement/lifecycle guard residue `0`. The disposable standalone fixture count was `1` and no hard delete was used. D-101 adds no AUTH migration or dependency and no production behavior; APP 0031 is applied only to persistent nonprod.

D-101 corrective status: implementation, focused/full local gates, corrected nonprod deployment, authenticated route/persistence checks, and read-only DB integrity checks pass. The v3 Day-column migration compatibility, shared Document permalink, exact active/archived/missing/cross-owner resolution, Task-authoritative permalink title resolution, and standalone/Task copy-link behavior are recorded in TEST_MATRIX. No new migration was introduced; APP 0031 remains the only D-101 migration. Persistent browser v3 localStorage fixture injection remains `NOT_VERIFIED` because the available connector permits only read-only page evaluation.

D-101 corrective 2 implementation commit `dab0693d416051f5bb69ddee3276acf130241101` was fast-forward pushed to `main` and deployed to canonical persistent nonprod as Worker version `3306f046-486a-404c-bc1b-c3c3be4d9c69`. The read-only resolver now returns `task_title` from `tasks.title`, rejects a missing Task Primary relation/Task with `resource_not_found`, and the Web generic and legacy routes use that projection without a `Task Note` fallback. Focused Task Primary/Documents integration was `25 / 25` (including `8 / 8` Task Primary), focused permalink Web was `2 / 2`, full Worker/D1 was `33 files / 296 tests`, and full Web was `9 files / 372 tests`; typecheck, builds, guard, dry-run, migration regression, and diff-check passed. Existing authenticated and fresh tabs showed the canonical Task URL with the current Task title and console warnings/errors `[]`; remote root was `200`, unauthenticated Documents API `401`, APP/AUTH pending `0 / 0`, quick checks `ok`, FK empty, and all probes `rows_written=0`. No new migration, API mutation, dependency, credential, bootstrap, production, restore, or destructive cleanup was used.

D-101 corrective classification: `APPROVED / CORRECTED / INTEGRATED / TESTED / MAIN_PUSHED / D101_DAY_COLUMN_V3_MIGRATION_VERIFIED / D101_SHARED_DOCUMENT_PERMALINK_VERIFIED / D101_EXACT_ROUTE_RESOLUTION_VERIFIED / D101_TASK_TITLE_RESOLUTION_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / DB_INTEGRITY_VERIFIED / NO_NEW_MIGRATION / AUTHENTICATED_BROWSER_VERIFIED / V3_PERSISTENT_BROWSER_NOT_VERIFIED / PRODUCTION_NOT_RUN / RELEASED_NO`.

### D-101 UI refinement — unified Markdown editor and resizable Task Note peek

D-101のWeb-only UI refinementとして、TodayのTask Note triggerを再利用可能なinline SVG Note glyphへ置き換え、`data-note-state="absent|present"`を保持した。standalone NotesとTask Note peekは共通の`NoteMarkdownEditor`を使用し、Markdown source textarea、固定line-number rail、scroll同期、versioned line-number preferenceを同じDOM/CSS contractで共有する。Task Note peekは残り高さをflexで使い、左端のpointer-capture対応resize handleとseparator keyboard操作（Arrow / Shift / Home / End）で幅を変更できる。

幅のpreferred値は`taskchute.web.task-note-peek-width.v1`の`{version:1,width}`としてmemory-only UI preferenceに保存し、viewportに合わせて表示幅だけをclampする。幅変更はDocumentのbody/title/revisionやSave経路を変更しない。mobileではpeekをfull-width化しhandleを隠す。`HitAHint`はTask Note peek全体をlocal keyboard ownerとして扱うため、plain `F`等のToday shortcutへ漏れない。

Implementation commit `8b1a09cf4f0e728848286d4194025114fc9ab713` was fast-forward pushed to canonical `main` and deployed to persistent nonprod Worker `taskchute-web-nonprod`, version `cc826428-6e8c-4bae-b723-bcf65ee36ed0`. Generated config / deploy guard confirmed APP `taskchute-app-nonprod`, AUTH `taskchute-auth-nonprod`, `RUNTIME_ENV=nonprod`, and `BOOTSTRAP_ENABLED=false`; root was `200` and unauthenticated `GET /api/v1/documents` was `401`.

Focused impacted Web coverage is `333 / 333`; full Web is `10 files / 382 tests`. Standard Web verification is `4 / 4` (full Web, typecheck, normal build, diff-check); exact nonprod build, deploy guard, and Wrangler dry-run also pass. The build environment continues to emit the known Wrangler log-file `EPERM` and Vite client chunk-size warning, without a non-zero command result. No Worker/API/schema/migration/dependency change was made; Worker/D1 and migration verification are `NOT_REQUIRED` by source-scope review.

Persistent authenticated browser verification used the existing session at approximately `1380 × 910`, Sidebar open, normal zoom (DPR/zoom metadata unavailable). The Today Note column visibly showed blue present and gray absent SVG glyphs with the same icon geometry. A real pointer drag on the Task Note separator changed the peek from `524px` to `654px` without a save; the same tab reload and a fresh tab in the existing authenticated session restored `654px` and the canonical body. The standalone Notes screen visibly used the same line-numbered Markdown editor. Keyboard focus on the peek owner did not activate Hit-a-Hint for plain `F`; browser console warning/error logs were `[]`. Existing Task/D-101 persistence semantics remain unchanged.

D-101 UI refinement classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / SHARED_MARKDOWN_EDITOR_VERIFIED / TASK_NOTE_SVG_VERIFIED / TASK_NOTE_PEEK_RESIZE_VERIFIED / TASK_NOTE_WIDTH_PERSISTENCE_VERIFIED / HIT_A_HINT_LOCAL_OWNER_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / CONSOLE_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`.

### D-101 UI refinement 2 — floating Task Note window

D-101のWeb-only refinement 2では、TodayのTask Note peekを固定right-pane幅モデルからbrowser-localのfree-floating windowへ変更した。desktopではpreferred `x / y / width / height`を`taskchute.web.task-note-window-geometry.v1`へ`{version:1,x,y,width,height}`のflat envelopeで保存し、viewport clampは表示値だけに適用する。旧`taskchute.web.task-note-peek-width.v1`は新keyがない場合の読み取りfallbackとして保持し、最初の実装で生成されたnested envelopeも読み取り互換とした。Document ID、Task ID、本文、title、revision、save state、minimized stateはgeometry keyへ保存しない。mobileは従来のfull-sheet boundaryを維持し、Preview UI / state / dependencyは追加していない。

タイトル/status barの空白領域は実ポインタで移動でき、8方向（上下左右・4隅）のpointer resizeとseparator keyboard resizeを提供する。最小サイズは`360 × 320`、移動/resize後はpreferred geometryを保存し、minimizeはexpanded geometryを保持したままcompact barへ切り替える。minimized stateはreload/fresh tabで復元せず、expandedで開く。expanded NoteはHit-a-Hintのglobal keyboard owner、minimized Noteはbarへfocusがある時だけlocal ownerとなるため、背面Todayでのplain `F`は利用できる。Save/autosave/CAS、Task title authority、Note API/Worker、schema/migrationは変更していない。

Implementation commit `e958e146ded59dbc163ea75ea67e403545bb85f8`をcanonical `main`へpushした後、contractのflat envelopeに合わせる補正commit `bcc447afab7494f41122241e83ea46aa21efc2c0`をfast-forward pushした。Focused geometry/TaskNoteEditor/Hit-a-Hintは`64 / 64`、full Webは`11 files / 404 tests`、typecheck、normal build、exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`がPASS。初回full Webは既存の非D-101 focus待機テスト2件が一時的に失敗したが、同一suite再実行で`404 / 404` PASSとなり、仕様変更としては扱っていない。Known Wrangler log-file `EPERM`とVite client chunk-size warningは非致命的環境警告である。Web-only impactのためWorker/D1 test、migration、API/DB feature verificationは`NOT_REQUIRED`、dependency changeはない。

補正後のexact pushed mainはcanonical persistent nonprod Worker `taskchute-web-nonprod`へ再deployされ、Worker versionは`29836067-0b54-49fb-b0e2-bb7297a1a901`。guard/generated configは`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`を確認した。既存authenticated tab（screenshot viewport約`1380 × 914`、Sidebar open、通常zoom、zoom metadata unavailable）で、タイトルバー空白領域の実ポインタdrag、右端・上端の実pointer resize、minimize → compact bar、restore、同一tab reloadを確認した。window rectはdrag後およそ`x=575,y=90,width=577,height=806`で、reload後も同じgeometryを再現した。minimize/restoreは本文を編集せず、表示状態は`保存済み`のままだった。

同一authenticated sessionのfresh tab（screenshot viewport約`1280 × 720`）でもexact Note/bodyと8 resize handleを確認し、preferred geometryは小さいviewport内へclampされて`x=576,y=16,width=577,height=688`として表示された。補正deploy後に実pointerで1px移動し、reload後も同じrendered geometryを確認した。これはpreferredとrenderedを分離する証拠であり、minimized stateは保存されない。minimized bar外の背面Todayでは実`F`でHit-a-Hint badgesが表示され、barへfocusした状態では`F`がHintを起動しなかった。Preview/プレビュー要素は0件、CUA console warning/errorは`[]`だった。button click no-drag、全8方向、keyboard edge resize、autosave非発火はfocused testsでPASSし、pointer race timingはこの検証では主張しない。

D-100 timing sample（tool wall time）はfocused Web `2.12 s`、full Web rerun `26.35 s`、exact nonprod build約`0.35 s`、deploy guard約`0.04 s`、補正deploy約`20 s`。preflight・browser・docs編集は個別artifactで計測していないため、推定値として報告しない。production、restore/recovery、credential取得・再設定、bootstrap変更、destructive cleanup、branch / PR / merge / tag / Releaseは行っていない。D-101 UI refinement 2 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / FLOATING_TASK_NOTE_WINDOW_VERIFIED / TASK_NOTE_GEOMETRY_PERSISTENCE_VERIFIED / TASK_NOTE_RESIZE_VERIFIED / TASK_NOTE_MINIMIZE_VERIFIED / HIT_A_HINT_MINIMIZED_OWNER_VERIFIED / FULL_WEB_VERIFIED / TYPECHECK_VERIFIED / BUILD_VERIFIED / NONPROD_BUILD_VERIFIED / DEPLOY_GUARD_VERIFIED / WRANGLER_DRY_RUN_VERIFIED / DIFF_CHECK_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / CONSOLE_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。

### D-101 UI refinement 2 corrective — floating Note chrome

前段のfree-floating window behaviorを維持したまま、通常のSave button・通常保存status・重複footer closeをexpanded surfaceから除去した。通常保存経路、Ctrl/Cmd+S、autosave、flush/barrier、ambiguity exact-retryは変更していない。minimized barはNote icon（`ノートを開く`）とclose `×`だけを表示し、expanded subtreeは`hidden` / computed `display:none` / `pointer-events:none`で完全に非表示とした。実dragは4px thresholdでcompact状態を維持し、desktop expandedのcapture-phase outside pointerdownは保存やfocus stealなしにminimizeし、背面のclick propagationを維持する。

Corrective implementation `9bbbbd680cb4f178590aa6ad3a34839ad4b9bdcb`をcanonical `main`へfast-forward pushし、persistent nonprodへWorker `9b4fc2f2-8d40-48eb-9cfa-a4aa3bb150a1`としてdeployした。Focused TaskNoteEditor/Hit-a-Hintは`52 / 52`、full Webは`11 files / 407 tests`、typecheck、normal/exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`がPASS。Web-only correctiveのためWorker/D1、migration、API/DB feature verification、dependency changeは`NOT_REQUIRED`。

同一authenticated tab（viewport `1379 × 912`、Sidebar open、通常zoom、zoom metadata unavailable）でexpanded chromeに通常Save/statusがなく、header closeが1件、footer closeが0件であることを確認した。minimize後はbarに`ノートを開く` iconと`×`だけが表示され、expanded subtreeはhiddenだった。実ポインタdragでbarを移動してもrestoreせず、icon clickでrestoreした。expanded状態からSidebarのTodayを実clickするとNoteはminimizeし、背面Today遷移が継続した。直後の実`F`でHit-a-Hint badgesが表示され、Note updateは開始されず、console warning/errorは`[]`だった。fresh authenticated tab（viewport `1280 × 720`）でも同じcanonical Note/body、header close 1件、footer close 0件、通常Save button 0件、通常status 0件、console `[]`を確認した。minimized stateをtab間で永続化しない既存仕様によりfresh tabがexpandedで開くことは期待どおりである。

Corrective classification: `APPROVED / CORRECTED / INTEGRATED / TESTED / MAIN_PUSHED / FLOATING_TASK_NOTE_CHROME_VERIFIED / MINIMIZED_BAR_COMPACT_VERIFIED / OUTSIDE_CLICK_MINIMIZE_VERIFIED / HIT_A_HINT_AFTER_OUTSIDE_CLICK_VERIFIED / FULL_WEB_VERIFIED / TYPECHECK_VERIFIED / BUILD_VERIFIED / NONPROD_BUILD_VERIFIED / DEPLOY_GUARD_VERIFIED / WRANGLER_DRY_RUN_VERIFIED / DIFF_CHECK_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / CONSOLE_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。

### D-101 UI refinement 2 v2 — compact window controls and minimized title

D-101 UI refinement 2 v2はWeb-onlyのchrome refinementとして、floating Task Noteのheader utility action後に`-` / maximize-or-restore icon / `×`をこの順で配置した。Closeはheaderのicon-only control（`ノートを閉じる`）に一本化し、minimized barはNote icon・現在のTask title・`×`を含むcompact layoutとした。titleはflex/ellipsisで長文でもaction領域を圧迫せず、Document title/body、save/autosave/CAS、ambiguity barrier、API、Worker、schema、migration、dependency、Previewは変更していない。

Maximizeはcomponent-local transient stateで、desktop safe boundsをviewportの16px inset（`viewport - 32`）から算出する。最大化中はdrag / resize / keyboard move・resizeを無効化し、resize handlesを表示しない。preferred normal `x / y / width / height`と既存localStorage envelopeは変更せず、restore時に通常geometryへ戻る。Browser確認ではcurrent pushed assetの既存authenticated sessionを使用し、viewport `1280 × 720`、Sidebar open、normal zoom metadata unavailableでcontrols `-` / square / `×`とicon-only closeを確認した。temporary viewport `1100 × 700`ではmaximize geometry `16,16,1068,668`、restore後のnormal geometry `534,37,529,626`を確認し、minimized barでは現在のTask titleとclose `×`、expanded subtree hiddenを確認した。通常状態では実ポインタのtitle-bar dragで位置が変わり、実ポインタのright-handle resizeで幅が`529`→`479`→`529`へ変化した。maximize中のhandlesは0、restore後は8であり、検証用geometryは元に戻した。既存D-101のsame-tab/fresh-tab persistence evidenceは有効であり、v2はgeometry storage formatを変更していない。

Implementation commit `cfa0cd81b1f1a36bdd1a26f2220782c57e0969b4`をcanonical `main`へfast-forward pushし、exact mainをcanonical persistent nonprod Worker `taskchute-web-nonprod`へdeployした。Worker versionは`4c519d01-f100-4931-a292-bdfcab9ede3e`、generated config / guardは`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`を確認した。Focused TaskNote/geometryは`34 / 34`、full Webは`11 files / 412 tests`、typecheck、normal build、exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`はPASS。Known Wrangler log-file `EPERM`とVite client chunk-size warningは非致命的環境警告で、Worker/D1、API、migration、dependency verificationはWeb-only scopeのため`NOT_REQUIRED`。Browser console warning/error logsは`[]`。production、restore/recovery、credential/re-login、bootstrap mutation、destructive cleanup、branch / PR / merge / tag / Releaseは行っていない。

D-101 UI refinement 2 v2 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / TASK_NOTE_WINDOW_CONTROLS_VERIFIED / TASK_NOTE_MAXIMIZE_TRANSIENT_VERIFIED / TASK_NOTE_PREFERRED_GEOMETRY_PRESERVED / TASK_NOTE_MINIMIZED_TITLE_VERIFIED / FULL_WEB_VERIFIED / TYPECHECK_VERIFIED / BUILD_VERIFIED / NONPROD_BUILD_VERIFIED / DEPLOY_GUARD_VERIFIED / WRANGLER_DRY_RUN_VERIFIED / DIFF_CHECK_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / CONSOLE_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。

### D-101 UI refinement 2 v2 corrective — single minimized-bar restore affordance

Source review identified that the v2 minimized bar still rendered a duplicate `ノートを開く` icon button. Corrective implementation `0571f9159bfa80b7dccc2c922fc4cec9d66e0026` removes that duplicate control without changing the approved v2 geometry or persistence model. The minimized bar itself is now the single restore affordance and contains exactly one Note icon, the current Task title, and `×`. Explicit minimize moves focus to the bar; Enter/Space and pointer activation on the bar body/title restore it. The close button is excluded from restore handling, while the existing drag threshold, outside-click pass-through, and Hit-a-Hint ownership remain intact.

Focused `TaskNoteEditor` coverage is `20 / 20`; full Web is `11 files / 414 tests`. Typecheck, normal build, exact nonprod build, deploy guard, Wrangler dry-run, and `git diff --check` pass. Exact pushed `main` was deployed to canonical persistent nonprod as Worker `66d2bdf1-38a3-4214-b85c-79c6a217b68d` with target `taskchute-web-nonprod`, APP `taskchute-app-nonprod`, AUTH `taskchute-auth-nonprod`, `RUNTIME_ENV=nonprod`, and `BOOTSTRAP_ENABLED=false`.

Authenticated current-asset browser verification used the existing session at the available viewport (zoom metadata unavailable). The deployed asset was confirmed as `index-CshX5mfh.js`; minimized-bar DOM showed one direct Note icon, zero `button[aria-label="ノートを開く"]`, one close button, and `hidden` expanded content. Real browser clicks on bar body/title and real Enter/Space key presses restored the editor; minimizing moved focus to the bar; clicking `×` closed without restoring. No Note content was edited, and console warning/error logs were `[]`.

No API/Worker/schema/migration/dependency change was required; migration is `NOT_REQUIRED`. Production, restore/recovery, credential/re-login, bootstrap mutation, destructive cleanup, branch / PR / merge / tag / Release were not run. Corrective classification: `APPROVED / CORRECTED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / TASK_NOTE_MINIMIZED_BAR_SINGLE_ICON_VERIFIED / TASK_NOTE_MINIMIZED_BAR_RESTORE_VERIFIED / TASK_NOTE_MINIMIZED_BAR_KEYBOARD_RESTORE_VERIFIED / TASK_NOTE_MINIMIZED_BAR_FOCUS_VERIFIED / TASK_NOTE_MINIMIZED_CLOSE_SEPARATION_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / CONSOLE_VERIFIED / FULL_WEB_VERIFIED / TYPECHECK_VERIFIED / BUILD_VERIFIED / NONPROD_BUILD_VERIFIED / DEPLOY_GUARD_VERIFIED / WRANGLER_DRY_RUN_VERIFIED / DIFF_CHECK_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RELEASED_NO`。

### D-102 Multi-Task Note Windows v0.1

D-102は既存Task Primary Document / Note editorのWeb presentationを、同一authenticated browser tab内でTask stable identityごとの複数floating windowへ拡張した。Appがopen window registry、active route、z-order、outside-click request、per-window dirty/unresolved/flush callbackを所有し、TaskNoteEditorは各windowのgeometry・focus・restore・minimizeと既存のsave/CAS/ambiguity/barrier semanticsを所有する。window identity・z-order・minimized/maximized stateはserver/localStorageへ永続化せず、shared Document/API/Worker/schema/migration/dependency semanticsも変更していない。preferred geometryを基準に24px deterministic cascadeを適用し、同一Taskの再表示は既存windowを再利用する。desktopではexpanded window内の兄弟クリックは他windowを最小化せず、window外クリックはfrontmost expanded windowだけを最小化する。canonical routeはactive windowだけを指し、reload/fresh tabはそのrouteのNoteを最大1件復元し、mobileは従来のsingle full-sheet presentationを維持する。

Implementation commit `3ba503a95b7703f41b652ac51335f0431c02b1b9`をcanonical `main`へfast-forward pushし、exact mainをcanonical persistent nonprod Worker `taskchute-web-nonprod`へdeployした。Worker versionは`5463d621-57c8-4a41-b3a2-ab23d0d6fdec`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`、`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`を確認した。Focused Webは`3 files / 301 tests`、full Webは`11 files / 416 tests`で、typecheck、normal build、exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`がPASS。Web-only scopeのためWorker/API/DB feature verification、migration、dependency changeは`NOT_REQUIRED`。

既存authenticated tab（viewport `1379 × 912`、DPR 1、Sidebar open、通常zoom、zoom metadata unavailable）で、Task `てst`（Document `01a098c8-0b01-7875-8793-97f2cdf98a6e`）とTask `test`（Document `01a0999d-af26-72f9-87c0-25c3da50e55c`）を実際に開き、2つのwindow root、独立したz-index、24px cascade geometry、同一Task再表示時のwindow数不変を確認した。兄弟windowの可視headerを実pointerで選択して双方がexpandedのまま前面化し、外側を実pointer clickしたときはfrontmostの`test`だけがminimizeされ、背面の`てst`はexpandedのままだった。reload後はcanonical routeの`test`だけが復元され、fresh authenticated tabでも同じcanonical Noteがsingle windowとして復元された。既存/fresh tabともconsole warning/errorは`[]`で、本文・titleは編集していない。Pointer timingによるsave raceはこの確認では主張せず、window interactionのfocused coverageをauthorityとする。

Production、restore/recovery、credential取得・再設定、bootstrap変更、destructive cleanup、branch / PR / merge / tag / Releaseは行っていない。D-102 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / FLOATING_TASK_NOTE_WINDOWS_VERIFIED / TASK_NOTE_WINDOW_IDENTITY_VERIFIED / TASK_NOTE_CASCADE_GEOMETRY_VERIFIED / TASK_NOTE_STACKING_VERIFIED / TASK_NOTE_OUTSIDE_CLICK_OWNERSHIP_VERIFIED / TASK_NOTE_RELOAD_SINGLE_ROUTE_VERIFIED / TASK_NOTE_FRESH_TAB_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / CONSOLE_VERIFIED / FULL_WEB_VERIFIED / TYPECHECK_VERIFIED / BUILD_VERIFIED / NONPROD_BUILD_VERIFIED / DEPLOY_GUARD_VERIFIED / WRANGLER_DRY_RUN_VERIFIED / DIFF_CHECK_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。

### D-102 corrective — cascade spacing and first-click controls

D-102 corrective implementation commit `5deba847305bea4cb06645be0017b33bb6fec471`をcanonical `main`へfast-forward pushし、exact mainをcanonical persistent nonprod Worker `taskchute-web-nonprod`へdeployした。Worker versionは`b74edecb-54df-4c5a-a436-a944807d16d4`、APP `taskchute-app-nonprod`、AUTH `taskchute-auth-nonprod`、`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`である。Web-only correctiveのため、Document/API/Worker/schema/migration/dependency semanticsは変更していない。

原因は、windowのactivation時に`taskNoteWindows`を`stackOrder`順でReact childrenとして並べ替え、pointerdown後のDOM順変更が背面windowの後続click targetを不安定にしていたことだった。修正ではregistry配列のrender orderを固定し、stack orderはDocument IDごとのz-index mapだけから導出する。新規windowのcascade offsetは24pxから48pxへ変更し、clamp後に既存geometryへ衝突した場合は、固定された候補方向・距離順で別geometryを選ぶpure helperの決定的fallbackを追加した。既存windowのgeometry preference、Document identity、save/CAS/ambiguity semanticsは変更していない。

Focused geometry/App Webは`2 files / 283 tests`、full Webは`11 files / 418 tests`で、typecheck、normal build、exact `CLOUDFLARE_ENV=nonprod` build、deploy guard、Wrangler dry-run、`git diff --check`がPASS。Worker/API/DB feature verification、migration、dependency changeはscope reviewにより`NOT_REQUIRED`である。geometry focused testではviewport `1200 × 900`、seed `700,100,420,500`に対しindex 0 / 2がそれぞれ`700,100` / `604,196`となり、index 1がoccupied geometryと衝突した場合は`652,52`へ決定的に退避することを確認した。

既存authenticated browser（viewport `1379 × 912`、Sidebar open、DPR 1、通常zoom）で、Task `てst` と `test`の2 windowを実際に開き、rendered geometry `483,40,577,668` と`435,88,577,668`、z-index `21 / 20`、差分48pxを確認した。前面windowを実pointer dragで右へ移動して背面操作領域を露出し、背面のリンクコピー、最小化、最小化bar復元、最大化、元サイズへの復元、最小化barのクローズを各1回の実クリックで実行した。背面windowの操作後も意図した1回だけが実行され、siblingのmounted DOM identityは維持された。same-tab reloadと既存authenticated sessionのfresh tabではcanonical routed Noteが復元され、既存tab/fresh tabのconsole warning/errorはともに`[]`だった。Task Note本文/titleは編集していない。

Pointer timingによるsave race、mobile/別zoom/device permutationsはこのcorrective browserでは主張せず、既存D-102 deferred/normal automated coverageをauthorityとする。Production、restore/recovery、credential取得・再設定、bootstrap変更、destructive cleanup、branch / PR / merge / tag / Releaseは行っていない。D-102 corrective classification: `APPROVED / CORRECTED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / TASK_NOTE_CASCADE_48PX_VERIFIED / TASK_NOTE_OCCUPIED_FALLBACK_VERIFIED / TASK_NOTE_STABLE_DOM_ORDER_VERIFIED / TASK_NOTE_INACTIVE_CONTROL_FIRST_CLICK_VERIFIED / TASK_NOTE_RELOAD_SINGLE_ROUTE_VERIFIED / TASK_NOTE_FRESH_TAB_VERIFIED / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / CONSOLE_VERIFIED / FULL_WEB_VERIFIED / TYPECHECK_VERIFIED / BUILD_VERIFIED / NONPROD_BUILD_VERIFIED / DEPLOY_GUARD_VERIFIED / WRANGLER_DRY_RUN_VERIFIED / DIFF_CHECK_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。
### D-103 Project Primary Document v0.1

D-103はshared Markdown-native `Document` foundationをProjectへ接続し、owner-scopedな`project_primary` relationをlazy Ensureする。Project titleはProject rowをauthorityとし、Project Primary Documentの`title`は保持しない。bodyだけを既存のfloating `TaskNoteEditor`経路で編集し、UpdateはDocument revision CASと既存operation replay/misuse境界を再利用する。Project BoardのNote affordance、Notesの`プロジェクトノート` filter、DayのProject cellから同じ`/?view=note&document=<document-id>`へ到達できる。archiveはrelation/documentを保持し、Project hard deleteは既存Project deletion barrierの内側でrelationとDocumentだけをatomicに削除し、Task/Entry/Execution/historyは保持する。

APP `0032_project_primary_documents.sql`はDocument kindへ`project_primary`を追加し、owner/project/documentのFK・一意性、Project delete assertion、Ensure/Update operation allow-listを追加した。既存Document、Task Primary relation、Task/Entry/Execution/Routine/calendar data、operation historyは保持され、AUTH migration・新規dependency・APIの別command semanticsはない。bounded fresh/upgrade migration validation、focused Project Primary/Project management `8 / 8`、full Worker/D1 `34 files / 301 tests`、full Web `11 files / 418 tests`、typecheck、normal/exact nonprod build、deploy guard、Wrangler dry-run、`git diff --check`はPASSした。

Implementation commit `becb5fdf44c8e3b5c42024d536b3fa2eec22e3ed`をcanonical `main`へfast-forward pushし、APP 0032をbackup gate後にpersistent nonprodへ適用した。fresh APP backupは`797334` bytes / SHA-256 `3083B111469F80F9AEA4BC0898694515E2DD0188ECDDCCEA378C8DF3DCD0C178`、AUTH backupは`5569` bytes / SHA-256 `C755B688118A9093FB1019861539211A8BA773E7DBF9D3C1A8606733D027EE4A`で、両方のisolated `quick_check=ok`・FK 0を確認した。Worker `153e10dc-585a-427b-94e3-191325d29caa`は`taskchute-web-nonprod`へdeployされ、`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP/AUTH canonical bindings、APP/AUTH pending `0 / 0`を確認した。root `200`、未認証Documents/Projects API `401`、APP/AUTH remote quick check `ok`、FK empty、read-only probe `rows_written=0`、invalid kind/revision、Project relation orphan/duplicate、Entry duplicate position、assertion/guard residue、active executionはすべて0だった。

既存authenticated browserで専用の非機密Project fixtureをUI作成し、Project BoardのNote affordanceからlazy creation、Project title authority、body保存、NotesのProject filter、同一tab reload、同一authenticated sessionのfresh tab persistenceを確認した。既存tabとfresh tabのconsole warning/errorはともに`[]`。Project Primaryのbodyは`D-103 non-sensitive browser verification`であり、個人情報・secret・既存Note本文は記録していない。fixture Projectは非破壊の検証データとして残置した。production、restore/recovery、credential取得・再設定、bootstrap変更、追加のdestructive cleanup、branch / PR / merge / tag / Releaseは行っていない。

D-103 classification: `APPROVED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / PROJECT_PRIMARY_DOCUMENT_FOUNDATION_VERIFIED / PROJECT_PRIMARY_DOCUMENT_ENSURE_UPDATE_VERIFIED / PROJECT_DELETE_PRIMARY_DOCUMENT_ATOMICITY_VERIFIED / APP_MIGRATION_0032_APPLIED_NONPROD / PERSISTENT_NONPROD_DEPLOYED / AUTHENTICATED_BROWSER_VERIFIED / DB_INTEGRITY_VERIFIED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。

### D-103 corrective — Today Project loading / Note affordance / Notes inline Project editor — 2026-09-14

D-103 corrective implementation commit `74155bee97a1b05eff05bcbfc07e311d0b68acb8`をcanonical `main`へfast-forward pushし、Todayのestablished current Dayでactive Project一覧を明示的な`idle / loading / loaded / error` stateとして読み込むようにした。assigned Projectは一覧取得成功後にだけ`（アーカイブ）`と表示し、一覧取得中・失敗時に誤ってarchive扱いしない。Project selectorは利用可能幅を使い、Project Note affordanceは固定幅で隣接して表示する。

Notes pageのProject Note選択は同じNotes editor内のbody-only inline editorへ切り替え、Project titleはcurrent Projectをauthorityとして表示する。Today / Project Board / direct routeのfloating editorは維持し、同一Projectのfloating windowが既に存在する場合はNotes選択で既存windowをactivate / restoreしてinline editorを二重にmountしない。inline body saveは既存Project Primaryのautosave / CAS / ambiguity / flush barrierを再利用し、Worker/API/schema/migration/dependency semanticsは変更していない。

Focused App + Notes Webは`2 files / 292 tests PASS`、full Webは`11 files / 422 tests PASS`、typecheck、normal build、exact `CLOUDFLARE_ENV=nonprod` build、deploy guard、Wrangler dry-run、`git diff --check`をPASSした。Web-only scopeのためWorker/API feature regressionとmigrationは`NOT_REQUIRED`。Wranglerの既存ログ出力先権限制約（EPERM）とclient chunk-size warningはあるが、build/dry-runはexit `0`で完了した。

Exact pushed mainをcanonical `taskchute-web-nonprod`へdeployし、Worker version `412df8a0-ac18-414f-b290-9ca39da630b9`、`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、APP/AUTH canonical bindingsを確認した。root `200`、未認証Documents/Projects API `401`、APP/AUTH migration pending `0 / 0`、APP/AUTH `quick_check=ok`、FK empty、Project relation/document duplicate `0`、Entry duplicate position `0`、active execution `0`、assertion/guard residue `0`、成功したread-only probeの`rows_written=0`を確認した。

このcorrective専用のauthenticated browser A/B/Cは、CUA上でpersistent tab一覧が空で既存authenticated tabを取得できなかったため`AUTHENTICATED_BROWSER_NOT_VERIFIED`とする。D-103 baseの既存authenticated browser evidenceは変更せず、今回のToday Project load / Note affordance / Notes inline editorのbrowser PASSへ流用しない。production、restore/recovery、credential取得・再設定、bootstrap変更、destructive cleanup、branch / PR / merge / tag / Releaseは行っていない。D-103 corrective classification: `APPROVED / CORRECTED / IMPLEMENTED / INTEGRATED / TESTED / MAIN_PUSHED / D103_PROJECT_LOAD_VERIFIED_LOCAL / D103_TODAY_PROJECT_NOTE_AFFORDANCE_VERIFIED_LOCAL / D103_NOTES_PROJECT_INLINE_VERIFIED_LOCAL / PERSISTENT_NONPROD_DEPLOYED / DB_INTEGRITY_VERIFIED / AUTHENTICATED_BROWSER_NOT_VERIFIED / WORKER_NOT_REQUIRED / MIGRATION_NOT_REQUIRED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / RELEASED_NO`。

### D-121 Android Today Task Row Figma micro-corrective 2 — 2026-09-20

Implementation commit `b8ec08774233eb9924595b382c97dc4121016784` was fast-forward pushed to canonical `main`. This corrective is limited to the two approved Android Today Task Row visual deltas: Completed metadata geometry and disabled Selection checkbox rendering. Completed actual range no longer uses flexible weight; it uses the 85dp reference width, explicit 3dp gap before the 10dp timer icon, 3dp after the icon, and a duration field with a 33dp minimum that can grow for longer localized text. Selection keeps the 48dp touch slot and 18dp / 5dp checkbox geometry; planned unselected uses `#202020` with `#7F7F7A`, selected planned keeps `#52A3FF` and the 12dp check, and disabled Running / Completed applies whole-visual alpha `0.38` to the `#202020` / `#7F7F7A` visual. Font family, line-height, row height, selection behavior, lifecycle semantics, swipe, D&D, API, Worker, schema, migration, and dependency behavior were not changed.

Local `:app:assembleDebug`, `:app:compileDebugAndroidTestKotlin`, and `git diff --check` passed. The existing `scripts/android-qa.ps1 -Surface Today` flow and the focused Completed test were started on `TaskChute_API33`, but the emulator/app process entered the same excessive-CPU test hang with the pre-change baseline as well; local instrumentation and its post-test MainActivity / crash-buffer smoke are therefore `NOT_VERIFIED`, not a product PASS or FAIL. No screenshot comparison was run.
...
Task Rowの開始・終了見込みはWeb Start Forecast相当のcurrent-time / running remaining estimate / display-order projectionへ寄せ、見積・実績durationは総分数表示へ統一した。Header DatePickerとTask Actionsの日付移動は同じCompose DatePicker implementationを共有する。D-134によりcurrent DayのRunningTaskPanelは104dp progress playerとなり、elapsed / remaining / progress / overrunを表示する。

**Implementation: IMPLEMENTED / Local: Android JVM 142/142 PASS, full Worker 37 files・341 tests PASS, full Web 14 files・478 tests PASS, typecheck PASS, instrumentation compile PASS, debug assemble PASS / Authenticated Android runtime: NOT_RUN / Galaxy S23: NOT_RUN / Persistent nonprod destructive delete E2E: NOT_RUN / Production: NOT_RUN / Released: NO**。今回の実機Verified evidenceはまだない。既存D-128/D-129 semanticsとserver authorityを変更せず、Android / Web presentation、既存Worker guard、canonical docsの範囲で整合した。
