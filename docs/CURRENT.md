...
- Automated evidence: focused Routine Board / column tests 13 / 13 PASS; focused Routine Worker (routine-r2a, routine-r2b, mode-management) 3 files / 19 tests PASS; integration routine-mode.integration.test.ts 2 tests PASS; full Web 4 files / 264 tests PASS; full Worker/D1 26 files / 222 tests PASS; typecheck, normal build, exact CLOUDFLARE_ENV=nonprod build, Wrangler nonprod dry-run, and git diff --check PASS. Windows migration helper produced no output and was interrupted before completion, so it remains NOT_RUN; D-085 migration status is nevertheless MIGRATION_NOT_REQUIRED after remote apply.
- Exact nonprod Worker taskchute-web-nonprod version 4e4ae507-c366-42fc-8e17-3a1f6bbaacf8 was deployed with RUNTIME_ENV=nonprod, BOOTSTRAP_ENABLED=false, APP taskchute-app-nonprod and AUTH taskchute-auth-nonprod bindings. APP migration 0025 executed 21 commands; APP/AUTH remote migration lists report no pending migrations (0 / 0).

### D-085 persistent nonprod browser / DB evidence

- Existing authenticated persistent CUA tab was reused without login or credential action. D075 future Task I was the disposable verification Entry converted to a Routine. Routine Board default was set to D068 Deep verification; Day occurrence selection of D068 Light verification opened the approved chooser, Cancel caused no write, and 今回だけ changed the occurrence. A subsequent direct selection of — persisted an explicit occurrence Modeなし. Same-tab reload and a fresh tab using the same authenticated session showed Day —; fresh Routine Board showed default D068 Deep verification, proving the default/override distinction and persistence.
- Browser UI showed no visible error; exact console warning/error count is NOT_VERIFIED because the available CUA surface does not expose console logs. Browser viewport/zoom were not exposed by the connector and are not claimed as measured.
- APP read-only evidence: target Entry 01a07e61-6a0c-7565-bba1-28fa09869a3f, Day 01a079a5-3a15-7a92-bcdc-f0f0faae610d, Section 01a02936-efda-713b-8935-5bf23181a474, lifecycle planned, planned start 1200, position 2, occurrence override present 1, live Mode NULL, Routine default Mode 01a07731-b966-73eb-81a2-4db9208ca237; latest Convert / Update / SetRoutineMode operations were successful. Day placement revision 33 was coherent; duplicate positions, routine guards, and transaction assertions were empty/zero; active executions 0. APP and AUTH PRAGMA quick_check=ok, FK checks empty, migration pending 0 / 0, and all successful audit queries reported rows_written=0.

Classification: APPROVED / IMPLEMENTED / INTEGRATED / LOCAL_TESTED / MAIN_PUSHED / PERSISTENT_NONPROD_DEPLOYED / ROUTINE_DEFAULT_BROWSER_VERIFIED / OCCURRENCE_OVERRIDE_BROWSER_VERIFIED / OCCURRENCE_CANCEL_NO_WRITE_VERIFIED / EXPLICIT_MODE_NONE_BROWSER_VERIFIED / SAME_TAB_RELOAD_VERIFIED / FRESH_AUTHENTICATED_TAB_VERIFIED / DB_INTEGRITY_VERIFIED / CONSOLE_NOT_VERIFIED / MIGRATION_NOT_REQUIRED / MIGRATION_HELPER_NOT_RUN / DEPENDENCY_CHANGE_NONE / API_EXISTING_ROUTE_ADDED / PRODUCTION_NOT_RUN / RESTORE_NOT_RUN / DESTRUCTIVE_CLEANUP_NOT_RUN / RELEASED_NO.
