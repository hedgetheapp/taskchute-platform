...
current established Dayのordinary running TaskのProject / Mode編集、Routine iconのcolor-only表示、D&D insertion
feedback、Today Routine列からの明示的なRoutine modal、`TaskChuteDay`見出し除去、icon付きSidebarと3表示状態を
定める。empty Section / empty `Sectionなし`のdropは既存`MoveEntry`へplacementなしで委譲する。Sidebar可視ラベルは
`Taskchute` / `Note` / `Rotuine` / `Setting`とし、`＋ Taskを追加` / `表示`の配置は維持する。既存のcommand / CAS /
retry / reconciliation、D-101 / D-103 Note entry point、schema/migration、dependency、Android、production semantics
は維持する。初回Web-only implementation後のapproved correctiveで、running metadata capabilityを既存
`UpdateTaskMetadata` / `SetEntryMode`とcurrent projectionへ整合させた。
...
## D-055 — Bulk Selection estimate change with per-Routine scope
Status: Approved

Bulk Selection v0.1 / v0.2のselection surfaceを再利用し、eligibleなplanned Entryへ一つのcommon target estimateを適用する。target estimateはpositive integer secondsまたは明示的な`見積なし`を表す`NULL`とし、ordinary Entry、Routine-derived Entry、両者のmixed selectionを一つのlogical commandでatomicに処理する。estimate変更はSection、planned start、positionを変更せず、Day `placement_revision`も増加させない。

ordinary-only selectionは既存のordinary planning boundary内のestablished displayed Dayを対象とし、current Dayに限定しない。未establish DayをBulk estimateのためだけに作らない。Routine-derived Entryを一件でも含むselectionはD-044に従いcurrent logical TaskChuteDayだけを対象とし、Serverがcanonical timezone / Day boundaryからcurrent Dayを解決する。non-current DayのRoutine scope request、running、completed、historical / protected、preview、suppressed / unavailable、mutation-locked stateは全体をrejectする。

Routine scopeはRoutineごとに`今回だけ`または`ルーティンに反映`を明示選択し、preselectしない。`すべて今回だけ` / `すべてルーティンに反映`のfill-all helperを提供し、helper後もrowごとにoverrideできる。未選択Routineがある間はfinal confirmをdisabledとし、cancel / Escape / dismissはno-writeでselectionを維持する。success後のnon-destructive mutationもselectionを維持し、WebはN個のsingle estimate commandではなく一件のBulk commandだけを送信する。

`今回だけ`はD-044 / D-046のOccurrence estimate override semanticsを再利用する。selected current occurrenceへ`estimate_override_present = 1`とtarget valueを保存し、selected Entryのeffective estimateをtargetへ同期する。targetが`NULL`の場合もexplicit `見積なし` overrideとして保存し、no override / inheritと区別する。RoutineDefinition、default、defaults revision、他Occurrence、future stateは変更しない。同じeffective estimateでもoverrideがない場合はexplicit overrideを保存し、同じ既存overrideの場合だけtrue no-opを許容する。

`ルーティンに反映`はD-044のcurrent single-Routine definition estimate semanticsを再利用する。selected RoutineDefinitionごとにdefault estimateをtargetへ更新し、`defaults_revision`を一回だけ増加させ、selected current occurrenceのestimate overrideをclearしてnew defaultを継承させる。already-materializedなcurrent / future planned Entryのうちestimate overrideがないものへpropagateし、explicit estimate override、past、running、completed、interrupted、その他protected stateは保持する。target `NULL`はdefault `NULL`として伝播する。unmaterialized future TaskChuteDay / RoutineOccurrence / Entryは作成しない。same Definitionを複数selected Entryが解決しても更新とrevision増加は一回だけとし、scope不一致は全体をrejectする。

request authorityは`operation_id`、displayed `taskchute_day_id`、全selected `entry_ids`、common `estimate_seconds`、Routineごとのscope、definition scopeの`expected_defaults_revision`とする。ordinary-onlyでは`routine_scopes: []`を許可し、Routine scopeはselected Routine-derived Entryをexactly coverする。user ID、RoutineDefinition ID、effective estimateをclient authorityとして受け取らない。resultはselected value change、occurrence override change、non-selected propagated value change、Definition revisionを区別できる形とする。`changed_entry_ids`はeffective estimateが変化したselected Entry、`propagated_entry_ids`はeffective estimateが変化したnon-selected materialized Entryだけを含め、metadata-only変更は含めない。

commandはselected Entry、Occurrence override、Definition default / revision、eligible current / future propagation、operation resultを一つのatomic outcomeとして確定する。owner、Day、lifecycle、Entry snapshot、Occurrence relation / override、Definition default / defaults revision、propagation target snapshotをguardし、stale state、wrong owner / Day、invalid target、injected failure、concurrent changeでpartial effectを残さない。D-020に従い同一operationのsemantic replay、array order normalization、different requestのoperation misuse rejection、ambiguous outcome boundaryを維持する。estimate commandはplacement revision authorityを持たず、current / future Dayのplacement revisionは成功前後で不変とする。

APP compatibility migration `0014_bulk_estimate_scoped.sql`は、既存command guardを再利用する場合に`routine_command_guards.command_type`へ新commandを追加し、`operations.command_type`へ`BulkSetEntriesEstimateScoped`を追加するCHECK rebuildだけとする。implementationがguardを安全に再利用できる場合は両CHECKを拡張し、不要な場合はoperationsだけを拡張する。既存operation / guard row、既存command type、Entry / Routine / Day / Section / Execution schema、PK / FK / tableを保持し、新しいestimate persistence table / column / sentinelは追加しない。migrationはcompatibility-onlyとし、productionへ適用しない。

本DecisionはD-044、D-046、D-051、D-052、D-053、D-054の歴史的文言を変更しない。Bulk Section / Project / Mode / Note / date、cross-Day move、actual correction、undo / restore、Routine schedule / Skip変更、production operationは扱わない。実装・migration・real-local・persistent nonprod verificationの完了を本Decisionだけで意味させず、状態は`docs/CURRENT.md`、`docs/FEATURES.md`、`docs/TEST_MATRIX.md`へ記録する。

## D-056 — Day Operations v0.1 date move and single planned delete
Status: Approved

Day Operations v0.1では、同一のatomic `BulkMoveEntriesToDay` logical commandで、一件または複数のordinary / Routine-derived planned Entryを、表示中の一つのestablished source Dayからcurrentまたはfuture logical dateへ移動する。sourceはcurrent / established futureに加えて、already-established past Dayも許可する。targetはcurrent、established future、またはunestablished futureとし、target-past move、unestablished past Dayの作成・backfillはこのsliceに含めない。logical dateは任意のvalid `YYYY-MM-DD`を受け付け、month / year boundaryに人工的な制限を置かない。sourceと同じlogical date、currentより前のtarget、invalid dateはrejectする。

date moveはTask / Entry identityを変更せず、Task、Project、Mode、estimate、Task / Project metadata、Routine relation、RoutineOccurrence identity、origin Day、recurrence schedule、enabled / pause、Board orderを保持する。source Dayのhistorical contextは書き換えない。RoutineOccurrenceの`origin_taskchute_day_id`は保持し、Entryだけがtarget Dayへ移る。target Dayに同じRoutineDefinitionの別Occurrenceが既に存在しても、二つをmerge / dedupeせず各Occurrence / Entryを保持する。source current Dayのreload / Routine ensureは、origin Occurrenceが既にmaterialize済みでEntryだけ移動した事実を認識し、replacement Entryを生成しない。

移動されたRoutine occurrenceは、origin DayからEntryが移ったという既存のpersisted factをscheduled-date intentとして扱う。後のRoutine schedule / period変更でtarget Dayが新scheduleから外れても、moved occurrenceをその理由だけでsuppress / deleteしない。通常のnon-moved occurrenceには既存D-047 suppression semanticsを継続する。date moveはRoutineDefinition、default、schedule、defaults revisionを変更せず、new scheduled-date table / columnは導入しない。field-level Section-plan overrideが存在する場合はoverrideを保持したままtarget DayのSection mappingへ同期し、存在しない場合にdate moveだけを理由とした新規overrideは作らない。estimate overrideは変更しない。

Section / planned-startは、D-037の「移動先で常に`Sectionなし` / 開始予定未設定へclearする」実装behaviorを本sliceに限ってsupersedeする。sourceが`Sectionなし`ならtargetも`Sectionなし + NULL`とする。sourceにreal stable Sectionがあり、target Dayのfrozen Section contextに同じSectionが存在すれば、同じSectionを維持し、planned startを**target Day contextの`logical_start_minute`**へ同期する。source Dayのminuteをtargetへ盲目的にコピーしない。targetにSectionがない場合は、`Sectionなし + NULL`へfallbackできるが、requestの`allow_section_fallback`がtrueであることを要求する。acknowledgementなし、またはpreflight後のtarget context変化で新たにfallbackが必要になった場合は全体をrejectし、silent clearやpartial mutationを行わない。D-043のSection / planned-start invariantと、既存Section-plan overrideのpresent / NULL semanticsを維持する。

request authorityはauthenticated ownerから解決される`operation_id`、source `taskchute_day_id`、重複のない全selected `entry_ids`、clientが指定する`target_logical_date`、`expected_source_placement_revision`、`allow_section_fallback`だけとする。target Day ID、target Section ID、target planned start、positions、user IDはclient authorityにしない。Serverはcanonical timezone / Day boundaryからcurrent logical dateを解決し、source / target Day、frozen context、Section mapping、Entry eligibilityをowner-scopedに検証する。選択Entryは全件がsource Day所属、planned、unexecuted、canonical established Entry、mutation-availableでなければならず、一件でも不適格なら全体をrejectする。past established Dayではdate moveだけをplanned rowへ狭く提供し、既存のBulk Section / Estimate / Delete boundaryは広げない。

target orderingはtarget Dayのcanonical pre-mutation orderを基準とする。target non-moverのrelative orderを保持し、moverはresulting Section groupのtailへappendする。複数moverはsource Dayのpre-mutation canonical display orderを保ち、ordinary / RoutineやTask IDで再groupしない。fallback moverはtargetのunsectioned tailへappendする。source Dayの無関係なpositionをcompactせず、target / sourceのplacement変更、Entry target Day / Section / planned start、operation resultを一つのatomic outcomeで確定する。

source DayのEntryが一件以上移動した成功ではsource `placement_revision`をcommand全体でexactly `+1`する。existing targetへ一件以上入った場合はtarget revisionをexactly `+1`する。new future targetはmutation内でD-041のeffective timezone / boundary / Section contextをfreezeしrevision `0`でestablishし、成功したmove後にrevision `1`となる。preflight、navigation、readはDay / context / RoutineOccurrence / Entryをmaterializeせず、validation failure / stale conflict / injected failureではempty target Dayを残さない。

source / target Day revision、target context、Entry snapshot、RoutineOccurrence relation / origin、Section-plan override、owner、lifecycle、target placement、future establishmentを同一transaction内でguardする。既存`placement_command_guards`のoperation-level uniquenessを弱めず、source / target conditional revision assertionで二Day mutationを保護する。same operation + same semantic requestはcanonical resultをreplayし、entry array orderの差はsemantic fingerprint前にnormalizeする。different source / target / fallback choiceでのoperation ID再利用はmisuseとしてrejectし、retryでEntry二重移動、Day二重establish、revision二重増分を起こさない。ambiguous outcomeはexact operation requestを保持して既存D-020 retry / reconciliation boundaryへ収束させる。

single planned deleteは既存`BulkDeleteEntries`へ一件のEntry IDを渡すUXとして実装し、D-051のdate / lifecycle / owner / revision / retry semanticsを再利用する。ordinary planned EntryはそのEntryだけをDay projectionから削除し、Task / Projectは保持する。Routine planned EntryはRoutineDefinition、RoutineOccurrence、other occurrenceを削除せず、existing occurrence Skip semanticsでその日だけを除外する。running / completed / interruptedは対象外とし、past established Dayのsingle deleteも既存D-051 boundaryを維持する。確認UIはRoutineの`その日だけ削除 / Skip`を明示する。

Bulk selectionではpast established Dayのplanned movable rowだけをdate move用途で選択可能にし、toolbarは`日付変更`だけを提供する。Section / Estimate / Deleteを、past rowが選択可能になったことだけを理由に有効化しない。selectionはstable Entry IDによるephemeral stateとして従来どおりreload / Day navigation / logoutでclearし、collapse / column / Sidebar変更では維持する。single rowの`日付変更`と、Bulkのcommon target date / read-only preflight / fallback warning / one-command confirmを提供し、confirm前はno-writeとする。
...
D-067 is a narrow, explicit correction exception to D-016 historical fact preservation. On the canonical current TaskChuteDay only, an authenticated user may explicitly confirm deletion of one `completed` Entry that represents a mistaken Start → Complete. The command is single-entry and irreversible: it deletes that Entry and every associated `executions` row, including actual start/end and actual-duration facts. Task, Project, other Entries, and unrelated Executions remain unchanged.

- The logical command is `DeleteCompletedEntry`; it is separate from planned-only `BulkDeleteEntries` and never supports completed bulk deletion.
- The server, not the client date, authorizes the current Day using the existing user timezone / day-boundary authority. Owner, Day membership, exact `completed` lifecycle, no-active-Execution, canonical Execution relation, and `expected_placement_revision` are rechecked at commit time. Past, future, planned, running, other-owner, stale, changed-target, and active-execution cases reject without partial effects.
- A successful transaction deletes dependent Executions first because `executions.entry_id` is `ON DELETE RESTRICT`, removes the Entry and Entry-bound guards/snapshots required by the existing FK graph, increments the Day `placement_revision` exactly once, and persists the operation result. Existing Start / Complete operation rows remain retained as retry/idempotency infrastructure; they are not treated as Execution fact authority.
- `Task`, `Project`, `RoutineDefinition`, `RoutineOccurrence`, occurrence task snapshot, future Routine schedule, and unrelated data remain. Keeping the RoutineOccurrence means `ensureCurrentDayRoutineEntries()` recognizes the occurrence and does not rematerialize the deleted same-day Entry; the operation does not become Routine Skip.
- Request identity is client-issued `operation_id` plus the exact semantic request. Same-request retry replays the canonical result, misuse rejects, ambiguous results retain the exact sent request, and no retry may increment the revision or delete another row.
- The Web surface is current completed row `… → 削除` followed by the existing centered destructive modal. It uses the approved Japanese title/body/buttons, safe initial focus, Escape/cancel, focus restoration, and a Routine retention note. Completed rows remain outside bulk selection; planned delete keeps the existing command and semantics.
