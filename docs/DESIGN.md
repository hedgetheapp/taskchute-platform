...
For D-116B, an eligible current-Day completed ordinary Entry shows `ルーティン化`
in the existing Routine cell. After success, a linked future-Routine state is
shown instead of another create action and is visibly/accessibly distinct from
the badge for an actual Routine occurrence. The canonical Day refetch supplies
the source correlation; pending state prevents a second dispatch.
...
Task Note peekはdesktopでは`position: fixed`のfree-floating windowとして表示し、top/status barの空白領域をdrag surface、8方向の内側resize handleを持つ。geometryは`task-note-window-geometry.ts`のpure helperでdefault、preferred read/write、viewport clamp、move、resize、minimized-bar moveを扱う。browser-local preferenceのcanonical envelopeは`{version:1,x,y,width,height}`であり、旧width-only keyは新keyがないときのfallbackに限る。clampは保存済みpreferred geometryを上書きせず、mobile breakpointではfull-sheet presentationを選ぶ。

Minimizeはexpanded DOM stateとpreferred expanded geometryを保持したcompact barへ切り替えるだけで、minimized state自体は保存しない。reload/fresh tabは保存済みgeometryでexpandedとして開く。expanded Noteはglobal Hit-a-Hint keyboard owner、minimized barはfocus中だけlocal ownerとなる。Preview、Note内容のlocal persistence、window identityのserver persistence、Task/Document mutationはこのrefinementに含めない。

UI refinement 2 correctiveでは、expanded windowに通常Save/statusを表示せず、close affordanceをheaderへ一本化した。minimized barはNote iconとcloseだけのcompact surfaceとし、expanded subtreeは`hidden`で保持しながら表示・pointer・focus対象から除外する。desktopのexpanded outside pointerdownはcapture-phaseでminimizeするが、default actionとpropagationを妨げず、背面操作を継続させる。compact barのpointer moveは閾値未満をclick restore、閾値超過をdragとして扱い、実drag後の誤restoreを防ぐ。これらはNote保存・flush・Document/API semanticsとは独立したpresentation interactionである。
## D-101 Task Primary Note

TodayのTask rowには独立した`ノート`列を置き、クリックしたTaskのTask Primary Documentをside-peekで開く。未作成の場合だけ既存のEnsure commandでlazy creationし、Task titleはDay projectionのauthorityをそのまま表示する。EditorはMarkdown source textareaと明示Saveを持ち、約1秒autosave、Ctrl/Cmd+S、close時のfocus復元を既存Notesのinteraction languageへ接続する。side-peekはdesktopではfree-floating fixed windowとしてDay tableのlayoutを縮めず、new-tab modeもsharedな`/?view=note&document=<document-id>`へ遷移する。exact IDがmissing / cross-ownerの場合は別Noteへfallbackせず利用不可を表示する。
...
## D-102 Multi-Task Note windows

desktopのTask NoteはTask stable identityごとに独立したfloating windowとして開ける。同一Taskの再クリックは既存windowを再利用し、別Taskは既存のpreferred geometry seedから24pxずつずらしたcascade位置で追加する。各windowのタイトル、Markdown editor、dirty/unresolved barrier、minimize/restore、geometry、focusは独立し、z-orderは最後にpointer/focusされたwindowをfrontmostへ上げる。兄弟windowを操作しても他windowをminimizeせず、expanded window外のpointerdownだけがfrontmost expanded windowをcompact barへ移行する。
...
Note保存のstatusとは別に、serialized JSON bodyのUTF-8 byte数が約90%に達したときnon-blocking warningを表示し、64 KiBを超える場合はlocal validationとして送信を止める。これはserver request ceilingを変更せず、editorの操作性・autosave・Ctrl/Cmd+Sで同一save guardを使う。
