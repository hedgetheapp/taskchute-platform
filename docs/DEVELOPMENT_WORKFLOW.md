...
通常のTask Contract実行はCodex通常速度を標準とし、主実装をLuna xhigh、探索・軽量実行をLuna Low〜Mediumへ配分する。Luna xhigh / Luna Low〜Medium / Sol Mediumはいずれも通常速度で実行する。Sol Mediumへのescalation条件は維持し、Sol High / Sol xhighは使用しない。品質・安全性・STOP条件の判定は速度によって省略しない。
