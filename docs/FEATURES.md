...
| Day Navigation v0.1 | In design | visual interaction targetは`docs/DESIGN.md`でcanonical化済み。D-041 Approvedにより、未来日readはnon-materializing preview、最初のsuccessful planning mutationはDay/contextとatomicにestablishする。runtimeはNOT_IMPLEMENTED、local / nonprod / production verificationはNOT_RUN。v0.1はfuture Routine previewとnon-current Day executionを含まない |
