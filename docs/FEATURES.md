...
| Day Table interaction | In design | UI-1 / UI-2A / UI-2B / UI-2CとStart Forecast v0.1をmainへIntegrated。current named orderは`実行 / Task / Project / Section / Routine / 見積 / 開始予定 / 開始見込`で、table-owned horizontal scroll、reserved Bulk slot、実用幅でのBulk / Execution / Task fixed-left foundation、Section collapse、focused Taskの`S`、same-Section / same-cohort D&D、D-032 read-only forecastを実装済み。Start Forecast implementation commit `8939c4d6af95e2fd21b7d91e0e946bee29a6c1fb`のshared / Worker / Web automated、real-local browser、APP integrityはPASS。collapse reload persistenceとcross-Section D&DはNOT_IMPLEMENTED、persistent nonprod / productionはNOT_RUN、Released NO。Bulk capability、Mode、Note、full開始/終了/実績、column customization/preferences、Search/Filter/full context interactionは引き続き未実装 |
