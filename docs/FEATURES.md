...
| Day Navigation v0.1 | In design | visual interaction targetは`docs/DESIGN.md`でcanonical化済み。D-041の未来日non-materializing preview / first successful mutation atomic establishmentと、D-042のpast unestablished empty record-none / read-only / no-backfillをApproved。established past Dayはfrozen canonical historyを表示する。runtimeはNOT_IMPLEMENTED、local / nonprod / production verificationはNOT_RUN。v0.1はfuture Routine preview、past historical correction、non-current Day executionを含まない |
