...
| Routine defaults / day override | In design | D-034 Approved。field-level occurrence overrideとRoutine default変更を分離し、Day UI編集時は`今回だけ / ルーティンに反映`を明示選択する。D-043により選択scope内のSection / planned-start pairは通常Entryと同じ同期規則を使うが、override / default persistenceとpropagationは未決 |
