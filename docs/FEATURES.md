...
| Section | In design | user-global stable persistence / bootstrapはmainへ実装。D-038 B1のversioned configuration / established Day context / legacy unknown / `Sectionなし` foundationはlocal candidateでImplemented・local verifiedだがmain未統合。B3のsettings lifecycleを含むbroader Section scopeは未実装 |
...
| Entry planning metadata | In design | D-026 + D-031 Approved。B1のEntry見積は`estimate_seconds INTEGER NULL`としてlocal candidateへImplemented・local verifiedだがmain未統合。planned start + derived placement/orderはB2で未実装 |
