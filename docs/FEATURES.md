...
| Day Table interaction | In design | UI-1 / UI-2AはmainへIntegratedし、current named order、table-owned horizontal scroll、presentation-only reserved Bulk slot、実用幅でのBulk / Execution / Task fixed-left foundation、狭幅CSS fallbackを実装。UI-2B commit `3861b9839b55a1453b0e2f230f03728e8d85059b`でSection collapse / expand、logical Day別in-session state、focused Taskの`S` Start / Completeを、UI-2C commit `95701371d6fe25be1a966789254944b3a1f41eca`でsame-Section / same-cohort Task D&DをmainへIntegrated。UI-2C Web 101 / 101、typecheck / build / diff-check、actual mouse real-local browser、APP integrityはPASS。collapse reload persistenceとcross-Section D&DはNOT_IMPLEMENTED、persistent nonprod / productionはNOT_RUN、Released NO。Bulk capability、Mode、Note、開始見込、full開始/終了/実績、column customization/preferences、Search/Filter/full context interactionは引き続き未実装 |
