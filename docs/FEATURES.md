...
| Entry planning metadata | In design | D-026 + D-031 + D-039 + D-043 Approved。B1のEntry見積はPR #13で、B2 planned startはcommit `316ad0d88f0f88d1445991904da587b1e0987dab`でmainへImplemented / Integrated済み。D-043のreal Section / range内non-null開始予定と`Sectionなし` / `NULL`のfull synchronizationはlocal commit `7d3c0cb0881dfc11725af6ff45eabad69f86a22a`でordinary EntryとR2A選択scopeへ実装済み。source review / local automated / real-local migration・browserはPASS、remote integration pending、persistent nonprod / productionはNOT_RUN、Released NO。broader planning metadata scopeは設計継続 |
| Planned-start ordering | Implemented | D-031 + D-039のderived canonical order、manual tie-break、atomicity / retry、Reorder cohort制約はmainへIntegrated済み。D-043のexplicit real Section選択によるSection開始minute設定とplanned-start clearによる`Sectionなし`同期はlocal commit `7d3c0cb0881dfc11725af6ff45eabad69f86a22a`で実装し、source review / local automated / real-local browserはPASS。remote integration pending、persistent nonprod / productionはNOT_RUN、Released NO |
...
| Minimal Routine R1 | Implemented | D-040 Approved。runtime commit `f9324e866deb74277d2fd83c5945f2df4b2b95da`をPR #14 merge commit `ebaff6d156813ba78b4c5c28818f9f55db9fd970`でmainへIntegrated。identity-preserving Routine化、daily lazy materialization、defaults、revision exactly once、inclusive end / Routine終了、minimal Web UXを実装。R1 baselineのRoutine planning field read-only境界は、local R2A commit `7d3c0cb0881dfc11725af6ff45eabad69f86a22a`のdedicated scope commands / UXでcurrent-Day planned Entryに限り拡張済み。R1 local / persistent nonprod evidenceはPASS。R2Aはsource review / local automated / real-local migration・browser PASS、remote integration pending、persistent nonprod / production NOT_RUN、Released NO |
| Routine defaults / day override | Implemented | D-034 + D-044 + D-045 + D-046 Approved。local commit `7d3c0cb0881dfc11725af6ff45eabad69f86a22a`でcurrent-Day planned Routine-derived Entryの`Section + 開始予定`同期unitと独立見積unit、no-write candidate、unit別`今回だけ / ルーティンに反映`、explicit NULLを含むtyped occurrence override、current defaultへのreset、eligible non-overridden planned occurrenceへのno-future-materialization propagation、Routine default revision、D-045 normalizationを実装。Source Review、Worker / D1 `117 / 117`、Web `78 / 78`、migration `2 scenarios`、typecheck / build / diff-check、real-local `0007` migration・preservation / signed-in browserはPASS。multi-Day propagation / retry / concurrency等はlocal automated evidenceで、persistent nonprod / productionはNOT_RUN、remote integration pending、Released NO |
