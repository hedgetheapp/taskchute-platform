...
| Routine foundation | Implemented | D-040 / D-043〜D-048 Approved。R1 / R2AはIntegrated。R2B local candidateでTask 0..1 Routine、Routine Board、typed recurrence / pause / suppression / snapshot、independent Board orderを実装。R2B source review / local automated / real-local `0008` migration・representative browserはPASS。controlled inclusive end-date browser subcaseは`TOOLING_BLOCKED / NOT_VERIFIED`、historical title / Projectのpast-Day browser subcase、persistent nonprod / productionはNOT_RUN。未commit・未統合、Released NO |
| Routine Board R2B | Implemented | D-047 / D-048のlocal candidate。Sidebar Board、OFF create、daily / N-day / weekly、inclusive period、pause/resume、schedule suppression/restore、Task metadata snapshot、inline defaults、independent reorderを実装。OFF metadata更新による誤suppressionをsource review / real-localで検出・修正済み。local candidateのためIntegrated / Verified / Releasedへ昇格しない |
