...
| Day Table interaction | In design | UI-1 commit `da4a8c8316d60d942dc73fbd53bb90d15df5517b`をmainへIntegrated。current named orderは`実行 / Task / Project / Section / Routine / 見積 / 開始予定`。UI-2Aはlocal commit `43789c990ed91febb2bb6036c1f3970dfe8f34a1`でtable-owned horizontal scroll、presentation-only reserved Bulk slot、実用幅でのBulk / Execution / Task fixed-left foundation、狭幅CSS fallbackを実装し、Web 84 / 84、typecheck / build / diff-check、signed-in wide / medium / narrow browser、APP integrityはPASS。GitHub canonical `main`未統合、persistent nonprod / productionはNOT_RUN、Released NO。Bulk capability、Mode、Note、開始見込、full開始/終了/実績、column customization/preferences、Search/Filter、Section collapse、D&D/full context interactionは引き続き未実装 |
