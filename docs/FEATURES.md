...
| Section | In design | user-global stable persistence / bootstrapはmainへ実装。D-038 B1のversioned configuration / established Day context / legacy unknown / `Sectionなし` foundationはPR #13でmainへImplemented / Integrated済み。local + persistent nonprod remote verificationはPASS、production verificationはNOT_RUN、Released NO。B3のsettings lifecycleを含むbroader Section scopeは未実装 |
...
| Entry planning metadata | In design | D-026 + D-031 + D-039 Approved。B1のEntry見積はPR #13で、B2 planned startは`planned_start_minute INTEGER NULL`としてcommit `316ad0d88f0f88d1445991904da587b1e0987dab`でmainへImplemented / Integrated済み。B2 source review / automated / real local migration / signed-in browser verificationはPASS、persistent nonprod / production verificationはNOT_RUN、Released NO。broader planning metadata scopeは設計継続 |
| Planned-start ordering | Implemented | D-031 + D-039のderived canonical order、既存`position`によるmanual tie-break、SetEntryPlannedStart atomicity / retry、Section move時clear、Reorder cohort制約をcommit `316ad0d88f0f88d1445991904da587b1e0987dab`でmainへ実装。source review / automated / real local browser verificationはPASS、persistent nonprod / production verificationはNOT_RUN、Released NO |
...
| Today / DayBoard ordering | Implemented | explicit Entry order + placement revision conflict protectionに加え、D-031 / D-039のNULL-first、planned minute昇順、same-minute manual tie-break、cohort-aware ReorderをB2で実装・local verification PASS |
...
| Day planning persistence B1 | Implemented | PR #13でmainへmerge・Integrated済み。local automated + source review + real local APP migration + signed-in browser + persistent nonprod migration/runtime/browser verificationはPASS。production verificationはNOT_RUN、Released NO。Section settings lifecycleはB3として未実装 |
| Day planning persistence B2 | Implemented | `0004_dogfood_day_b2.sql`、planned-start persistence / command、derived placement/order、Web editorをcommit `316ad0d88f0f88d1445991904da587b1e0987dab`でmainへIntegrated。source review、Worker 87、Web 49、isolated migration 25 checks、real local migration / browserはPASS。persistent nonprod / productionはNOT_RUN、Released NO |
...
| Routine defaults / day override | In design | D-034 Approved。field-level occurrence overrideとRoutine default変更を分離し、Day UI編集時は`今回だけ / ルーティンに反映`を明示選択する |
