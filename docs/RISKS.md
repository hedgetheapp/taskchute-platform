...
no-write、Routine occurrence-only mappingを再利用する。D-120 focused App `293 / 293`、Worker `11 / 11`、
full Web `14 files / 474 tests`、Worker/D1 `37 files / 339 tests`、typecheck/build/guard/diff-check、
persistent nonprod/DB、exact final CIはPASSした。current/future browserではkeyboard placement/reorder、
single / multi-selectionの即時order・対象focus保持、
...
Project / Mode編集はcurrent established Dayのordinary running Entryだけに限定し、既存のCAS / operation /
retry boundaryへ接続した。planning、lifecycle、D&D placement semantics、Routine relationは変更しない。
ProjectはTask-level authority、Modeはlive Entry relationを更新するが、開始時Mode snapshotとcompleted historical
projectionは保持する。初回Web-only実装でresource conflictとなった証拠は歴史的に保持し、承認済みcorrectiveで
Worker eligibilityとWeb UIを整合させた。残るリスクは、running中のlive Mode変更後にcompletedへ遷移した場合も開始時
snapshotを履歴authorityとする既存boundaryを、将来のProduct semantics拡張で再確認することである。
...
automated/local evidenceとは分離する。D-107 correctiveのGitHub-hosted Linux runnerはKVMが
利用できずAPI 33 AVDが`sys.boot_completed`へ到達しなかったが、後続のlocal
`TaskChute_API33`（API 33 / Google APIs / x86_64）で`connectedDebugAndroidTest`全10件PASS、
screenshot / UI tree / crash buffer emptyを確認した。Galaxy S23 smokeもユーザー確認済みPASSで
あり、Emulator evidenceとは別に扱う。今後はGitHub runnerのKVM依存をCI runtime evidenceへ
再利用せず、local AVDまたは別のhosted emulatorが必要な点を残存riskとする。
...
Mitigation/evidence: Worker derives the Hub key from the authenticated app user, validates the WebSocket Origin and internal handoff, accepts no domain command over the socket, and publishes only after `response.ok` from a canonical mutation. The Web client validates a versioned bounded protocol, uses bounded exponential reconnect with jitter, coalesces scope refreshes (including same-microtask duplicate and wildcard Day scopes), and defers refresh while local mutation/editor barriers exist. Project mutation scope includes dependent Documents, and the nonprod guard requires exactly one SQLite `REALTIME_HUB` binding. Focused protocol/client/DO tests, full Web `14 files / 442 tests`, full Worker/D1 `36 files / 307 tests`, exact pushed-SHA CI run `34808771287`, and persistent nonprod binding/HTTP/DB checks are PASS. Same existing authenticated sessionの独立browser tabsでDay / Project / Mode / Routine / clean Document propagation、dirty draftの非上書き、same-tab/fresh-tab canonical restorationを実測し、CUA exposed warning/errorは`0 / 0`だった。一方、CUAにsocket単独切断・reconnect手段がないためdisconnect/reconnect browser E2Eは`NOT_RUN`であり、browser run内のCAS conflict saveもpayload hard-stopにより完了していない。browser操作後のD1 read-only再照会はCloudflare API `7403`で実行できず、直前deploy時のDB evidenceを再利用し、post-browser recheckは`NOT_RUN`とした。No DO domain state, APP/AUTH migration, polling, offline persistence, or production rollout is included.
...
Mitigation/evidence: focused Web `5 files / 328 tests`, full Web `13 files / 438 tests`, full Worker/D1 `34 files / 301 tests`, typecheck/build/exact nonprod build/deploy guard/Wrangler dry-run/diff-check pass; GitHub Actions run `34800601555` for test SHA `cd8a4882cf0534e278ee948ce57a98571de831c4` is success. The common client helper measures the exact serialized JSON UTF-8 body, warns near the existing 64 KiB Worker ceiling, and blocks over-limit sends without changing draft state. Authenticated browser verification is explicitly `AUTHENTICATED_BROWSER_NOT_VERIFIED` because no persistent browser tab was available and credentials/re-login were prohibited.

The repository-safety boundary is intentionally narrow: ruleset `23241921` is active for `main` with deletion and non-fast-forward rules only. Direct normal fast-forward push remains allowed; PR requirement and required status checks remain off in v0.1. The CI workflow is informational/read-only and never deploys or mutates D1. Final persistent nonprod Worker `8d413923-1f2f-444c-b760-b4b4a06ac134` returned root `200` and unauthenticated Documents API `401`; APP/AUTH pending is `0 / 0`, quick checks are `ok`, FK results empty, owner/document duplicate key and other anomaly/guard probes are `0`, and successful read-only probes report `rows_written=0`. Production and restore/recovery remain not run.
...
D-090 Corrective 2 closure: source review identified that the prior barrier was coupled to `dirty`, leaving a clean existing Note vulnerable to in-app navigation/logout and unload after an ambiguous Update. The corrective commit `519ed3c090dd1021c3c9c0bf01cee27197ed7ca6` makes unresolved state independently blocking in App transitions and `beforeunload`; focused B1–B5 tests and the full Web suite pass. The exact retained request remains the only retry path until exact Retry or exact-ID canonical reconciliation resolves it.

## R-042 — Standalone Note autosave and lifecycle closure

D-091–D-093 add immediate standalone Note creation, server title allocation, autosave, archive/restore, and confirmation-gated hard delete. The primary risks are duplicate Create during debounce/retry, title collision with archived rows, stale Update, lifecycle mutation during an in-flight save, and accidental loss of an ambiguous request.

Mitigation / evidence: APP 0030 adds only `archived_at`, owner-scoped standalone title uniqueness across active and archived rows, and the existing operation allow-list extension. Worker tests cover exact replay/misuse, title allocation, revision CAS, lifecycle CAS, assertion rollback, and owner scope. Web tests cover immediate Create, autosave, explicit flush, dirty/unresolved barriers, archive view, read-only archived editor, and hard-delete confirmation. No dependency, AUTH migration, request-limit change, attachment storage, or public endpoint was added.

Persistent nonprod evidence is PASS: the migration hard-gate exports were independently readable, APP 0030 was applied once, pending is `0 / 0`, root is `200`, unauthenticated Documents API is `401`, and final APP/AUTH quick/FK and duplicate/invalid/guard probes are clean with `rows_written=0`. The existing authenticated session and two fresh authenticated tabs verified immediate Create, autosave, reload persistence, archive/restore, and disposable-fixture hard delete; console warnings/errors were `0 / 0`. The normal Windows migration helper remains `NOT_RUN` because it exceeded the bounded wait and was safely interrupted; bounded fresh/upgrade validation is PASS. Production, restore/recovery execution, and unrelated destructive cleanup remain not run.

### R-043 — Lifecycle ambiguity exact retry and title allocation race evidence

D-091–D-093 source review reopened two narrow risks. A lifecycle archive/restore/delete response classified as `infrastructure_ambiguous` retained no exact Web request, allowing a later click to create a new operation identity; and the prior title-allocation evidence did not deterministically exercise concurrent Create/Create, Create/Rename, Rename/Rename, archived-title reservation, or owner isolation.

The corrective `82bc3614a361f3638d2860d5f96875bc3d6a09f8` freezes the original lifecycle payload behind an unresolved barrier, exposes only exact Retry, blocks replacement lifecycle actions, and never treats missing hard-delete state as success. The earlier archive/restore GET-equivalence resolution is superseded by R-044. Deterministic RACE-1 through RACE-5 Worker tests now prove unique smallest-suffix allocation, archived titles participating in the reservation, owner scoping, and exact replay of each allocated result. Focused/full Worker and Web suites and static gates pass; no Worker/API/schema/migration/dependency change was introduced.

The corrective was deployed to persistent nonprod as Worker `50adf62e-1a57-479f-97a2-9020c6ffd494`. Root/API401, APP/AUTH pending `0 / 0`, quick/FK, duplicate Document/operation, invalid Document, assertion/guard, and read-only `rows_written=0` probes are PASS. Existing authenticated D-091–D-093 browser evidence remains PASS; deterministic ambiguity fault injection is test-only evidence and is not claimed as a browser result. Production, restore, credential/re-login, migration reapply, and unrelated destructive cleanup remain not run.

### R-044 — Do not borrow equivalent lifecycle state

D-092 lifecycle ambiguity had a narrower no-borrow risk: an archive/restore GET could show the requested final revision/state even when another operation produced it, and the Web could incorrectly clear the retained operation without replaying its own identity.

Corrective `aaa4b819d03e33f15b19ae192180563aab72fe12` treats GET only as informational and requires exact replay of the retained `operation_id`, `document_id`, `expected_revision`, and archived target for resolution. Deterministic NB-1–NB-5 tests cover equivalent archive/restore state, exact replay, revision conflict, and hard-delete 404. The corrective has no Worker/API/schema/migration/dependency change; focused/full suites, static gates, and persistent nonprod safety/DB evidence pass. Existing ordinary authenticated browser evidence remains PASS, while ambiguity fault injection is not claimed as browser evidence.

### R-045 — Web temporary-surface consistency

D-094 and D-095 reduce UI consistency risk by sharing outside-click / Escape cleanup, strict logical-date input, Today-style calendar structure, destructive-action styling, fixed status placement, Routine viewport scroll ownership, and a geometry-stable Notes rail across the affected Web surfaces. Full local regression/static gates and exact nonprod deploy pass. Existing authenticated browser checks at approximately `592 × 884` with sidebar open and a same-session fresh tab at approximately `1234 × 711` observed the shared calendar and unchanged Notes ON/OFF geometry; browser console logs were retrieved as an empty array. Pixel-perfect cross-device geometry, exact zoom metadata, and broader viewport permutations remain outside this evidence, but no API/Worker/schema/migration/dependency change was introduced.

D-095 corrective `704570bc7c618329f3a7bd41c780c59a38e5b43d` closes the remaining Today-state parity and trigger ambiguity: Routine/Effective Day receive canonical current logical date separately from selected date, and the independent calendar button/icon is removed so the text field is the only normal trigger. Focused/full Web tests, full Worker/D1 regression, static gates, exact nonprod deploy, existing authenticated browser, same-session fresh tab, root/401, APP/AUTH quick/FK, pending `0 / 0`, and read-only `rows_written=0` evidence pass. The remaining risk is limited to broader device/zoom permutations not covered by this connector; browser console logs were `[]`.

## R-046 — D-083 empty real-Section Shift traversal

D-083 source review found that keyboard cross-Section traversal could skip an empty configured real Section because the movement list was derived from groups containing entries. A draft-only `Sectionなし` group could also be mistaken for a real traversal destination.

Corrective `bdaa561c311661154aa11ea20f04adaa02a9b80f` now treats every configured real Section as a destination, keeps `Sectionなし` conditional on an actual effective unsectioned Entry, and reuses the existing Section-only MoveEntry / Routine chooser paths. Focused `4 / 4`, full Web `5 files / 300`, full Worker/D1 `32 files / 285`, typecheck/build/exact nonprod build/deploy guard/dry-run/diff-check, authenticated representative Shift interaction, same/fresh-tab persistence, and APP/AUTH read-only integrity pass. No Worker/API/schema/migration/dependency change was introduced.

The exact empty-target semantics are automated-test verified. Persistent browser verification used a safe disposable pair on the authenticated current Day and proved real keyboard movement into the adjacent configured Day Section and persistence, but that Day already contained existing dogfood rows; therefore the exact empty-target browser subcase remains `NOT_VERIFIED` rather than being inferred. The CUA surface did not expose numeric viewport/zoom or console APIs. Disposable fixture rows remain in nonprod; no unrelated data, production, credentials, bootstrap, restore, or destructive cleanup was touched.

## R-047 — D-096 continuous Shift placement and suppression-visible reorder parity

D-096 extends only ordinary planned current-Day keyboard placement input. The Web keeps the existing global serial Day mutation dispatcher, derives each new gesture from canonical Day plus pending Move/Reorder overlays, preserves Move steps across queue barriers, and keeps lifecycle, retained ambiguity, Routine, historical, future/past, and other non-commutative placement boundaries intact. The Worker Reorder command now applies the Day projection's suppression predicate consistently across its full visible membership/CAS/assertion/mutation universe while reading the full physical Section for temporary collision-free offsets; suppressed Routine physical Entries remain untouched and excluded from visible payloads.

Focused deferred-response coverage and full local Worker/Web suites pass for Move → Move, Reorder → Move, Move → Reorder, effective-order derivation, and suppression preservation. Persistent nonprod deployment and read-only integrity checks pass. The authenticated connector accepted two real Shift gestures and restored the result in same/fresh tabs, but its fast response path did not expose the exact multi-request race timing (`保存中 1件` was observed rather than a stable multi-item window); deferred automated tests remain the authoritative race evidence. The connector exposed neither console logs nor numeric zoom metadata, so those evidence dimensions remain `NOT_VERIFIED`. No API/schema/migration/dependency/security change was introduced.

## R-048 — Today initial Arrow focus and distinct Task focus indicator

D-097 narrows the Today keyboard-entry risk: plain ArrowUp and ArrowDown now share the first visible canonical Task bootstrap only when no Task owns focus, while existing J/K, Shift placement, collapsed-section, draft/provisional, interactive-control, modal/popover, calendar, and IME ownership remain unchanged. The document-level bridge is Today-scoped and capture-phase, guarded against default-prevented events, modifiers, composition, and an already-focused Task. The Task lifecycle background remains the state channel; `:focus-visible` provides the independent 2px accent-blue outline without layout shift.

Focused App tests and full Web/Worker/static gates pass, and fresh authenticated persistent tabs observed real ArrowDown/ArrowUp focus on the same first visible canonical completed Task at `1280×720` with Sidebar open. Running+focused browser observation and console retrieval remain `NOT_VERIFIED` because no safe running fixture was available and the connector exposes no console API; running/completed DOM-state coverage is automated PASS. A stale browser Accessibility index also caused an unintended existing non-sensitive Task to be started and immediately completed during evidence collection; final active executions are `0`, and this incident is retained in CURRENT for audit transparency. No Worker/API/schema/migration/dependency change was introduced.

## R-049 — D-097 focus frame occlusion by sticky Day cells

D-097's initial focus behavior was functionally correct, but its parent row outline could be painted underneath sticky Bulk / Execution / Task cells. The corrective `c2e88cf81d6c92e20a9355e6d0e8c2f77cf5748e` uses a pointer-transparent row-local absolute overlay above those descendants, without changing lifecycle backgrounds, row geometry, D&D feedback, or keyboard semantics.

Focused source coverage and full local suites/static gates pass. In the deployed authenticated persistent tab, real focus on planned and completed rows showed the continuous frame across sticky and nonsticky cells, and the frame remained visible after horizontal scrolling. Running-focus visual browser evidence and exact console counts remain `NOT_VERIFIED` because no safe running fixture was exercised and the connector does not expose console APIs. No Worker/API/schema/migration/dependency change was introduced; production, restore, credential/re-login, bootstrap, and unrelated destructive cleanup remain not run.

## R-050 — D-098 Add focus continuity and hierarchical Escape evidence boundary

D-098 addresses two Today keyboard/focus risks: canonical Add reconciliation could return focus to a stale row instead of the user's focused cell, and row Escape could skip the intended editor/cell/row hierarchy. The Web-only implementation uses a generation-scoped semantic focus locator, preserves outside-row/user pointer focus, handles successful and ambiguous Add reconciliation consistently, and stops inline-editor Escape propagation at the editor owner. No Worker/API/schema/migration/dependency change was introduced.

Focused D-098 coverage (`7 / 7`), full Web/Worker suites, typecheck, builds, deploy guard, dry-run, and read-only nonprod safety passed. Existing authenticated browser evidence is intentionally partial: a real title editor open + first Escape was observed and `dev.logs()` returned `[]`, but connector AX/input instability prevented safely establishing a disposable Add fixture and observing Add→Tab focus continuity or the second cell→row Escape. Those browser items remain `NOT_VERIFIED`; automated deferred-response evidence is authoritative. The existing tab's active execution/stale placement state was left untouched, with no direct cleanup. Future verification should use a stable authenticated browser surface and a disposable Task only.

## R-051 — D-099 Hit-a-Hint keyboard ownership and browser evidence boundary

D-099 adds a Web-only transient keyboard mode whose capture-phase listener must remain the sole owner of unmodified Hint input while active. The primary risks are accidentally activating existing `S / I / N / E / D / X / J / K / Arrow` shortcuts, labeling hidden or stale targets, changing editable values when a focus-only target is selected, or allowing D-098 pending Add reconciliation to steal a Hint-selected focus.

Mitigation is centralized target snapshot/filtering, fixed-length prefix-free labels, explicit local keyboard-owner guards, capture-phase propagation control, stale-target validation, pointer/scroll/resize cancellation, portal-only badges, and reuse of D-098 focus authority. Focused Hit-a-Hint `32 / 32`, App + Hint `291 / 291`, full Web `350`, full Worker/D1 `287`, typecheck/build/exact nonprod build/deploy guard/dry-run/diff-check pass locally. No Worker/API/schema/migration/dependency change was introduced.

The risk was verified after exact main push and persistent nonprod deployment. Existing authenticated tabs at `1569 × 912` (DPR 1, Sidebar open) showed 124 Today badges, safe label activation to Notes, 17 Notes badges, `?` suppression while Hint was active, Escape cancellation, same-tab reload persistence, and a fresh authenticated tab with 66 badges; CUA console logs were `[]`. Root `200`, unauthenticated Documents API `401`, APP/AUTH pending `0 / 0`, quick checks `ok`, FK empty, duplicate Document/title groups `0`, assertion residue `0`, and read-only `rows_written=0` evidence pass. No Worker/API/schema/migration/dependency change was introduced beyond the Web-only D-099 feature; production, restore, credentials, bootstrap, and destructive cleanup remain not run.

The D-099 corrective closed the remaining neutral-focus cancellation risk. Escape now blurs only the connected explicit negative-tabindex neutral container when it remains active, with a next-frame recheck for browser refocus timing; Task row and other meaningful focus surfaces are preserved. In the authenticated persistent tab at `1379 × 912` (DPR 1, Sidebar open), neutral Today focus → `F` → `Escape` left `BODY` active with no full-container outline, and `ArrowDown` restored normal row navigation. Task row focus also remained on the same logical row through cancellation. Focused `34 / 34`, full Web `352`, final Worker version `f4a1e106-5d43-4a0c-a3fc-2192ef543946`, console `[]`, quick/FK, and `rows_written=0` evidence pass. No Worker/API/schema/migration/dependency change was introduced by the corrective.
## R-052 — Task Primary Document identity and Day Note surface

D-101 introduces a second approved Document kind without making the generic Document row Task-specific. The principal risks are duplicate Task relations, a Task Primary Document leaking into another owner or Task, a Day projection accidentally loading Markdown body, a new-tab bootstrap becoming a write-capable GET, or a Task title being silently forked into Document state. APP `0031_task_primary_documents.sql` enforces owner/task/document foreign keys, one relation per owner/task, stable kind, and existing operation compatibility; the Day projection carries only nullable `primary_document_id`.

Mitigation/evidence: APP `0031` preservation and the D-101 corrective focused/full suites pass. The corrective adds the shared owner-scoped `/?view=note&document=<document-id>` resolver, exact archived/missing/cross-owner behavior without fallback, shared copy-link affordances, and v3 Day-column preference migration before older formats. Corrective 2 additionally resolves `task_title` by owner-scoped join to the current Task authority, so a generic or legacy permalink cannot display a stale Today title or `Task Note` merely because the Task is absent from the current projection; missing relation/Task is unavailable. Worker `3306f046-486a-404c-bc1b-c3c3be4d9c69` returned root `200` and unauthenticated Documents API `401`; APP/AUTH pending was `0 / 0`, quick checks were `ok`, FK checks empty, and read-only probes reported `rows_written=0`. Existing authenticated Task and disposable standalone Note verification covered same/fresh-tab persistence, active/archived exact opening, and empty console logs. Direct persistent v3 localStorage fixture injection remains `NOT_VERIFIED` because the connector page-evaluation surface is read-only; automated v3→v4 coverage is PASS. No body/title content is recorded here.

The approved boundary remains narrow: Task Primary has no archive/delete lifecycle, standalone Note lifecycle is unchanged, Project/RoutineOccurrence relations remain out of scope, and no attachments, preview, search/backlinks, offline persistence, dependency, AUTH migration, production, restore, or unrelated destructive cleanup were added.

## R-053 — D-101 shared Note editor and resizable Task Note peek

The D-101 UI refinement introduces shared Markdown editor markup across standalone and Task Notes, a line-number preference, and a user-resizable fixed Task Note peek. The risks are divergent editor behavior, a resize interaction changing Document state, preference values escaping the viewport, keyboard leakage into Today/Hit-a-Hint shortcuts, and layout or mobile regressions.

Mitigation/evidence: `NoteMarkdownEditor` is the single Web editor primitive with a fixed gutter and scroll synchronization; line-number preference uses the existing versioned localStorage convention. Peek width is isolated in pure helpers, persisted as `{version:1,width}`, clamped for display, and controlled by a pointer-capture separator with keyboard Home/End/Arrow/Shift semantics. The editor subtree is a local keyboard owner. Focused impacted Web `333 / 333`, full Web `10 files / 382`, typecheck/build/exact nonprod build/deploy guard/Wrangler dry-run/diff-check pass. No Worker/API/schema/migration/dependency change was introduced.

Persistent authenticated browser verification at approximately `1380 × 910` with Sidebar open observed the present/absent SVG glyphs, shared Notes/Task editor surface, real pointer resize `524px → 654px`, same-tab reload and fresh-tab width persistence, no save-state change, and no console warnings/errors (`[]`). Worker/D1 verification is `NOT_REQUIRED` by source-scope review; this refinement has no persisted server behavior. Production, restore/recovery, credentials/re-login, bootstrap mutation, unrelated destructive cleanup, and Release remain not run.

## R-054 — D-101 floating Task Note geometry

The D-101 UI refinement 2 changes only browser-local presentation geometry. Risk is that a saved position/size becomes unreachable after viewport changes, that minimized state or Note content leaks into the geometry preference, or that resize/drag keyboard ownership mutates a Document or leaks into Today shortcuts.

Mitigation/evidence: pure geometry helpers enforce `360 × 320` minimums, safe-gutter clamping, eight-direction resize, keyboard edge resize, preferred/rendered separation, and a flat versioned envelope. The legacy width key remains read-only fallback; the new key stores only geometry. Minimize retains expanded dimensions in memory and is deliberately not persisted. Focused geometry/TaskNoteEditor/Hit-a-Hint coverage is `64 / 64`, full Web `11 files / 404 tests`, and static gates pass. Persistent authenticated CUA verification observed real title-bar drag, real edge resize, minimize/restore, same/fresh-tab geometry persistence with fresh-viewport clamp, minimized/outside focus Hint behavior, no save-state change, no Preview UI, and empty console logs (`[]`).

This is Web-only: Worker/D1 and migration verification are `NOT_REQUIRED` by source-scope review, and no Document/API/schema/dependency behavior changed. Broader device/zoom permutations and pointer race timing were not separately measured; automated interaction coverage remains the authority for those boundaries. Production, restore/recovery, credentials/re-login, bootstrap mutation, unrelated destructive cleanup, and Release remain not run.

## R-055 — D-101 floating Note chrome corrective

The chrome corrective removes visible normal save/status controls and the duplicate footer close while retaining the existing save paths and ambiguity barrier. The residual risk is interaction regression at the boundary between compact-bar drag/click, desktop outside-click minimization, and background keyboard ownership.

Mitigation/evidence: focused TaskNoteEditor/Hit-a-Hint coverage is `52 / 52`, full Web is `11 files / 407 tests`, and typecheck/build/exact nonprod build/deploy guard/Wrangler dry-run/diff-check pass. Persistent authenticated browser verification at `1379 × 912` observed compact-only minimized chrome, real compact-bar pointer drag without restore, icon restore, outside Today click with continued navigation, and Hit-a-Hint after uppercase `F`; a fresh authenticated tab at `1280 × 720` restored the canonical Note without save/status chrome. Both tabs reported empty warning/error logs (`[]`). No Worker/API/schema/migration/dependency behavior changed; broader device/zoom permutations remain outside this corrective.

## R-056 — D-101 floating Note chrome v2

The v2 window-control refinement adds a compact minimize/maximize-or-restore/close group and a minimized-bar title. Risks are ambiguous icon meaning, title overflow hiding controls, maximize geometry becoming unreachable after viewport changes, or disabled max-state interactions changing the existing normal window behavior.

Mitigation/evidence: controls have explicit Japanese accessible names and deterministic DOM order; the minimized main area uses flex/ellipsis while the action area remains fixed; maximize derives safe `16px` viewport bounds and keeps preferred normal geometry untouched; drag/resize/keyboard handlers and resize handles are guarded while maximized. Focused coverage is `34 / 34`, full Web is `11 files / 412 tests`, and static gates pass. Persistent authenticated browser evidence confirmed icon-only controls, minimized title/close, maximize/restore geometry, real title-bar drag, real resize, handle removal/restoration, and console logs `[]`. This remains Web-only; broader device/zoom permutations are not separately measured, and Worker/D1/migration/API/dependency verification is `NOT_REQUIRED`.

## R-057 — D-101 minimized-bar duplicate restore affordance corrective

The v2 minimized-bar chrome had a duplicate Note restore button beside the bar's own restore behavior, which could create competing focus/click semantics. The corrective removes the duplicate button and makes the focusable bar itself the single restore target while excluding `×` from restore. Focused coverage is `20 / 20`, full Web is `11 files / 414 tests`, and typecheck/build/exact nonprod build/deploy guard/Wrangler dry-run/diff-check pass. Authenticated current-asset browser verification confirmed one Note icon, current Task title, close-only action area, bar focus after minimize, pointer body/title restore, Enter/Space restore, close without restore, and console logs `[]`. No server behavior changed; broader mobile/device/zoom permutations remain outside this corrective.

## R-058 — D-102 Multi-Task Note window coordination

複数Task Noteを同一tabで表示すると、window identityの重複、z-orderの不整合、背面windowの誤minimize、active routeと表示windowの不一致、dirty/unresolved windowのnavigation barrier取りこぼしがリスクになる。D-102ではApp registryをsingle coordinatorとし、Task stable identityでdeduplicate、per-window flush/barrier stateを集約、frontmost expanded windowだけをoutside-click ownerとし、active Documentだけをcanonical routeへ反映する。新windowは既存geometryから24px deterministic cascadeし、window stateはmemory-onlyでreload/fresh tabは最大1つをrouteから復元する。

Focused Web `3 files / 301 tests`、full Web `11 files / 416 tests`、typecheck/build/exact nonprod build/deploy guard/Wrangler dry-run/diff-checkがPASS。既存authenticated tab（`1379 × 912`、Sidebar open、DPR 1）で2 Task Noteの独立root/z-index/cascade、実pointer sibling activation、frontmost-only outside minimize、same-task dedup、reload/fresh authenticated tab single-route restorationを確認し、existing/fresh console logsは`[]`だった。Task Note save/CAS/ambiguity semantics、Document/API/Worker/schema/migration/dependencyは変更していない。mobile single-sheet、multi-tab synchronization、自然なsave race timing、別zoom/device permutationsはこの確認の未実施範囲であり、既存focused testsが該当境界のauthorityとなる。

## R-059 — D-102 cascade collision and inactive control activation corrective

48px cascadeは24pxよりwindow間の視認性を確保する一方、viewport端ではclampにより複数候補が同一geometryへ折り畳まれる可能性がある。さらに、z-orderをReact child orderと共有するとactivationのpointerdownと後続control clickの間で背面windowのhit targetが不安定になる。

Mitigation/evidence: `cascadeTaskNoteWindowGeometry()`はoccupied rendered geometryを入力として固定された方向・距離順のpure fallbackを使い、Appはregistry arrayのDOM順を保持してz-index mapだけを更新する。focused geometry/App Webは`2 files / 283 tests`、full Webは`11 files / 418 tests`、typecheck/build/exact nonprod build/deploy guard/Wrangler dry-run/diff-checkがPASS。Persistent authenticated browser（`1379 × 912`、Sidebar open）で48px差の2 window、実pointerによる背面link copy、minimize/restore、maximize/restore、minimized-bar close、same/fresh-tab restoration、console `[]`を確認した。mobile/別zoom/device permutations、自然なsave race timing、Worker/API/DB semanticsはこのcorrectiveの未実施または`NOT_REQUIRED`範囲であり、Document save/CAS/ambiguity semanticsは変更していない。
## R-060 — D-103 Project Primary Document identity and lifecycle boundary

D-103 extends the shared owner-scoped Markdown `Document` foundation with a lazy `project_primary` relation. The main risks are duplicate Project/document relations, a Project title becoming stale Document state, Project Primary leakage across owners, a resolver falling back to an unrelated Note, or Project deletion leaving an orphan relation/document. APP `0032_project_primary_documents.sql` enforces typed Document kind, owner/project/document foreign keys, one relation per owner/project and document, and the existing operation allow-list. Ensure and Update use the existing transaction assertion, exact replay/misuse, owner-scope, and revision-CAS conventions. Project title remains the Project row authority; the Project Primary Document stores Markdown body only.

Mitigation/evidence: bounded fresh/upgrade migration validation passed; focused Project Primary/Project management `8 / 8`, full Worker/D1 `34 files / 301 tests`, full Web `11 files / 418 tests`, typecheck/build/exact nonprod build/deploy guard/Wrangler dry-run/diff-check passed. Exact pushed main was deployed as Worker `153e10dc-585a-427b-94e3-191325d29caa` to canonical `taskchute-web-nonprod`; APP 0032 was applied only after fresh APP/AUTH backup and isolated `quick_check=ok`/FK 0 validation. Remote APP/AUTH pending is `0 / 0`, quick checks are `ok`, FK results are empty, relation/document/position/assertion/active-execution anomaly probes are `0`, and read-only probes report `rows_written=0`.

Existing authenticated browser verification used a dedicated non-sensitive Project fixture. Project Board Note affordance, lazy Project Primary creation, body save, Notes Project filter, same-tab reload, and fresh authenticated-tab persistence passed; Project title authority was visible and both tabs' console warning/error logs were `[]`. The fixture remains as non-destructive nonprod verification data. Task Primary, standalone Note, RoutineOccurrence Document, attachments, preview/search/backlinks, production, restore/recovery, credential/re-login, and unrelated destructive cleanup remain outside this work item.

D-103 corrective evidence: Today now distinguishes active Project loading (`idle / loading / loaded / error`) so an assigned Project is not mislabeled as archived before authoritative list success. The Project cell keeps the selector flexible and the Note trigger stable, while Notes Project rows use one body-only inline editor per Project/tab; an already mounted floating Project window is activated/restored instead of creating a second writer. Focused App + Notes `292 / 292` and full Web `422 / 422` passed, and the exact pushed main was deployed to canonical nonprod Worker `412df8a0-ac18-414f-b290-9ca39da630b9`. The new corrective browser A/B/C evidence remains `AUTHENTICATED_BROWSER_NOT_VERIFIED` because CUA exposed no persistent authenticated tab in this run; prior D-103 browser evidence is not relabeled.
