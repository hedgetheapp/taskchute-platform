...
The corrective `82bc3614a361f3638d2860d5f96875bc3d6a09f8` freezes the original lifecycle payload behind an unresolved barrier, exposes only exact Retry, blocks replacement lifecycle actions, reconciles archive/restore only by exact document identity/revision/state, and never treats missing hard-delete state as success. Deterministic RACE-1 through RACE-5 Worker tests now prove unique smallest-suffix allocation, archived titles participating in the reservation, owner scoping, and exact replay of each allocated result. Focused/full Worker and Web suites and static gates pass; no Worker/API/schema/migration/dependency change was introduced.
