...
D-094 reduces UI consistency risk by sharing outside-click / Escape cleanup, strict logical-date input, destructive-action styling, fixed status placement, and Routine viewport scroll ownership across the affected Web surfaces. Focused and full Web tests, typecheck/build/static gates, exact nonprod deploy, and authenticated existing/fresh-tab UI checks pass. The authenticated CUA viewport was approximately `592 × 884` with sidebar open; broad desktop-width visual coverage and exact console warning/error counts were not measurable through this surface, so those remain `NOT_VERIFIED` rather than being inferred from automated tests. No API/Worker/schema/migration/dependency change was introduced.
