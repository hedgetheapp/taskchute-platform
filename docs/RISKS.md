...
Mitigation/evidence: focused Task Primary Worker `5 / 5`, Documents `16 / 16`, focused App + TaskNoteEditor `263 / 263`, full Web `7 files / 356`, full Worker/D1 `33 files / 292`, migration fresh/upgrade, typecheck/build/exact nonprod build/deploy guard/Wrangler dry-run/diff-check all pass. Persistent nonprod Worker `0ea63a97-c0af-4d6f-be76-4265940e0f21` returned root `200` and unauthenticated Documents API `401`; APP/AUTH pending is `0 / 0`, quick checks are `ok`, FK checks empty, and read-only probes report `rows_written=0`. An existing authenticated non-sensitive Task was used only for Task Primary verification; same-tab reload and a fresh authenticated tab restored the canonical saved body, with empty console warning/error logs. No body/title content is recorded here.
