...
The Android drag presentation keeps the lifted source Entry as the same stable-keyed item in the provisional list, moves that real Entry to the resolved target, and renders that keyed row as a separate teal placeholder visual while the lifted visual follows the pointer once. The gesture-owning row remains composed for the full pointer sequence; no synthetic Task identity replaces the source and no second pointer translation is applied to the list row. Only eligible planned ordinary rows are relative anchors. An empty or Completed-only Section resolves to the existing Section-only `MoveEntry` with omitted placement so canonical planned-tail semantics place the Entry without reordering historical rows. Visible empty Section bands are captured as stable drag targets; off-screen auto-scroll is not part of this corrective.
