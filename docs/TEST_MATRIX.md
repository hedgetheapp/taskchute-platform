...
| D101-RESOLVE | Shared Document route | standalone active/archived and Task Primary resolve by exact owner-scoped ID; missing/cross-owner IDs remain unavailable with no fallback; Task bootstrap replaces to shared URL | App route + Documents/Task Primary integration `23 / 23`; NotesBoard exact-route tests | PASS |
...
| D101-REG | Existing behavior | Day projection, migration consumer, Notes, Routine, Settings, operation and lifecycle regressions | full Worker/D1 `33 files / 294 tests`; full Web `9 files / 372 tests` | PASS |
...
| D101-NONPROD | Exact deployment and unauthenticated safety | canonical Worker/bindings/vars, root `200`, protected Documents API `401`, APP/AUTH pending `0 / 0` | Worker `a00c5342-a1a4-4734-b72e-040adab43ded`; generated config/guard; HTTP probes | PASS |
| D101-BROWSER | Authenticated Task Note and standalone permalink persistence | Task Primary shared URL/copy/reload/fresh-tab; standalone active and archived exact URL, missing no-fallback, restore, copy, same-tab reload/fresh-tab; empty console warnings/errors | real CUA interaction on existing authenticated session; no synthetic DOM events; main console `[]` | PASS |
