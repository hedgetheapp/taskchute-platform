...
| D138-RUNTIME | Runtime / device | focused AVD, authenticated Web, persistent nonprod targeted QA, Galaxy S23 | persistent nonprod deploy Worker `f7b3d630-b42c-412d-ba66-56523ba65711` PASS; root `200`; protected API `401`; APP/AUTH pending migrations `0 / 0`; APP/AUTH quick_check `ok`; APP/AUTH FK checks empty; read-only probes `rows_written=0`; authenticated Web / disposable QA and focused AVD not run; Galaxy S23 representative smoke PASS / USER_CONFIRMED (current D-138 APK, not an exhaustive device suite); no existing data mutation | PASS / NOT_RUN |
...
| D137-ANDROID | Android Daily / shared footer / date-header parity | Task / Notes / Daily / Settings navigation、Today-equivalent date header、Daily date navigation、独立タイトルなし、Today共有Material 3 DatePickerDialog、公式Material Symbols Rounded header icons、body editor | date-header corrective `083b16454ae714e4ed96deb441e36c08ba34ab3c`; focused date-label unit `1 / 1`、Android JVM compile / instrumentation compile / assemble / diff-check PASS; exact-SHA CI `35958642230` Android JVM / signed APK PASS、Web/Worker SKIP; artifact `taskchute-android-debug-083b16454ae714e4ed96deb441e36c08ba34ab3c` / ID `10791685990`; Daily-specific runtime NOT_RUN; prior Today picker cancel AVD `1 / 1` PASS and Notes AVD `14 / 15` partial evidence retained | PASS / RUNTIME_NOT_RUN |
...
| D135-JVM | Pure Markdown commands / transformation | Bold、Heading、Bullet、Checkbox、Quote、Link、selection/caret、source/display offset mapping、malformed fallback、task checkbox exact toggle / malformed guard | MarkdownLiveEditorTest 14 / 14 PASS。最終Android JVM 182 / 182、failures/errors/skipped 0 / 0 / 0 | PASS |
| D135-AVD | Notes runtime | Standalone body、共通toolbar、Task Primary editor、autosave surface、rendered Checkbox accessibility action / marker tap | TaskChute_API33 Notes 13 / 13 PASS。marker実タップを含む。最終debug APK install、MainActivity起動、UI tree、target crash buffer emptyもPASS。AVDは実IMEの日本語composition品質を保証しない | PASS / DEVICE-SPECIFIC IME CAVEAT |
...
| D135-CI | Exact implementation SHA | classifier、Android JVM、signed nonprod APK、instrumentation APK compile / signing / upload | implementation SHA 3d9781bd175ea35b95438597e80294c9a8a69ed8、run 35835388710 PASS。Web / Worker SKIP。artifact taskchute-android-debug-3d9781bd175ea35b95438597e80294c9a8a69ed8、ID 10739460429、expires 2026-09-30T08:09:39Z | PASS |
| D135-BOUNDARY | Product / persistence / release | Existing Notes semantics and platform boundaries unchanged; caret / marker corrective is Android presentation plus raw-source update only | NotesController autosave/CAS/conflict/ambiguous retry/safe flushは未変更。Worker/API、Web、schema、migration、offline persistence、dependency、Productionは変更・実施なし。Galaxy S23 NOT_RUN / PRODUCT_OWNER_MANUAL、Production NOT_RUN、Released NO | PASS / NOT_REQUIRED / NOT_RUN / NO |
...
| D133-UI | Android Settings UI | Create / Edit共通full form、14 recurrence familyのlocal Schedule Sheet、invalid / incomplete draftのApply disabled、DatePicker、Add / Saveのsingle write | `RoutineScheduleValidationTest` `2 / 2 PASS`、`SettingsControllerTest` `13 / 13 PASS`、invalid interval / blank・malformed・duplicate weekday / invalid ordinalのApply disabledとvalid draftのenabledをfocused Compose testへ追加。`:app:assembleDebug` / `:app:compileDebugAndroidTestKotlin` / `git diff --check` PASS。AVD instrumentationは`adb devices`空・`emulator`コマンド未提供でNOT_RUN | PASS / RUNTIME_NOT_RUN |
| D133-CI | Exact implementation SHA | classifier、Android JVM / signed APK / instrumentation APK compile、Web / Worker / D1 / typecheck / build | corrective SHA `c7cced74e1fda8a718a76801f85ceb6bb3ef1886`、run `35735293187`（rerun PASS）。classifier・Android job PASS、Web / Worker SKIP。APK `taskchute-android-debug-c7cced74e1fda8a718a76801f85ceb6bb3ef1886`、ID `10697646632`、expiry `2026-09-29T13:48:07Z` | PASS |
...
| D121-TASKROW-DEVICE | Galaxy S23 representative visual smoke | Product Owner reviewed exact APK `b4f3925e020a1f358bc213f50d92910388201a52` / artifact `taskchute-android-debug-b4f3925e020a1f358bc213f50d92910388201a52`; 84dp Task Row、left projected start/end slot、Planned / Running / Completed visual treatment、Project / Mode context、selection checkbox、trailing action/status balanceを確認し「概ね問題なし」とした。pixel-perfect screenshot comparisonや全interaction stateのdevice verificationには昇格しない。 | PASS / USER_CONFIRMED |
| D121-TASKROW-BOUNDARY | Product / persistence boundary | Today Android row/model/parser/resources/tests only; swipe, selection, D&D, Future/Past, planning/execution semantics preserved; no Worker/API, schema/migration, dependency, realtime, persistent nonprod, production, or Release change; Galaxy S23 representative visual smoke is `PASS / USER_CONFIRMED`, while authenticated emulator screenshot comparison remains `SCREENSHOT_NOT_RUN`. | PASS / NOT_REQUIRED / NOT_RUN / NO |
...
| D126-DEVICE | AVD / Galaxy S23 final-main smoke | TaskChute_API33 Today 40/40 PASS; instrumentation 593.36s, post-test install/smoke 1.79s, total 595.39s; MainActivity resolved and crash buffer empty. Product Owner confirmed latest stable-signed APK updated in place without uninstall, opened Future Day, created via Quick Add, edited, verified Right → Left actions, Left → Right Selection, absence of future Start / Complete / RunningTaskPanel, current execution regression, and past read-only behavior as 問題なし. | PASS / USER_CONFIRMED |
...
| D122-DEVICE | Galaxy S23 signing transition / updateability | Initial legacy-signer transition: PASS / USER_CONFIRMED. Subsequent same-key in-place update: Product Owner updated from the D-122 stable-signed APK to artifact taskchute-android-debug-91d1802b5b1f76a000a44414c8b432b53973cbe1 without uninstall, retained login state / existing app state, and completed the D-126 runtime smoke successfully. | PASS / USER_CONFIRMED |
...
| D121-HEADER-GLYPH-DEVICE | Galaxy S23 latest APK smoke | Product Owner installed/checked the latest debug APK and reported no issue with the Today header corrective | PASS |
| D121-HEADER-GLYPH-BOUNDARY | Product/persistence boundary | Only Today header glyph references and three local vector resources changed; no Worker/API, schema/migration, Gradle dependency, realtime, persistent nonprod, production, or Release change | PASS / NOT_REQUIRED / NOT_RUN / NO |
...
| D121-FINAL-VISUAL | Final Figma visual parity evidence | Implementation `9922f893dc5b62cb44c45444913f5ea6a138cc2f`; fresh emulator captures for Today normal/selected/running/completed/swipe/actions, Notes list/editor, Settings home/Sections compared with Figma reference frames; old purple-heavy/default glyph/oversized hierarchy corrected | TESTED / PENDING_DEVICE |
| D121-FINAL-AVD | Final Windows local runtime gate | `TaskChute_API33` All `35 / 35`（Today `25` + Notes `7` + Settings `2` + Security `1`）、instrumentation `525.05s` / total `526.11s`、Debug APK install、MainActivity、crash buffer empty | PASS |
| D121-FINAL-CI | Exact final implementation SHA | run `35356241475` / run 108; Classifier PASS、Android JVM and APK verification PASS、Web and Worker verification SKIPPED; artifact `taskchute-android-debug-9922f893dc5b62cb44c45444913f5ea6a138cc2f`, ID `10551753719`, expiry `2026-09-25T14:28:39Z` | PASS |
| D121-FINAL-BOUNDARY | Product / persistence boundary | Product/Domain/API、schema/migration、dependency、realtime protocol、persistent nonprod、productionは変更なし。Galaxy S23 fresh final-main smokeは`PENDING_SMOKE`、production `NOT_RUN`、Released `NO` | PASS / NOT_REQUIRED / NOT_RUN / NO |
...
| D120-WEB-FOCUSED | Multi-select canonical block order, same-Section reorder, cross-Section exact placement, empty Section / `Sectionなし`, unselected drag behavior | `apps/web/test/web/App.test.tsx` D-120 focused cases; App file `293 / 293` PASS, including the existing `2 / 2` targeted empty/cross-Section cases and corrective continuous-focus case `1 / 1` | PASS |
...
| D120-REG | Full impacted Web / Worker regression | full Web `14 files / 474 tests`; full Worker/D1 `37 files / 339 tests` | PASS |
| D120-NONPROD | Persistent authenticated browser / DB evidence | Corrective Worker `df9a37c4-7928-47a3-9bb6-11fa940f4d2b` from exact main `b601ac204d5079cccf6516fefe90f7a9aadb46c7`; current `2026-09-17` ordinary TaskのShift下2回・上2回をmouseなしで実行し、各回で順序の即時反映と対象row focusを確認。複数選択blockでも同Section移動とSection境界移動を実行し、選択2件・先頭Task focus・即時Day移動を維持、reload後にcanonical配置へ収束。検証fixtureは元のSectionへ復元。console `0 / 0`; APP quick/FK/guard/assertion/active-execution checks PASS、read-only probes `rows_written=0` | PASS with pointer-D&D browser replay NOT_CAPTURED |
| D120-CI | Exact final main verification | prior corrective exact SHA `b601ac204d5079cccf6516fefe90f7a9aadb46c7`; run `35237010582`; corrective2 exact SHA `f387612adf99dfc467ffedf4350b39a7bd77e735`; run `35285756109`; classifier / Web/Worker PASS、Android JVM/APK job SKIPPED by impact classifier | PASS |
| D120-CORR2-FOCUS | Focus authority | Move / Reorder delayed focus restore cannot override a later explicit focus intent; stale RAF restore is ignored by key + generation | focused App regressions `3 / 3` PASS; persistent browser explicit focus navigation remained on the later Task after save/reconcile | PASS |
| D120-CORR2-QUEUE | Continuous Shift placement | two selected ordinary planned Tasks accept Shift+ArrowDown 6回 + Shift+ArrowUp 6回 while placement writes are pending, without an arbitrary pending cap; selection/order/focus converge after reload | focused App stress regression `1 / 1`; persistent nonprod browser current Day `2026-09-18` accepted the continuous sequence and reloaded to canonical Section配置 | PASS |
| D120-CORR2-NONPROD | Exact deployment / browser | corrective2 exact main deployed to canonical persistent nonprod; no Worker/API/schema change; browser focus/selection/order evidence and console | Worker version `c2f3a938-d7a7-4e3b-a40f-cfd4974b1d10`; console warning/error `0 / 0`; read-only post-deploy browser evidence | PASS |
...
D-120 browserではkeyboard direct-placement/reorderとRoutine occurrence-onlyのreload convergenceを確認した。
correctiveでは、連続Shift入力後もsingle / multi-selectionの対象row focusと即時optimistic orderを維持し、
Section境界をまたぐblock移動後も選択状態を保持することを確認した。座標pointer dragのcommit結果だけは
現CUAで取得できなかったため、automated pointer/empty-target coverageとは同一のbrowser evidenceとして扱わない。
...
| D117-NONPROD | Persistent nonprod / authenticated browser | Exact corrected `main` deployed to `taskchute-web-nonprod`; Worker `93af048b-2e3b-4052-b319-f07c3611b280`; `RUNTIME_ENV=nonprod`, `BOOTSTRAP_ENABLED=false`, canonical APP/AUTH/RealtimeHub bindings; root `200`; unauthenticated Documents and Realtime API `401`. Existing authenticated tab created/started/completed disposable `D117 running metadata QA`; running settle enabled Project/Mode controls, existing metadata changes were accepted, and reload converged to canonical `Life` / `D105 Browser Mode（アーカイブ）`; console warning/error `0 / 0`. Fixture retained. | PASS |
| D117-DB | Persistent DB integrity | APP/AUTH remote `quick_check=ok`, FK empty, pending `0 / 0`; read-only probes `rows_written=0`; no migration or APP/AUTH data write in this corrective | PASS / NOT_REQUIRED |
| D117-CI | Exact implementation SHA | GitHub Actions run `35174027860`, same-SHA failed-Web-job rerun PASS: Classifier, Web/Worker, Android JVM/APK all PASS; Android artifact `taskchute-android-debug-c0ba6066ebd00c45c6761d953daa8f1c37d93bdb`, ID `10478121485`, expires `2026-09-24T02:22:56Z` | PASS |
...
| D116B-BROWSER | Authenticated conversion behavior | Existing same-user session; synthetic completed source, Task live Project `NULL` vs Entry Project snapshot, historical Mode copy, single create/correlation, source remains completed/ordinary, reload converges; on `2026-09-17`, next-day materialization remained exactly one across reload; console warnings/errors `0 / 0`; fixture retained | PASS for conversion/reload + next-day materialization |
| D116B-MATERIALIZE | Future Routine materialization | Worker/D1 integration test verifies first future current-Day load creates one and repeat load remains one; persistent nonprod on `2026-09-17`: occurrence UI `1 → reload → 1`, D1 count `1`, source completed ordinary Entry unchanged with `routine_occurrence_id=NULL`, Execution count `1`, read-only probe `rows_written=0` | PASS automated + persistent browser |
| D116B-BOUNDARY | Scope | No AUTH migration, production, restore, dependency, or release operation | PASS / NOT_RUN / NO |

Wrangler emitted a local log-directory `EPERM` diagnostic and the existing Vite chunk-size warning; successful commands exited `0`. Exact-SHA CI, APP-only migration, isolated backup/recovery validation, canonical nonprod deploy, API safety, DB integrity, and authenticated conversion/reload browser checks passed. Persistent browser verification of next-day Routine materialization passed on `2026-09-17` with occurrence count remaining exactly one across reload. Production and restore remain `NOT_RUN`; Released remains `NO`.
...
| D115-TASK-NOTE | Task Note toggle | Toggles only the corresponding Task floating Note; close uses safe flush and unresolved-operation barrier. Corrective `b8e2878632dc4a7a37dee0cd03cfcf044fb4e07c` avoids the fixed floating-window hit-target obstruction over the horizontally-scrolled Note column. Authenticated browser verified first-time Create→open→same-row close, existing Document open→same-row close, corresponding-window-only close with another window retained, and header `×` close; console warning/error `0 / 0` | PASS Web tests + authenticated browser |
| D115-PROJECT-NOTE | Project Note editor ownership | Safe flush/transfer from Notes inline to the existing floating editor; no competing writer and no standalone floating Document type. Authenticated browser verified inline→floating transfer of the same Project Primary Document, inline placeholder/no competing editor, autosave, and reload/reopen persistence | PASS Web tests + authenticated browser |
...
| D115-NONPROD | Persistent nonprod | Corrective main `b8e2878632dc4a7a37dee0cd03cfcf044fb4e07c` deployed to Worker `taskchute-web-nonprod`, version `3c574852-4a89-48b0-a2ee-ffc60aa6382e`; root 200; unauthenticated Documents API 401; no migration | PASS |
| D115-DB | APP/AUTH integrity | Both quick_check `ok`, FK checks empty, read-only probes `rows_written=0` | PASS |
| D115-BROWSER | Authenticated Notes browser | Existing authorized tab: standalone create/focus/autosave and reload/reopen restored canonical synthetic Note; Task Note first-time and existing-document same-row toggles close the exact window, unrelated window remains, header `×` works; Project Primary Note inline→floating keeps one writer and reload/reopen preserves synthetic content; console warning/error `0 / 0` | PASS |
...
| D104-REAUTH | Session expiry | Authenticated 401 preserves mounted editor/window state and dirty draft in memory, freezes mutation, requires reauth, and distinguishes canonical conflict | focused App/NotesBoard/TaskNoteEditor tests; standalone, Project inline, Task floating, Project floating auth paths; wrong-principal resume regression | PASS local |
...
| D104-CI | Push-time CI | Read-only CI runs on main push/manual dispatch with pinned supported Node and no deploy/migration | `.github/workflows/ci.yml`; run `34800601555`, exact SHA `cd8a4882cf0534e278ee948ce57a98571de831c4`, all steps success | PASS |
...
| D104-LOCAL | Local gates | Focused and full tests, typecheck, builds, deploy guard, dry-run and diff check | focused Web `5 files / 328 tests`; full Web `13 files / 438`; full Worker/D1 `34 files / 301`; static gates PASS; preflight helper hit Windows FETCH_HEAD EPERM | PASS except helper environment error |
| D104-NONPROD | Persistent nonprod | Exact main deployed with canonical Worker/config and unauthenticated safety | Worker `8d413923-1f2f-444c-b760-b4b4a06ac134`; root `200`; Documents API `401`; APP/AUTH pending `0 / 0` | PASS |
...
Contract: D-065 final verification後のAPP / AUTH state reconciliation。read-only query、既存backupのreadability / isolated recovery確認、canonical docs maintenanceのみを許可し、APP / AUTH write、new backup、restore、cleanup、migration、deploy、runtime変更、production操作は行わない。

| ID | Verification target | Evidence |
| --- | --- | --- |
| PROJECT-RECONCILE-01 | Snapshot A canonical baseline | PASS: Projects / project board items / archives `3 / 3 / 0`、Board revision `16`、operations `22`、order `B → A-renamed → C`。A / B / C IDsを記録 |
| PROJECT-RECONCILE-02 | Snapshot A→Bのoperation delta | PASS: retained operation historyのseq23〜29に7件を特定し、CreateProject `2` + DeleteProject `5`で完全一致 |
| PROJECT-RECONCILE-03 | 3 Project disappearance explanation | PASS: A / B / Cはそれぞれsuccessful DeleteProject seq25 / seq24 / seq26で説明。全件unassigned_task_count `0`、actor / purposeの推測なし |
| PROJECT-RECONCILE-04 | APP integrity and non-Project preservation | PASS: Snapshot B `0 / 0 / 0` Projects / board items / archives、operations `29`。isolated recovery `quick_check=ok` / FK `0`。closeout current APPも`quick_check=ok` / FK `0` / orphan Project refs `0` / rows_written `0` |
| PROJECT-RECONCILE-05 | AUTH preservation and write boundary | PASS: AUTH `quick_check=ok` / FK `0`、users / accounts / sessions `1 / 1 / 5`、rows_written `0`、login capability保持。APP / AUTH migration pending `0 / 0` |

Snapshot Bのexisting APP backupは`apps/web/.wrangler/private-backups/d065-final-pre-delete-app-20260906.sql`、`52,578 bytes`、SHA-256 `095E382FCE688F911F4FADF0DA2F5C468A83585BC1A191DC36E357C387AD5A90`、readable / ignored。isolated SQLite importはlatest migration `0019_project_management.sql`、`quick_check=ok`、FK `0`だった。Snapshot B後の別verification fixtureにより、closeout current APPはProjects / board items / archives `1 / 1 / 0`、Tasks / Entries / Executions / RoutineDefinitions / RoutineOccurrences `3 / 2 / 0 / 1 / 0`、operations `34`、Board revision `26`であり、Snapshot Bとは区別する。

| seq | operation_id | command / result |
| --- | --- | --- |
| 23 | `01a07477-5cb5-7691-a24d-985526c75154` | `CreateProject` temporary `01a07477-5cb5-768f-b2be-3836caec7398`、title `てst` |
| 24 | `01a07477-976c-70a6-8e11-c0ad5fb2edd4` | `DeleteProject` B `01a0745e-3cee-7551-b240-4a803fbaaa14`、Board revision `18`、unassigned `0` |
| 25 | `01a07477-a4be-7554-93d5-367980982a60` | `DeleteProject` A `01a0745e-14f5-7f8a-8b25-202c0fb117e8`、Board revision `19`、unassigned `0` |
| 26 | `01a07477-b085-7b58-8e03-11202a232e81` | `DeleteProject` C `01a0745e-590e-7104-b79f-c653946f1b13`、Board revision `20`、unassigned `0` |
| 27 | `01a07477-bd04-7e5c-bce9-e2b2755d3ad5` | `DeleteProject` temporary `01a07477-5cb5-768f-b2be-3836caec7398`、Board revision `21`、unassigned `0` |
| 28 | `01a07477-cd48-7f43-8d82-3d257f283c8e` | `CreateProject` temporary `01a07477-cd48-76e5-b8fe-97075fc5e9eb`、title `てst` |
| 29 | `01a07477-d6fe-7f05-80bf-fd419101761f` | `DeleteProject` temporary `01a07477-cd48-76e5-b8fe-97075fc5e9eb`、Board revision `23`、unassigned `0` |

Conclusion: Snapshot A→BのProject消失とoperation増分は retained canonical operation historyで完全にaccountされ、direct DB mutationを示す証拠はない。全reconciliation queryはread-onlyで`rows_written=0`。historical baselineの重複IDは`PROJECT-CORRECTIVE-BASELINE-05`へ改名し、latest correctiveの`PROJECT-CORRECTIVE-05`は維持した。
...
| PROJECT-CORRECTIVE-BASELINE-05 | persistent nonprod corrective deploy / authenticated mutation / focus / console | NOT_RUN (Wrangler deploy network approval rejected before deployment) |
...
Product Ownerの明示承認に基づくnonprod APP cleanupの事後reconciliation。前turn中断中にcleanup commandの一部が完了していたため、このresume runでは追加deleteを実行していない。

| ID | Verification target | Evidence |
| --- | --- | --- |
| PROJECT-CLEANUP-01 | cleanupで削除されたProject identityとoperationを特定し、Project / Board itemの差分を確定する | PASS (NONPROD_D1 read-only): cleanup新規分はProject `2` / `project_board_items` `2`; operation IDs `01a071a5-a5a2-7ced-9454-66fd103d9e86`, `01a071a5-affa-7823-96bb-37ee455eb47a` |
| PROJECT-CLEANUP-02 | Task / Entry / Execution / Routine / snapshot / Day / Sectionを誤って削除していない | PASS (NONPROD_D1): Tasks `55`、Entries `62`、Executions `25`、RoutineDefinitions `11`、RoutineOccurrences `20`、routine snapshots `20`、entry snapshots `25`、Sections `3`、Days `10`、contexts `30` |
| PROJECT-CLEANUP-03 | APP post-cleanup integrity、Project reference、operation retention | PASS (NONPROD_D1): `quick_check = ok`、FK `0`、live Task Project assignment `0`、orphan ref `0`、historical Project refs Routine `2` / Entry `1`、operations `309` |
| PROJECT-CLEANUP-04 | fresh private APP backup、readability、isolated recovery | PASS (POST_CLEANUP_ONLY): `d065-cleanup-pre-app-20260905.sql`、`313,540 bytes`、SHA-256 `0E1B8FE0278671609E42A1D788B0E584FBEF15AC7E4CC7CFC49183F67152A936`、`.wrangler/` ignored、isolated `quick_check = ok` / FK `0` |
| PROJECT-CLEANUP-05 | cleanup前fresh backup HARD GATE | NOT_ATTESTED: cleanup effects were already present before the resumed run's fresh export; no additional destructive command was issued after that export |
| PROJECT-CLEANUP-06 | AUTH preservation and login data | PASS (NONPROD_D1 read-only): AUTH untouched、pending `0`、`quick_check = ok`、FK `0`、users / accounts / sessions `1 / 1 / 4` |

Deleted scope recorded exactly: prior D-065 disposable Project `01a07197-986d-7297-91d0-a2d912d82c61` was deleted by operation `01a0719b-081a-732d-8947-3774a64c98ce`; the cleanup follow-up removed existing development Projects `01a051d9-ead5-7ab2-b4ab-1059f3876bca` and `01a0293c-0a4f-79ce-97b3-f3b1cc2beb71` through the two operations above, plus their two Project Board items. No Project archive / guard, Task, Entry, Execution, Routine, snapshot, Day, or Section rows were deleted. Project Board head `1` remained; operation history was retained and increased from `307` to `309` by the two DeleteProject audit rows.

The fresh export was taken after the cleanup effects were already present; it is therefore a post-cleanup backup and not evidence that the pre-cleanup HARD GATE was satisfied. The older `d065-pre-0019-app.sql` isolated recovery showed `projects 2 / tasks 55 / entries 62 / executions 25 / operations 302`, but it predates APP `0019` and is not claimed as a cleanup-immediate backup. No remote restore was performed. production, AUTH cleanup, binding / permission / OAuth scope changes were not performed.
...
- authenticated browserではactual Start inputのvalue / displayed value `0900`、Start編集中のEnd button、End inputの`0915`、Task width`280px → 290px`とreload / new-tab後の`290px`保持を確認した。neutral surfaceの`↓ / ↑ / J / K`、`S / N / E / D / ? / Esc`、input中S抑制、calendarの前後月・前年翌年、outside click close、`--:-- / --分`を確認した。IME composition中のshortcut suppressionはlocal Web regressionで確認し、real OS Japanese IMEは`NOT_RUN`とした。Sで開始したordinary fixtureは同じrunで完了へ戻し、active executionを残していない。Dは削除確認までで、最終削除は行っていない。
...
| BULK-05 | Ordinary delete | planned ordinary EntryだけをDayからremoveし、Task identity / Project / other Day / Execution factsを保持する | Approved (D-051) | PASS (LOCAL_AUTOMATED + PERSISTENT_NONPROD_E2E) |
| BULK-06 | Routine skip | Routine-derived Entry / RoutineDefinitionを保持し、Occurrenceへ当日だけの`reason = 'skip'`を保存してreload / ensureで再生成しない | Approved (D-051, D-040) | PASS (LOCAL_AUTOMATED + PERSISTENT_NONPROD_E2E) |
| BULK-07 | Atomicity / retry | mixed batchのall-or-nothing、stale / owner / Day / lifecycle / execution guard、duplicate ID reject、same-operation replay、injected rollbackを満たす | Approved (D-051, D-020) | PASS (LOCAL_AUTOMATED + PERSISTENT_NONPROD_E2E retry) |
...
- authenticated nonprod browserでordinary / Routine-derived mixed selection、処理説明、final `削除`、canonical reconciliation、selection clear、reload persistence、Routine Board、Columns / Sidebar / Section collapse、console warnings / errors `0 / 0`を確認した。一次mutationはDay `01a0557e-ae83-7596-bf8d-96f3de2bc3cc`のrevision `18 -> 19`で、ordinary Task `01a0557e-ae83-7349-9cb8-247be7580dab` / Entry `01a0557e-ae83-7f83-af32-5e4b80cc28b2`はEntryのみ削除、Routine Definition `01a06296-bc0f-7601-a272-4d120cae448e` / Occurrence `01a06424-9566-7d27-b257-b757cf95d871` / Entry `01a06424-9566-7c26-a3f2-26ab77614782`は保持、`skip` suppression exactly `1`となった
- 一次operation `01a06600-254f-7a0d-ba7b-8ab71a89d063`は`BulkDeleteEntries / success`の1 rowを記録した。同じconfirmationを二重送信したretry probeでは、通常UI作成のordinary Entry `01a06603-f1e7-7661-b10c-3d9cc22d1749` / Routine Entry `01a06604-0bd3-7861-bfa2-627eb308a858`に対し、Day revision `21 -> 22`、retry operation `01a06605-2ae5-732a-bddb-cdec748bde65`のoperation row `1`、suppression row `1`でsecond effectなしを確認した。Task / RoutineDefinition / Occurrence identity保持、reload後不再現、Routine Board `ON` / `daily` / open pause `0`、APP `quick_check = ok`、FK `0`、active Execution `0`もPASS。production feature verification `NOT_RUN`、Released `NO`

## Bulk Selection v0.2A — Section change

| ID | Area | Requirement | Contract | Evidence |
|---|---|---|---|---|
| BULK-SECTION-01 | Eligibility | established current Dayのordinary planned Entryだけを対象とし、Routine-derived / running / completed / historical / preview / locked stateをrejectまたはno-writeとする | Approved (D-052) | PASS (LOCAL_AUTOMATED + WEB + REAL_LOCAL_BROWSER + NONPROD_BROWSER) |
| BULK-SECTION-02 | Command / D-043 | 一つの`BulkMoveEntriesToSection` commandをowner-scopedに実行し、real Sectionはserver-derived logical start、SectionなしはSection / startともNULLへ同期する | Approved (D-052, D-043, D-020) | PASS (LOCAL_AUTOMATED + NONPROD_BROWSER + D1_OPERATION_LOG) |
| BULK-SECTION-03 | Ordering / revision | moversをcurrent Day display orderのままtarget末尾へappendし、変更時だけplacement revision exactly `+1`、same-Section start-only、canonical no-opではposition / revisionを不必要に変更しない | Approved (D-052) | PASS (LOCAL_AUTOMATED + NONPROD_BROWSER + D1_READ_ONLY) |
| BULK-SECTION-04 | Atomicity / retry | owner / Day / lifecycle / snapshot / stale revision guard、all-or-nothing rollback、same-operation replay / misuse rejectionを満たす | Approved (D-052, D-020) | PASS (LOCAL_AUTOMATED); NONPROD double-click produced one operation row |
| BULK-SECTION-05 | UI / selection | `Section変更` picker、Section order / Sectionなし、Routine混在disabled hint、selection retention、reload / navigation / auth clearを満たす | Approved (D-052, D-051) | PASS (WEB + REAL_LOCAL_BROWSER + NONPROD_BROWSER) |
| BULK-SECTION-06 | Migration | `0011_bulk_section_change.sql`はoperations command_type CHECKだけを拡張し、existing operations / schema / constraintsを保持する | Approved (D-052) | PASS (MIGRATION_REGRESSION + NONPROD_MIGRATION + D1_SCHEMA) |
| BULK-SECTION-07 | Persistent safety | nonprod APP migration / deploy / bootstrap boundary / D1 integrity / authenticated mutationを確認する | Approved (D-024, D-052) | PASS (NONPROD_MIGRATION + NONPROD_DEPLOY + NONPROD_BROWSER + D1_INTEGRITY + POST_RECOVERY_BACKUP); pre-migration full DB backup `NOT_RUN` |

Bulk Selection v0.2A integrated evidence（2026-09-03）:

- D-052 canonical Decision commit `0398477f8f149a32af940e3d0aca0cfce5b1bc41`、runtime commit `1a21c313d131d0a8e3673a9c4b68bce3282c30ff`をGitHub `main`へpush済み。migration `apps/web/migrations/app/0011_bulk_section_change.sql`、shared request/result、API route、Worker application、Web toolbar/picker、focused tests / migration harnessを含む
- local focused Bulk Section Worker `5 / 5 PASS`、Worker全体 `159 / 159 PASS`、Web全体 `140 / 140 PASS`、migration regression `4 scenarios PASS`、typecheck、production build、`git diff --check`、source review `PASS`。Wrangler user-log `EPERM` warningはあったが各commandのexit codeは`0`
- real-localではAPP `0011` migration、異なるSectionのordinary planned TaskのSection変更、server-derived start、display order、selection retention、reload persistence / selection clearを確認した
- persistent nonprod migration前のAPP baselineはoperations `197` row、APP `0011` pendingのみ、AUTH pendingなし、APP migration後operations `197` rowを保持した。migration後のAPP / AUTH pendingは`0 / 0`、APP `quick_check = ok`、FK violations `0`、read-only query `rows_written = 0`、operations schemaは`BulkMoveEntriesToSection` CHECK追加のみである。migration前full DB backupは取得できなかったが、post-migration private ignored recovery backupをAPP `209,818 bytes` / SHA-256 `52A05664C6C28631BCA22F8AC02878F0447E22F93775B4371A5D5335D3D22F51`、AUTH `3,862 bytes` / SHA-256 `30475F539BE52CD1C80EDD5956E8FDCADD03441EF392BE3942FCFB75625F468D`で取得し、SQL構造・終端・readabilityを確認した。restoreは行っていない
- exact `main@1a21c313d131d0a8e3673a9c4b68bce3282c30ff`をnonprod configでbuildし、Worker `taskchute-web-nonprod` / version `68c4964f-a73d-4e8c-a3d3-3ce5364d2bda`、`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、canonical AUTH / APP bindingを確認。root `200`、protected API `401`、bootstrap POST `404`
- authenticated nonprod browserでTask `Bulk Section v02A nonprod A/B/C 20260903`を作成し、Sectionなし / Morning / DayからEveningへ一つのcommandで移動。target start `20:00`、stable append positions `3 / 4 / 5`、3件selection retention、reload後のEvening persistence / selection clearを確認した。AのSectionなし移動はrevision `26 -> 27`、Section / planned startのNULL同期を確認し、B/Cのsynchronized no-opは`changed_entry_ids=[]`、revision `27`不変だった
- nonprod Day `01a0557e-ae83-7596-bf8d-96f3de2bc3cc`のfirst bulk resultはrevision `26`（Task追加直後`25`から`+1`）、Sectionなし resultは`27`（`26`から`+1`）、no-op resultは`27`。operationsはmigration直後`197`、Task追加後`200`、Bulk Section success `3` row後`203`で、first double-click operationも一つのlog rowに留まった。local automatedではexact same-operation replay、operation misuse、injected rollback / retry、Routine / lifecycle / owner / stale guardsをPASSした
- verification-only test dataはcleanupせず残置。Routine planned rowsはnonprod DBに存在するが既存skip suppressionによりcurrent projection非表示のため、Routine stateを変更せず、Routine混在UI blockはlocal Web testでPASS。production feature verification `NOT_RUN`、Released `NO`

## Bulk Selection v0.2B1 — Routine-inclusive Section occurrence change

| ID | Area | Requirement | Contract | Evidence |
|---|---|---|---|---|
| BULK-ROUTINE-SECTION-01 | Eligibility / boundary | established current Dayのplanned ordinary / Routine-derived Entryだけをowner-scopedに受理し、Routine relation / origin Day / suppression / lifecycle / preview / non-current boundaryを守る | Approved (D-053, D-020, D-041, D-042) | PASS (LOCAL_AUTOMATED + REAL_LOCAL + NONPROD_BROWSER) |
| BULK-ROUTINE-SECTION-02 | Command / authority | Routineを含むSection変更をN件の個別commandではなく一つの`BulkMoveEntriesToSectionOccurrence`で処理し、clientはtarget Sectionだけを指定、startはD-043 contextからserver deriveする | Approved (D-053, D-043) | PASS (LOCAL_AUTOMATED + NONPROD_BROWSER + D1_OPERATION_LOG) |
| BULK-ROUTINE-SECTION-03 | `今回だけ` UX | Routine-inclusive selectionでcandidate picker後にexplicit acknowledgementを要求し、Routine settings / defaults / future / other Occurrenceを変更しない。cancel / Escape / dismissはno-write、success後selectionを保持する | Approved (D-053, D-044) | PASS (WEB + REAL_LOCAL_BROWSER + NONPROD_BROWSER) |
| BULK-ROUTINE-SECTION-04 | D-043 synchronization | real Sectionは`section_id + planned_start_minute`、Sectionなしはordinary / Routineとも`NULL + NULL`へ同期する | Approved (D-053, D-043) | PASS (LOCAL_AUTOMATED + REAL_LOCAL + NONPROD_BROWSER) |
| BULK-ROUTINE-SECTION-05 | Routine override | existing R2A typed fieldsでOccurrence overrideをcreate / replaceし、explicit NULL pairもpresent overrideとして保存する | Approved (D-053, D-044, D-046) | PASS (LOCAL_AUTOMATED + REAL_LOCAL + NONPROD_BROWSER) |
| BULK-ROUTINE-SECTION-06 | Override-only / no-op | effective pairが同じでもno overrideならoverride-onlyとしてpersistしDay revisionを変えず、same existing overrideだけtrue no-opとする | Approved (D-053, D-020) | PASS (LOCAL_AUTOMATED + REAL_LOCAL + NONPROD_BROWSER) |
| BULK-ROUTINE-SECTION-07 | Ordering / revision | mixed moverをpre-mutation Day display orderでtarget末尾へappendし、visible change時だけrevision exactly `+1`、override-only / true no-opはrevision不変とする | Approved (D-053) | PASS (LOCAL_AUTOMATED + REAL_LOCAL + NONPROD_BROWSER) |
| BULK-ROUTINE-SECTION-08 | Atomicity / retry | owner / Day / snapshot / stale revision / Routine relation guard、all-or-nothing rollback、same-operation replay / ambiguityを満たす | Approved (D-053, D-020) | PASS (LOCAL_AUTOMATED; persistent operation log has one row per logical command) |
| BULK-ROUTINE-SECTION-09 | Routine authority | RoutineDefinition identity / Section default / planned-start default / defaults_revision、other Occurrence、schedule、Board、suppressionを変更せずfuture propagationを行わない | Approved (D-053, D-043, D-044) | PASS (LOCAL_AUTOMATED + REAL_LOCAL + NONPROD_D1) |
| BULK-ROUTINE-SECTION-10 | Migration | `0012_bulk_routine_section_occurrence.sql`は`operations.command_type` CHECKだけを拡張し、existing operation rows / schema / constraintsを保持してnew commandを受理する | Approved (D-053) | PASS (MIGRATION_REGRESSION + NONPROD_MIGRATION + D1_SCHEMA) |
| BULK-ROUTINE-SECTION-11 | Persistent safety | fresh private backup HARD GATE後にAPP migration / deployを行い、APP/AUTH pending、quick check、FK、bootstrap-disabled boundary、authenticated verificationを確認する | Approved (D-024, D-053) | PASS (NONPROD_BACKUP + NONPROD_MIGRATION + NONPROD_DEPLOY + NONPROD_BROWSER + D1_INTEGRITY) |

Bulk Selection v0.2B1 integrated evidence（2026-09-03）:

- D-053 canonical Decision commit `e398d2b3a5e0a3937fe35169fe1e69162a26b6dd`、runtime implementation commit `70d88e2be8d33ada2e520029f2b322301d30c5d0`をGitHub `main`へpush済み。migration `apps/web/migrations/app/0012_bulk_routine_section_occurrence.sql`、shared request/result、API route、Worker application、Web acknowledgement / selection、focused tests / migration harnessを含む
- local focused Worker `8 / 8 PASS`、Worker全体 `162 / 162 PASS`、focused Web `2 / 2 PASS`、Web全体 `140 / 140 PASS`、migration regression `4 scenarios PASS`（fresh `0001 -> 0012`、existing operation preservation / constraints）、typecheck、production build、`git diff --check`、source review `PASS`。Wrangler user-logへのsandbox `EPERM` warningはあったがrequired commandはexit `0`
- real-localではAPP `0012` migration、mixed ordinary / Routine Section move、confirmation前DB no-write、server-derived `04:00`、stable mixed append、selection retention、reload persistence、Routine Board、Sectionなし explicit NULL override、override-only revision不変、same existing override no-op、narrow table-owned scroll、console warnings / errors `0 / 0`を確認した。non-current direct rejectはWorker integration boundaryでPASS
- persistent nonprod pre-migration APP pendingは`0012_bulk_routine_section_occurrence.sql`のみ、AUTH pendingはなし。APP `212,706 bytes` / SHA-256 `1B34E6955CEDA74BE317E91889BCAF17B2C1F7B265C7AC4729CA7E96FD50154B`、AUTH `3,862 bytes` / SHA-256 `30475F539BE52CD1C80EDD5956E8FDCADD03441EF392BE3942FCFB75625F468D`のfresh private ignored SQL backupを取得し、非空・D1 header・schema marker・終端・readability・ignoreを確認してからHARD GATEをPASSした。restoreは行っていない
- APP `0012`だけを適用し、AUTHは未変更。post-migration pending `0 / 0`、migration前APP operations `203` rowのcount / command distribution保持、`BulkMoveEntriesToSectionOccurrence` CHECK、APP/AUTH `quick_check = ok`、FK `0`を確認した。verification-only dataはcleanupせず残置
- exact `main@70d88e2be8d33ada2e520029f2b322301d30c5d0`をnonprod buildし、Worker `taskchute-web-nonprod` version `24c6e42d-0632-448e-bb6f-4a8956d9c71e`、`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、canonical bindingsを確認。authenticated browserでmixed target Morning revision `33 -> 34` / `04:00` / positions `3 / 4`、Sectionなし revision `34 -> 35` / NULL pair、override-only revision `36`不変 / `changed_entry_ids=[]`、confirmation前no-write、selection retention、reload、Routine BoardをPASS。Task / Entry `3 / 3`、RoutineDefinition / Occurrence `2 / 2`、defaults `NULL / NULL`・revision `0`、schedule / Board item `1 / 1`、suppression `0`、B1 operation `3` rows、console warnings / errors `0 / 0`
- remaining: Bulk `Routineへ反映`、multi-RoutineDefinition default propagation、future / past Routine bulk edit、Bulk date / Project / Mode / Note、cross-Day move、undo / restore、production feature verification `NOT_RUN`。Released `NO`
...
| ID | Area | Requirement | Evidence |
|---|---|---|---|
| D066-WEB-01 | Scheduler | accepted ordinary Day intents are dispatched with at most one in-flight command | PASS (focused/full Web automated) |
| D066-WEB-02 | Add | draft clears immediately, provisional UUIDv7 row is visible, dependent edits wait, and Add failure cancels dependents | PASS (focused/full Web automated) |
...
| D066-WEB-05 | Lifecycle | pending Start + Complete preserves execution identity and does not fabricate actual timestamps or implicit interruption | PASS (focused/full Web automated) |
...
| D066-WEB-08 | Navigation | navigation is deferred while queue drains and reload/close guard does not persist a client queue | PASS (focused/full Web automated; browser representative) |
...
- Implementation / Git: `main@9747366fee0a7e74d70540cd922a9f55df6bbb55`, fast-forward pushed from start `2edcf4408fcd747cf80c11e217091675578f49e2`; pre-existing untracked review artifacts were not staged.
- Local: focused Worker `7 / 7`, focused Web `187 / 187`, full Worker / D1 `191 / 191`, full Web `203 / 203`, typecheck, production build, `git diff --check` all PASS. The direct isolated migration checks passed fresh `0001 -> 0020` and upgrade `0019 -> 0020`, preserving existing operations and rejecting unknown command types; the repository migration helper was tooling-hung and is not the relied-upon evidence.
- D-067 local domain coverage: ordinary completed Entry deletes all attached Executions atomically and preserves Task / Project / unrelated Entry/Execution; same-title unrelated Task is retained; RoutineDefinition, RoutineOccurrence, occurrence snapshot and no-regeneration are retained; planned/running/past/future/owner/active/stale/misuse reject; exact replay, retry identity, revision conflict, injected rollback, and concurrent same-entry winner/loser are covered. Local D1 quick check / FK check pass. This is local D1 integration evidence; signed-in real-local destructive browser mutation was not run.
- Backup HARD GATE: APP `d067-pre-0020-app-20260906.sql` `110,769 bytes` / SHA-256 `47BF1A6F56C61D267FCF89FE2C2D6488CCA31C1F8701AC7BAD5D0959A31E4E91`; AUTH `d067-pre-0020-auth-20260906.sql` `4,704 bytes` / SHA-256 `E70BF2930AB0106607FFA81A0B15B57598F042166DD7F55B2EB3A53ADC0380AA`; non-empty, readable, `.wrangler/` ignored, isolated recovery `quick_check = ok`, FK `0`, restore `NOT_RUN`.
- Persistent nonprod: APP `0020_delete_completed_entry.sql` applied after APP pending `0020` / AUTH `0`; post pending `0 / 0`; APP entries / executions / operations `14 / 13 / 102`, active executions `0`, orphan executions `0`, `quick_check = ok`, FK `0`; AUTH users / sessions `1 / 5`, `quick_check = ok`, FK `0`. Worker `taskchute-web-nonprod` version `cf45a294-84f7-4f09-869f-16cced6764bb`; root `200`, protected API `401`, bootstrap `404`; canonical bindings and nonprod vars unchanged.
- Authenticated browser: current completed row overflow → delete → exact confirmation modal → cancel; row remained and focus returned to overflow trigger. `完全に削除` was never activated. Browser console exact warning/error count was not collected and remains `NOT_VERIFIED`.
- Destructive gate: first persistent nonprod `DeleteCompletedEntry` write is `NOT_RUN_BY_CONTRACT`; Product Owner explicit approval is required before dispatch. Production, restore, branch, PR, merge, tag, and release are `NOT_RUN`.

## D-067 serial dispatcher corrective — 2026-09-06

Contract: Approved D-067 corrective. The existing `DeleteCompletedEntry` migration, Worker command, Domain semantics, and API contract are unchanged; only the Web dispatch/retry orchestration converges on D-066's global serial dispatcher. Start SHA was `d9bb90e5f60d20ad3a3e16866b1ad5053daaebdb`; implementation commit `b2c4d6a29240e689efd8027e7cba3c7e5521510c` was fast-forward pushed to `main`.

| ID | Verification target | Evidence |
|---|---|---|
| D067-SERIAL-01 | Initial completed delete is queued behind an ordinary mutation; max one in flight; latest placement revision is used immediately before first dispatch | PASS (LOCAL_WEB): deferred Add X → confirmed completed delete; delete HTTP stayed `0` until X resolved, then sent with reconciled revision `7` |
| D067-SERIAL-02 | Unsent queued delete is distinct from sent retained operation and has no retry UI | PASS (LOCAL_WEB): queued delete retained client intent only, pending panel / individual retry absent before dispatch |
| D067-SERIAL-03 | Sent identity freezes after dispatch | PASS (LOCAL_WEB): `operation_id`, Day ID, Entry ID, and all semantic request fields remained exact across the sent request |
| D067-SERIAL-04 | Ambiguous delete pauses later ordinary work and retained retry uses dispatcher with exact request | PASS (LOCAL_WEB): later Add HTTP `0` while delete was ambiguous; retry dispatched the byte-equivalent logical request through `enqueueRetainedRetry` |
| D067-SERIAL-05 | Revision conflict pauses queue, reconciles, clears retained delete, and cancels stale queued work | PASS (LOCAL_WEB): queued Add was not sent, delete retry UI was absent, and canonical conflict feedback was shown |
| D067-SERIAL-06 | Navigation / settings / logout / unload barriers include queued and in-flight delete | PASS (SOURCE + existing D-066 regression): queue count / in-flight / active scopes remain part of the existing Day mutation barriers; no bypass path was added |
...
| D070-WEB-01 | Web | future selector、unset `—`、Mode Board order、set / replace / clear、live rename、preview barrier、Project → Mode → Section | focused D-070 Web `3 / 3`、D-068 Mode Web `5 / 5`、full Web `213 / 213` | PASS |
...
| D070-ENV-01 | Persistent nonprod safety | backup HARD GATE, exact nonprod build/dry-run/deploy, root/protected/bootstrap safety, read-only APP/AUTH integrity | Worker `b312bacb-f529-4c2a-9433-2095a5b3cc39` + D1 | PASS |
| D070-ENV-02 | Authenticated browser | future established Day Mode set / replace / clear, reload/fresh-tab, visible unset `—` | CUA authenticated session、2026-09-08、target Entry、fresh tabs、console logs | PASS |
...
- Initial authenticated browser verificationは、existing nonprod credential/passwordを取得・推測・再設定せず、CUA authenticated tabがなかったため未実施だった。これは後続のauthorized existing session closeoutで更新した。

D-070 unset-label corrective / authenticated browser closeout（2026-09-07）:

- Web-only corrective commit `13af655250ecd7718e61591d33f01dc3c2df9285`で、current / future / draft Mode selectorのnull optionを`Modeなし`からcanonical `—`へ変更し、current ordinary planned Entryのvisible label、established future ordinary planned Entryのvisible label、future set / clearを明示検証した。focused D-070 Web `3 / 3`、D-068 Mode Web `5 / 5`、focused Worker Mode `4 / 4`、full Web `213 / 213`、full Worker / D1 `199 / 199`、Day Navigation `15 / 15`、typecheck、production build、exact nonprod build、Wrangler dry-run、`git diff --check`はPASSした。
- migration regressionはschema / migration変更なしのため既存D-070 baseline `4 scenarios PASS`を維持する一方、今回のcorrective後のhelper再実行はWindows Wrangler subprocessが無出力のままtooling-hungとなり中断した。source / full Worker / remote pending `0 / 0`には影響せず、追加のmigrationは作成・適用していない。
- Exact `main@13af655250ecd7718e61591d33f01dc3c2df9285`をnonprod build / dry-runし、Worker `taskchute-web-nonprod` version `b312bacb-f529-4c2a-9433-2095a5b3cc39`へdeployした。generated configはcanonical APP/AUTH binding、`RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`、`migrations=[]`。deploy前後のAPP / AUTH pendingは`0 / 0`、root `200`、protected API `401`、bootstrap `404`。fresh backupはAPP `155,619 bytes` / SHA-256 `3E329C9CB247CB2B408D4CAFA9A151C152B7285D3D62EADD45304387A05CCE13`、AUTH `5,136 bytes` / SHA-256 `0250A77BBD964065CA785315AE4E14F0489ECEB4AA612160E2C670607A5D0BF3`でprivate ignored pathへ保存し、restoreは実行していない。
- Existing authorized nonprod sessionをcredential操作なしで利用し、2026-09-08 established future ordinary planned Entry（Task `01a07726-8846-707b-af03-8e0dd71d6760`、Entry `01a07726-8846-7009-bf80-eee709b5daf2`、Day `01a07724-839c-718f-addf-baab70224268`）を確認した。initial unset selectorのAX valueは`—`。Mode A `D068 Deep verification`をsetしfresh authenticated tabで保持、Mode B `D068 Light verification`へreplaceしfresh authenticated tabで保持、clear後にfresh authenticated tabで再び`—`を確認した。fresh-tab evidenceのためA/Bを追加で一度ずつ再確認したが、final target relationは不在。browser console logsは対象tab / final fresh tabとも空集合。
- Read-only APP evidenceはtarget identity / Day / planned ordinary / Routine nullを保持し、pre-deploy backupの`placement_revision=13`からfinal `13`でMode-only operations中不変。SetEntryMode成功operationはtarget EntryについてA→B→A→B→NULL、最終`entry_modes` target rowは不在、target以外を含むAPP countsはProjects / Tasks / Entries / Executions / operations `1 / 31 / 22 / 13 / 151`、Mode definitions / relations `2 / 2`、active Execution `0`、orphan Execution / entry_mode `0 / 0`。APP / AUTH final quick_check `ok`、FK empty、AUTH users / accounts / sessions `1 / 1 / 6`、全audit query `rows_written=0`。production、restore、cleanup、credential / bootstrap変更、releaseは`NOT_RUN`。

## D-071 Mode Settings Board UI parity — 2026-09-07

| ID | Area | Requirement | Evidence | Status |
|---|---|---|---|---|
| D071-WEB-01 | Structure | Project-parity header / add / dense rows / name + 52px action column、visible orderなし | focused Mode Board `12 / 12`のstructure / loading / empty coverage | PASS |
| D071-WEB-02 | Rename / menu | title-click / overflow inline rename、Enter / blur commit、Escape / unchanged / empty no mutation、same-title identity | focused Mode Boardのexact CAS / identity / menu coverage | PASS |
| D071-WEB-03 | D&D | whole-row、interactive suppression、midpoint before / after、no-op、revision / canonical persistence | focused Mode Boardのupper / lower midpoint、one exact reorder、no-op coverage | PASS |
| D071-WEB-04 | Keyboard / a11y | roving focus、J/K/Arrow、`?` help、focus trap、Escape / focus restore、IME / form / menu suppression、ARIA | focused Mode Boardのkeyboard / help / focus / suppression coverage | PASS |
| D071-WEB-05 | Feedback | create / rename / reorder success、error / exact ambiguity retry、no layout shift / focus steal | focused Mode Boardのsuccess / error / retained exact retry coverage | PASS |
...
| ID | Area | Requirement | Evidence | Status |
|---|---|---|---|---|
| D073-DOM-01 | Atomic lifecycle | ordinary running A -> interrupted history, ordinary planned B -> active, one planned same-Task continuation, max one active Execution | focused Worker `6 / 6` + persistent APP rows; post-closeout active `0` | PASS |
| D073-DOM-02 | Placement / estimate | B planned start unchanged; same-minute direct-after-B; different-minute interruption cohort; later-minute B remains after inserted continuation; no-cohort fallback; chain residual estimate; frozen Section | focused Worker `6 / 6` + persistent same-Section rows: cohort / continuation / B positions `5 / 6 / 7`, logical minutes `1256 / 1256 / 1320`, B planned minute unchanged | PASS |
| D073-DOM-03 | Safety / retry | stale active / placement reject, operation replay / misuse, injected D1 ambiguity leaves no partial rows | focused Worker `6 / 6`, APP guards / assertions `0 / 0`, all audit queries rows_written `0` | PASS |
...
| D073-REG-01 | Regression | normal Start / Complete, execution correction, delete completed FK cleanup, Mode management, D-072 toolbar | full Worker `24 files / 206 tests`, full Web `4 files / 229 tests`, focused Web `3 / 3`, typecheck / build / exact nonprod build / dry-run / diff-check | PASS |
| D073-MIG-01 | Migration | no new migration; existing APP 0023 remains unchanged; core rows, legacy singleton chains, NULL legacy outcomes, allow-list, trigger, FK / quick check | migration regression `4 scenarios passed` (fresh `0001 -> 0023` chain, R2A / R2B, duplicate-Task fail-safe, Bulk / Mode / Interrupt constraints) plus APP/AUTH pending `0 / 0` | PASS |
| D073-ENV-01 | Persistent nonprod | exact pushed-main deploy, safety probes, no pending migration, read-only APP/AUTH integrity | Worker `22adba9c-104b-4de9-89bc-ab0afc27ab5b`, `200 / 401 / 404`, APP/AUTH quick check `ok`, FK empty, audit rows_written `0` | PASS |
...
D-073 placement corrective closeout evidence: exact `main@8c94e30e4c9488e99626f461aae1f260a5162189`をgenerated nonprod configからdeployし、same-Sectionのinterrupt-minute cohort、continuation、later-minute Bをauthenticated browserで確認した。Bのplanned logical minuteは`1320`のまま、cohort / continuation / Bのpersistent positionsは`5 / 6 / 7`、continuationのlogical minuteは`1256`。same-minute direct-after-B、different-minute cohort-end、interrupt-minute cohortなしのfallbackはfocused Worker `6 / 6`で回帰した。既存の認証済みsessionを利用し、credential取得・推測・再設定、bootstrap再有効化、direct SQL feature mutation、production mutation、restore、cleanupは行っていない。複数continuationのProduct semanticsは`docs/OPEN_QUESTIONS.md`に残し、今回のplacement correctiveでは決定していない。
...
| ID | Area | Requirement | Evidence | Status |
|---|---|---|---|---|
| D075-WEB-01 | Layout boundary | date/navigation and Day toolbar stay outside the single Day scroll owner; Section summary / Task rows move inside it; no root/nested double vertical scroll | focused layout / DOM tests; fresh authenticated nonprod geometry at `1280 × 720`, surface `clientHeight=452`, `scrollTop=985` | PASS |
| D075-WEB-02 | Sticky header / alignment | table column heading remains visible with opaque background and heading / row tracks remain synchronized through horizontal scroll | heading `top=148..182` at top and bottom; `scrollLeft=464`, Project header / row both `left=157..307` | PASS |
| D075-WEB-03 | Overlay / interaction regression | row overflow is not clipped; Display / column submenu, calendar, modal, shortcut help, D-074 S / I, offscreen Arrow focus remain usable | portal parent `BODY`, fixed menu; persistent browser overlay / keyboard checks; console empty | PASS |
| D075-REG-01 | Regression | existing Day / Web / Worker behavior remains intact; no API / Domain / ordering / persisted-data change | focused Web `2 files / 211 tests`, full Web `4 files / 236 tests`, full Worker `24 files / 207 tests`, typecheck / build / diff-check | PASS |
| D075-MIG-01 | Migration / safety | no new schema / migration / dependency; APP / AUTH remain intact | generated nonprod config; remote APP/AUTH pending `0 / 0`; isolated backup recovery quick check `ok`, FK empty; local migration helper attempted but hung and was safely interrupted | PASS (`NOT_REQUIRED`) |
| D075-ENV-01 | Persistent nonprod | exact pushed-main build, generated config, dry-run, deploy, safety probes, read-only APP/AUTH evidence | main `8f7d00c`, Worker `2a1536fb-549c-423f-96d4-01777d95da22`, root / protected / bootstrap `200 / 401 / 404`, APP/AUTH quick/FK evidence | PASS |
| D075-ENV-02 | Authenticated browser | long Day top/middle/bottom, fixed regions, horizontal alignment, reload / fresh authenticated tab, overlays, console | existing authenticated session + fresh tabs; bottom geometry, row overflow portal, Display / calendar / modal / help, D-074 S / I, console warning/error `0 / 0` | PASS |
| D075-ENV-03 | Viewport resize | vertical resize with no overlap or double scrollbar | current CUA browser connector exposes no viewport resize API; static `1280 × 720` geometry and responsive flex CSS covered | NOT_RUN |

D-075 evidence closeout:
...
| D081-ENV-02 | Disabled bootstrap safety posture | `RUNTIME_ENV=nonprod`、`BOOTSTRAP_ENABLED=false`; root `GET /` `200`; unauthenticated `GET /api/v1/projects` `401`; exact unauthenticated `POST /api/internal/bootstrap` `404` | PASS |
...
| D082-CORR-03 | Corrective local regression | D-056 relevant date-move + Routine `2 files / 13 tests`; full Worker `25 files / 215 tests`; full Web `4 files / 257 tests`; typecheck / build / exact nonprod build / dry-run / diff-check | PASS |
...
| D085-MIG-SAFETY-01 | Pre-migration backup / recovery | Existing pre-0025 APP/AUTH SQL backups are present with recorded SHA-256; isolated `node:sqlite` recovery validation before apply reported quick_check ok and FK 0 for both | PASS; historical evidence verified read-only; no restore executed during corrective |
| D085-WORKER-01 | Routine Mode domain | default / occurrence override, explicit NULL Modeなし, propagation, conversion inheritance, delete clear, replay / misuse / revision / ambiguity boundaries | PASS; focused Worker 3 files / 19 tests; integration 5 / 5 |
| D085-CORR-WORKER-01 | Same-value occurrence no-op | inherited effective Mode same-value request persists success without creating override; existing override and explicit NULL semantics remain stable; CAS/replay boundary retained | PASS; routine-mode.integration.test.ts 5 / 5 |
| D085-CORR-SUPPRESSION-01 | Suppressed propagation | Routine Board default Mode change updates eligible no-override planned occurrence but leaves suppressed occurrence untouched | PASS; routine-mode.integration.test.ts 5 / 5 |
...
| D085-REG-01 | Existing regressions | full Worker/D1 26 files / 225 tests; typecheck / normal build / exact nonprod build / Wrangler dry-run / diff-check | PASS; Web source unchanged, prior full Web evidence retained |
| D085-ENV-01 | Exact persistent nonprod | Worker 39b39929-3c9a-4b1e-a3ab-a4fb1919e479; RUNTIME_ENV=nonprod; BOOTSTRAP_ENABLED=false; existing authenticated session reused | PASS |
| D085-BROWSER-01 | Default / occurrence browser behavior | default same-value reselect; default Light→Deep; explicit occurrence — remains unchanged; same-tab reload / fresh authenticated tab | PASS |
| D085-CONSOLE-01 | Browser console | authenticated tab console-log reads | PASS; 0 errors / 0 warnings |
...
| D085-CORR-LOCAL-01 | Corrective local regression | same-value no-op, override/NULL preservation, suppressed propagation, full Worker, typecheck/build/dry-run/diff-check | PASS; corrective integration 5 / 5; full Worker 26 files / 225 tests |
...
| D101-WORKER | Task Primary commands | owner-scoped Ensure/Update, exact replay/misuse, atomic relation+Document Create, revision CAS, concurrent Ensure convergence, same-result stale Update rejection, shared permalink resolver | Documents/Task Primary integration `25 / 25` | PASS |
| D101-WEB | Task Note surface | Note column, side-peek editor, Markdown source, explicit/Ctrl/Cmd Save, open-mode persistence, focus/close, shared `/?view=note&document=<document-id>` permalink, exact kind/owner resolution, no body in Day projection | App + TaskNoteEditor + corrective Web focused `297 / 297`; full Web `9 files / 372 tests` | PASS |
| D101-CORRECTIVE | v3 Day-column migration | v3 is discovered before v2/v1; custom order/width/hidden/task width are preserved; Note is inserted after Section; exactly one Mode remains and v4 is idempotent | `day-columns` corrective tests `8 / 8` | PASS |
| D101-RESOLVE | Shared Document route | standalone active/archived and Task Primary resolve by exact owner-scoped ID; Task Primary `task_title` comes from current Task authority even when Today does not contain the Task; missing/cross-owner IDs remain unavailable with no fallback; Task bootstrap replaces to shared URL | App route + Documents/Task Primary integration `25 / 25`; NotesBoard exact-route tests | PASS |
| D101-CORR2 | Task Primary title resolution | generic and legacy permalink routes use the resolver's owner-scoped current `tasks.title`; rename keeps the same Document ID/URL; missing relation/Task is `resource_not_found`; no `Task Note` fallback or Today dependency | focused route `2 / 2`, Task Primary integration `8 / 8`, deployed existing/fresh authenticated tabs | PASS |
| D101-COPY | Shared copy link | standalone and Task Primary editors copy `/?view=note&document=<document-id>` with understandable success/failure handling | NotesBoard + TaskNoteEditor focused tests | PASS |
| D101-REG | Existing behavior | Day projection, migration consumer, Notes, Routine, Settings, operation and lifecycle regressions | full Worker/D1 `33 files / 296 tests`; full Web `9 files / 372 tests` | PASS |
...
| D101-NONPROD | Exact deployment and unauthenticated safety | canonical Worker/bindings/vars, root `200`, protected Documents API `401`, APP/AUTH pending `0 / 0` | Worker `3306f046-486a-404c-bc1b-c3c3be4d9c69`; generated config/guard; HTTP probes | PASS |
| D101-BROWSER | Authenticated Task Note and standalone permalink persistence | Task Primary shared URL/copy/reload/fresh-tab with current Task-authoritative title; standalone active and archived exact URL, missing no-fallback, restore, copy, same-tab reload/fresh-tab; empty console warnings/errors | real CUA interaction on existing authenticated session; existing and fresh tab after corrective deploy showed Task title `てst`; no synthetic DOM events; console `[]` | PASS |
