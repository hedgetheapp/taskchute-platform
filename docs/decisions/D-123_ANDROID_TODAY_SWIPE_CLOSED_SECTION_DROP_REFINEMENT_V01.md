...
- For an eligible ordinary planned Task on the current established Day, left swipe reveals direct
...
- D-110 Task Primary Note eligibility is preserved independently from planning eligibility. A
  visible row with a valid `taskId` may expose a Note-only left-swipe affordance even when planning
