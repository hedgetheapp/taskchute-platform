...
- **Right → Left** (conventional left swipe; the finger moves from the right side toward the left) exposes direct `編集`, direct `ノート` when a valid `taskId` exists, and
  `その他`;
...
- From normal Today mode, **Left → Right** (conventional right swipe; the finger moves from the left side toward the right) on a selection-eligible Task row enters Selection Mode.
...
- Left → Right selection entry (conventional right swipe) must coexist with Right → Left Task actions (conventional left swipe) and long-press drag.
...
- If the Right → Left action menu (conventional left swipe) is open, a Left → Right gesture (conventional right swipe) on its owner closes to neutral only; a later separate Left → Right gesture from neutral may enter Selection Mode.
...
- planned and Running **Right → Left** (conventional left swipe) action composition and execution-action hiding;
- Running revealed-background continuity;
- Completed Note-only regression;
- **Left → Right** (conventional right swipe) Selection Mode entry with the swiped row initially selected;
- all-row checkbox visibility with ineligible rows remaining disabled;
- whole-row tap selection toggle in Selection Mode;
- no regression to Right → Left actions (conventional left swipe), Left → Right selection (conventional right swipe), long-press D&D, bulk action eligibility, lifecycle actions, or
